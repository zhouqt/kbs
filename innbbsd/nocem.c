...
NCMupdate(char *issuer, char *type)
{
	...
	fflush(fp);
	fclose(fp);
	innbbslog("NCMupdate add Issuer: %s , Type: %s\n", ISSUER, TYPE);
	sleep(1);
	if (readNCMfile(INNDHOME) == -1)
		innbbslog("fail to readNCMfile\n");
}
...
NCMregister()
{
	...
	if ((NNTP = inetclient(LeeymBBS, "7777", "tcp")) < 0) {
		innbbslog("NCMregister :Err: server %s %s error: cant connect\n", LeeymBBS, "7777");
		return 0;
	}
	if (!(NNTPrfp = fdopen(NNTP, "r")) || !(NNTPwfp = fdopen(NNTP, "w"))) {
		innbbslog("NCMregister :Err: fdopen failed\n");
		return 0;
	}
	fgets(NNTPbuffer, sizeof NNTPbuffer, NNTPrfp);
	if (atoi(NNTPbuffer) != 200) {
		innbbslog("NCMregister :Err: server error: %s", NNTPbuffer);
		return 0;
	}
...
NCMcancel()
{
	...
	innbbslog("NCMcancel %s %s, count:%d spam:%d, ok:%d fail:%d\n",
...
receive_nocem()
{
	...
	if (!num_spammid) {
		innbbslog("NCMparse: nothing to cancel\n");
		return 0;
	}
#ifdef  PGP5
	if (ncmdebug)
		innbbslog("NCM: verifying PGP sign\n");

	rel = NCMverify();

	if (rel != PGPGOOD) {
		innbbslog("NCMverify %s, %s, %s\n", errmsg, MSGID, ISSUER);
		return 0;
	}
#endif

	if (ncmdebug)
		innbbslog("NCM: canceling spam in NoCeM Notice\n");
	return NCMcancel();
}
#endif	
