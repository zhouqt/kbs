...
#if (defined(sgi) && (IRIX == 3)) || (defined(umips) && defined (SYSTYPE_SYSV))
    {
...
#define _POSIX_VDISABLE 0       /* or 0377? */
#endif
...
# endif
    ...
    newtermptr->tio.c_cc[VINTR] = _POSIX_VDISABLE;
    newtermptr->tio.c_cc[VQUIT] = _POSIX_VDISABLE;
    newtermptr->tio.c_cc[VINTR] = _POSIX_VDISABLE;
# ifdef VSTART
    newtermptr->tio.c_cc[VSTART] = _POSIX_VDISABLE;
# endif
# ifdef VSTOP
    newtermptr->tio.c_cc[VSTOP] = _POSIX_VDISABLE;
# endif
# ifdef VDISCARD
    newtermptr->tio.c_cc[VDISCARD] = _POSIX_VDISABLE;
# endif
# ifdef VSUSP
    newtermptr->tio.c_cc[VSUSP] = _POSIX_VDISABLE;
# endif
# ifdef VDSUSP
    newtermptr->tio.c_cc[VDSUSP] = _POSIX_VDISABLE;
# endif
    ...
    newtermptr->tc.t_quitc = _POSIX_VDISABLE;
    newtermptr->tc.t_intrc = _POSIX_VDISABLE;
    newtermptr->tc.t_startc = _POSIX_VDISABLE;
    newtermptr->tc.t_stopc = _POSIX_VDISABLE;
...
void addutmp()
{
    ...
    (void) strncpy((char *) utmp.ut_id, ttydev + strlen(ttydev) - PTYCHLEN, sizeof(utmp.ut_id));
...
#  endif
# endif
    (void) strncpy((char *) utmp.ut_user, (pw && pw->pw_name) ? pw->pw_name : "???", sizeof(utmp.ut_user));
    (void) strncpy((char *) utmp.ut_id, ttydev + strlen(ttydev) - PTYCHLEN, sizeof(utmp.ut_id));
    utmp.ut_pid = getpid();
...
# endif
     ...
     * common utmp fields 
     */
    (void) strncpy(utmp.ut_line, ttydev + strlen("/dev/"), sizeof(utmp.ut_line));
    (void) strncpy(utmp.ut_name, (pw && pw->pw_name) ? pw->pw_name : "???", sizeof(utmp.ut_name));
...
int child_pid;
{
    ...
    while (read(utmpf, (char *) &utmp, sizeof(utmp)) == sizeof(utmp)) {
        if (strncmp(utmp.ut_line, ttydev + strlen("/dev/"), sizeof(utmp.ut_line)) != 0) {
            tslot++;
            continue;
        }
