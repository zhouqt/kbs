...
char   *filename, *userid;
{
	...
	if (fd < 0) {
		innbbslog("<bbslink> Err: can't open %s\n", filename);
		return 0;
	}
	FD_SIZE = st.st_size;
	if (FD_BUF == NULL) {
...
time_t  mtime;
{
	...
	if (nf == NULL) {
		innbbslog("<bbslink> save_outgoing: No such board %s\n", board);
		return;
	} else {
		group = nf->newsgroups;
		server = nf->path;
		...
		*ptr2 = savech;
		...
		if (nl->host && *nl->host) {
			if (nl->feedfp == NULL) {
				...
				if (nl->feedfp == NULL) {
					innbbslog("<save outgoing> append failed for %s/%s.link", INNDHOME, nl->node);
				}
			}
			if (nl->feedfp != NULL) {
...
soverview_t *sover;
{
	...
	if (FN == NULL) {
		innbbslog("<save_article> err: %s %s\n", board, filename);
		if (Verbose)
...
#endif
#endif
			strncpy(FROM_BUF, fileglue("%s@%s (%s)", userid, MYADDR, nickname), sizeof FROM_BUF);
			FROM = FROM_BUF;
			sover.from = FROM;
			sover.board = board;
...
char   *textline;
{
	...
	if (USEIHAVE) {
		if (status == 235) {
			if (NEWSFEED == LOCAL) {
				innbbslog("Sendout <%s> from %s/%s\n", msgid, board, filename);
			}
		} else if (status == 437 || status == 435) {
			innbbslog("<bbslink> :Warn: %d %s <%s>\n", status, (char *) tcpmessage(), msgid);
			if (Verbose)
				printf(":Warn: %d %s <%s>\n", status, (char *) tcpmessage(), msgid);
			return 0;
		} else {
			innbbslog("<bbslink> :Err: %d %s of <%s>\n", status, (char *) tcpmessage(), msgid);
			if (Verbose)
				printf(":Err: %d %s of <%s>\n", status, (char *) tcpmessage(), msgid);
			queuefeed(node, textline);
			return 0;
		...
		if (status == 250) {
			innbbslog("<bbslink> DATA Sendout <%s> from %s/%s\n", msgid, board, filename);
			if (Verbose)
	...
	if (!USEDATA) {
		...
		if (!NoAction) {
			...
			if ((NNTP = inetclient(hostname, hostport, "tcp")) < 0) {
				innbbslog("<bbslink> :Err: server %s %s error: cant connect\n", hostname, hostport);
				if (Verbose)
			...
			if (atoi(NNTPbuffer) != 200) {
				innbbslog("<bbslink> :Err: server error: %s", NNTPbuffer);
				if (Verbose)
		...
		if (!NoAction) {
			...
			if ((NNTP = inetclient("localhost", hostport, "tcp")) < 0) {
				innbbslog("<bbslink> :Err: server %s port %s error: cant connect\n", hostname, hostport);
				if (Verbose)
	...
	if (BODY != NULL && !NoAction) {
		if (USEIHAVE) {
			...
			if (status == 335) {
				...
				innbbslog("<bbslink> :Warn: %d %s, IHAVE <%s>\n", status, (char *) tcpmessage(), msgid);
				if (Verbose)
					printf(":Warn: %d %s, IHAVE <%s>\n", status, (char *) tcpmessage(), msgid);
				returnstatus = 0;
			} else {
				innbbslog("<bbslink> :Err: %d %s, IHAVE <%s>\n", status, (char *) tcpmessage(), msgid);
				if (Verbose)
			...
			if (status == 340) {
				...
				innbbslog("<bbslink> :Warn: %d %s, POST <%s>\n", status, (char *) tcpmessage(), msgid);
				if (Verbose)
					printf(":Warn: %d %s, POST <%s>\n", status, (char *) tcpmessage(), msgid);
				returnstatus = 0;
			} else {
				innbbslog("<bbslink> :Err: %d %s, POST <%s>\n", status, (char *) tcpmessage(), msgid);
				if (Verbose)
			...
			if (status == 354) {
				...
				innbbslog("<bbslink> :Err: %d %s, DATA <%s>\n", status, (char *) tcpmessage(), msgid);
				if (Verbose)
...
char   *board, *filename, *from, *subject;
{
	...
	innbbslog("<cancel_outgoing> Try to move moderated post from %s to deleted\n", board);
	if (Verbose)
	...
	if (FN == NULL) {
		innbbslog("<cancel_outgoing> can't run %s/bbspost\n", INNDHOME);
		if (Verbose)
	...
	innbbslog("%s/bbspost cancel %s %s %s moderate\n", INNDHOME, BBSHOME, board, filename);
	if (Verbose)
...
close_link()
{
	...
	if (status != 205 && status != 221) {
		innbbslog("<bbslink> :Err: Cannot quit message '%d %s'\n", status, (char *) tcpmessage());
		if (Verbose)
...
char  **argv;
{
	...
	for (nlcount = 0; nlcount < NLCOUNT; nlcount++) {
		...
		if (localsendout || remotesendout || localfailed || remotefailed)
			innbbslog("<bbslink> [%s]%s lsend:%d rsend:%d lfail:%d rfail:%d\n",
...
int     sig;
{
	innbbslog("kill signal received %d, terminated\n", sig);
	if (Verbose)
