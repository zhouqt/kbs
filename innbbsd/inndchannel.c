...
ClientType *client;
{
	...
	if (client->ihavecount > 0 || client->statcount > 0) {
		innbbslog("%s@%s rec: %d dup: %d fail: %d size: %d, stat rec: %d fail: %d, time sec: %d\n",
...
*/
		...
		if (local != NULL & local->tm_hour == His_Maint_Hour &&
			...
			if (!maint) {
				innbbslog(":Maint: start (%d:%d).\n", local->tm_hour, local->tm_min);
				HISmaint();
				time(&now);
				local = localtime(&now);
				if (local != NULL)
					innbbslog(":Maint: end (%d:%d).\n", local->tm_hour, local->tm_min);
				maint = 1;
			}
		} else {
			maint = 0;
		...
		if (FD_ISSET(bbsinnd, &orfd)) {
			...
			if (getpeername(ns, (struct sockaddr *) & there, &length) >= 0) {
				...
				if ((char *) search_nodelist(client[i].hostname, client[i].username) == NULL) {
					innbbslog(":Err: invalid connection (%s@%s).\n", client[i].username, client[i].hostname);
...
char  **argv;
{
	...
	if ( ( getuid() != (uid_t) BBSUID && getgid() != (gid_t) BBSGID ) && !inetdstart ) {
