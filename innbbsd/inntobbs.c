...
int     type;
{
	...
	if (nf->path != NULL) {
		...
		for (ptr1 = nf->path; ptr1 && *ptr1;) {
			...
			*ptr2 = '\0';
			/*
			 * innbbslog("search node %s\n",ptr1);
			 */
			nl = (nodelist_t *) search_nodelist_bynode(ptr1);
			/*
			 * innbbslog("search node node %s, host %s fp
...
char   *
externfilter(indata, filterbuffer, cmd)
char   *indata, **filterbuffer, *cmd;
{
	...
	if (fp == NULL) {
		innbbslog("Err: can't popen %s\n", cmd);
		return NULL;
	}
	if (fwrite(indata, 1, len, fp) != len) {
		innbbslog("Err: fwrite to %s failed, len %d\n", cmd, len);
		return NULL;
	}
	pclose(fp);
