...
author: Mika Kojo, 1996

Copyrights (c) 1996 SSH Communications Security Oy, Espoo, Finland
              
Code is heavily based on Bruce Schneier's, implementor of blowfish, code.

This algorithm is ideal for long usage of one secret key. Each time 
key is changed all S and P -boxes are to be recalculated. 
...
static word32 blowfish_pbox[16 + 2] =
{
...
static word32 blowfish_sbox[256 * 4] =
{
...
/* Round loop unrolling macros,
   S is a pointer to a S-Box array organized in 4 unsigned longs at
   a row.
   */
...
static void blowfish_encrypt(BlowfishContext *context,
...
static void blowfish_decrypt(BlowfishContext *context,
			     word32 xl, word32 xr, word32 *output)
{
  word32 yl;
  word32 yr;
...
void blowfish_transform(word32 l, word32 r, word32 *output,
		        int encrypt, void *context)
{
  if (encrypt)
    blowfish_encrypt((BlowfishContext *)context,
		     l, r, output);
  else
    blowfish_decrypt((BlowfishContext *)context,
		     l, r, output);
}
...
void blowfish_cbc_encrypt(BlowfishContext *ctx, unsigned char *dst, 
			  const unsigned char *src, unsigned int len)
{
  ...
  iv0 = GET_32BIT_LSB_FIRST(ctx->iv);
...
void blowfish_cbc_decrypt(BlowfishContext *ctx, unsigned char *dst, 
			  const unsigned char *src, unsigned int len)
{
		      ...
		      int for_encyption)
{
  ...
  for (j = 0, i = 0; i < 16 + 2; i++)
    {
      temp = (((word32)key[j] << 24) |
	     ((word32)key[(j + 1) % keybytes] << 16) |
	     ((word32)key[(j + 2) % keybytes] << 8) |
	     ((word32)key[(j + 3) % keybytes]));
