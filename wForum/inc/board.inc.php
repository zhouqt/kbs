...
<?php 
	if ($threadNum==0) {
		...
		echo '<img loaded="no" src="pic/plus.gif" id="followImg'.($i+$start).'" style="cursor:hand;" onclick="loadThreadFollow('.$articles[$i]['ID'].",'".$boardName."')\" title=展开贴子列表>";
	}
...
<?php
			if ($threadNum>0) {
?>
<tr style="display:none" id="follow<?php echo $i+$start; ?>"><td colspan=5 id="followTd<?php echo $i+$start;?>" style="padding:0px"><div style="width:240px;margin-left:18px;border:1px solid black;background-color:lightyellow;color:black;padding:2px" onclick="loadThreadFollow(<?php echo $i;?>,'<?php echo $boardName; ?>')">正在读取关于本主题的跟贴，请稍侯……</div></td></tr>
<?php
			}
		}
?>
