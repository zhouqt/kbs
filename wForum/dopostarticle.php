...
function doPostAritcles($boardID,$boardName,$boardArr,$reID){
	...
	$ret=bbs_postarticle($boardName,preg_replace("/\\\(['|\"|\\\])/","$1",trim($_POST['subject'])),preg_replace("/\\\(['|\"|\\\])/","$1",$_POST['Content']),intval($_POST['signature']), $reID,$outgo,intval($_POST['anonymous']),intval($_POST['emailflag']),intval($_POST['texflag']));
	switch ($ret) {
		case -1:
			foundErr("错误的讨论区名称。");
		case -2:
