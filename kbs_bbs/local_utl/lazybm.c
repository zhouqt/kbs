...
{
    if (brd->level&PERM_NOZAP) return 1;
    else if (brd->level&PERM_LOGINOK) return 1;
    return (brd->level==0);
}
...
{
    int i, j, uid;
    if( !strcasecmp(uname, "SYSOP") || !isascii(*uname) ) return 0;
    if( ! isNormalBoard(&(bcache[nbrd])) ) return 0;
    /*  Anonymous board */
...
/*        return 0;*/
    ...
    if(j >= MAXUSRBM) fprintf(stderr, "%s too many BMs!\n", uname);
    return i;
}
...
int isUserOnline(int uid)
{
    ...
    for(i=0; i<USHM_SIZE; i++) {
        if(!(utmpshm->uinfo[i].active)) continue;
        if(uid == utmpshm->uinfo[i].uid) return (i+1);
    }
    return 0;
}
...
/*    char * fmt = " %14.14s%36.36s%24.24s%8.8s";*/
    ...
    for(i=0; i<nBMCount; i++) {
        if(i%2) { fgc = 37; bgc = 44; }
        else { fgc = 31; bgc = 46; }

        pBM = &(pBMInfo[i]);
        if(!(uid = pBM->uid)) /* continue */;
        if(!(brd = pBM->brd[0])) continue;    /*  maybe error???  */

        if(isUserOnline(uid)) pBM->lastlogin = now;
        dftime = (now - pBM->lastlogin)/86400;
        if( dftime >= nWarningTime && uid) warningBM ++;   /*---*/
        if(uid)
            strftime(mdate, 30, timefmt, localtime(&(pBM->lastlogin)));
/*        else if(!(pBM->brd[1]) && bcache[brd-1].flag & ANONY_FLAG)*/
/*            strcpy(mdate, "[匿名版所属帐号]");*/
        else strcpy(mdate, "[该帐号已注销]");
        sprintf(tbuf, "%d", dftime);
        sprintf(lbuf, fmt, fgc, bgc, pBM->uname, bcache[brd-1].filename,
                mdate, uid ? tbuf : nulstr);
        fprintf(stdout, "%s\n", lbuf);
        for(j=1;j<MAXUSRBM;j++) {
            if(!(brd = pBM->brd[j])) break;
            sprintf(lbuf, fmt, fgc, bgc, nulstr, bcache[brd-1].filename,
                    nulstr, nulstr);
            fprintf(stdout, "%s\n", lbuf);
        }
    }
...
int main(int argc, char ** argv)
{
    ...
    for(i=0;i<get_boardcount();i++) {
        if(!(bcache[i].filename[0])) continue;  /* invalid board    */
        if(bcache[i].BM[0] <= ' ') continue;    /* no board manager */
    ...
    if(checkRecord(passfile) < 0) exit(-1);
