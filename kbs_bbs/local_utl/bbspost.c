...
int visitflag;
char emailad[STRLEN];
char fromhost[IPLEN + 1];       /* Leeward: 97.12.20: ÈÃ 3W ·¢ÎÄÓ°ÏìÉúÃüÁ¦ */
char EMode = ' ';               /* Leeward: 97.12.23: for 3W modify article */
int sign;                       /*Haohmaru.µÚsign¸öÇ©Ãûµµ */

struct userec alluser;

int CheckBM(BMstr, record)      /* ¸ù¾ÝÊäÈëµÄ°æÖ÷Ãûµ¥ ÅÐ¶Ïµ±Ç°userÊÇ·ñÊÇ°æÖ÷ added by dong, 1998.9.28 */
    char *BMstr;
    ...
    int usermail;
{
    ...
    if ('L' == EMode) {
        ...
        if (!HAS_PERM(currentuser, PERM_SYSOP) && !HAS_PERM(currentuser, PERM_LOGINOK)) {       /* Leeward 98.07.26 ¼ÓÈëÈ¨ÏÞÅÐ¶Ï */
            printf(":Err: User %s try to send letter while not registered .\n", userid);
            return;
        }

        if (HAS_PERM(currentuser, PERM_DENYMAIL)) {     /* Bigman: 2000.9.5 ·â½ûMail ²»ÄÜ·¢ÐÅ */
...
/*        search_record(BOARDS, &brdhdr, sizeof(brdhdr), cmpbnames, ptr);*/
    ...
    if (usermail) {
        ...
        } else {                /*now+=28800;Haohmaru.99.4.21.²»ÖªµÀÎªÊ²Ã´WWWµÄÊ±ÖÓ±ÈÏµÍ³Ê±ÖÓÂý8Ð¡Ê± */
            sprintf(buf, "·¢ÐÅÈË: %s (%s), ÐÅÇø: %s\n±ê  Ìâ: %s\n·¢ÐÅÕ¾: BBS " NAME_BBS_CHINESE "Õ¾ (%24.24s) \033[1m\033[32mWWW-POST\033[m\033[m\n\n", userid, currentuser->username,
