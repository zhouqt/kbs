...
main(int argc, char *argv[])
{
    ...
    t = fsize(inf_name);
    printf("Old: %s, %d x %d = %d (%d) %s\n", inf_name, record_num, inf_size, inf_size * record_num, t, t == inf_size * record_num ? "OK" : "Error");

    t = fsize(outf_name);
    printf("New: %s, %d x %d = %d (%d) %s\n", outf_name, record_num, outf_size, outf_size * record_num, t, t == outf_size * record_num ? "OK" : "Error");
}
