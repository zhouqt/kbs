...
/* 处理信件发送 */
static int m_send(struct userec *user,void *arg){
    return (!m_check_perm(user)?0:mail_file(name,path,user->userid,title,m_check_mode(),NULL));
}
...
#define ME_QUIT(s) do{fprintf(stderr,"error: %s\n",s);exit(__LINE__);}while(0)
    ...
    opterr=0;
    while((ret=getopt(argc,argv,"p:u:l:m:s:h"))!=-1){
        switch(ret){
            case 'p':                           /* 权限检测 */
                flag|=MC_PERM;
