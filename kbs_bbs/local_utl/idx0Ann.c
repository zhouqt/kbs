...
/*    char psPrefix[SHORTLEN];*/    /*  output prefix string    */
	...
	if(NULL == dname) return gnIgnoreError;
	llen = strlen(dname) - 1;
	if('/' == dname[llen]) dname[llen] = 1;	/*	erase the trailing '/'	*/
	...
	ptr = strrchr(dname, '/');
	if(NULL == ptr) strcpy(dirNameEn, dname);
	else strcpy(dirNameEn, ptr+1);

	if( NULL != brdtitle && strlen(brdtitle)>0 )
		strcpy(vcTitle, brdtitle);
	else strcpy(vcTitle, "无标题");
				...
				if(NULL != (ptr = strrchr(buf, '('))) {
					if(!strncmp(ptr, "(BM: ", 5)) {
						*ptr = 0;		/* form title str in buf */
						...
						if(NULL != end) *end = 0;	/* BM: ... field */
						else *psDirBM = 0;
						end = strchr(psDirBM, ' ');	/* only one id allowed */
						if(NULL != end) *end = 0;
					} else
						if(NULL != bmstr && strlen(bmstr) > 0)
...
/*				strftime(buf, 20, "%Y-%m-%d %T", localtime(&ltt_dir));*/
		...
		if(!strncmp(psName+38, "(BM: BMS)", 9) || !strncmp(psName+38, "(BM: SYSOPS)", 12))
			continue;
		if(strlen(psPath) <= 0) continue;	/* not to form loops */
		strcpy(fname, dname);
		strcat(fname, psPath);
		...
		if(S_ISDIR(lst_p.st_mode)) {		/*	sub-directory	*/
			...
			fprintf(pOutFile, "%*.*s\033[33m%4d\033[37m. [\033[1;32m目录\033[37m]"
					...
					gnDirDepth<7 ? gnsDirBackColor[gnDirDepth] : 0,
					gnDirDepth<7 ? gnsDirForeColor[gnDirDepth] : 37,
					lntval, psName,
					70-gnDirDepth*3-lntval/*llen*/,
					psCTime);
...
/*			if(lntval > llen) lntval = llen;*/
			fprintf(pOutFile, "%*.*s%4d. [\033[1;36m文件\033[0;37m]"
					...
					(gnDirDepth-1)*3, (gnDirDepth-1)*3, " ", nNumber,
					lntval, lntval, psName,
					70-gnDirDepth*3-lntval,
					psCTime);
	...
	while(++idx < argc) {
		switch(*(argv[idx])) {
			...
			default:	/*	root dir of source directory tree	*/
				...
				if('/' == psSrcDir[len]) psSrcDir[len] = 0;
				break;
		}
	}
	if(strlen(psSrcDir) <= 0) return -1;
	return 0;
}
