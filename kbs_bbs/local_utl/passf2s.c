...
#define PASSWDS_FILE ".PASSWDS.HAPPY"
#undef printf
#undef perror
...
convert_userdefine(struct olduserec *olduser, struct userec *user)
{
	...
	if (!(oldudef & MY_DEF_LOGOUT))
		udef &= ~DEF_LOGOUT;
	/*if (!(oldudef & MY_DEF_LOGINFROM))
		udef &= ~DEF_LOGINFROM;*/
	if (!(oldudef & MY_DEF_NOTEPAD))
		udef &= ~DEF_NOTEPAD;
	if (!(oldudef & MY_DEF_GRAPH))
		udef &= ~DEF_SHOWSTATISTIC;
	if (!(oldudef & MY_DEF_TOP10))
		udef &= ~DEF_SHOWHOT;
	if (!(oldudef & MY_DEF_SHOWHOROSCOPE))
		udef &= ~DEF_SHOWHOROSCOPE;
	if (!(oldudef & MY_DEF_RANDSIGN))
		udef &= ~DEF_RANDSIGN;
