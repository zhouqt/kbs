...
#define BOARDS_FILE ".BOARDS.HAPPY"

#undef printf
#undef perror

static void
convert_flag(struct oldboardheader *oldbrd, struct boardheader *brd)
{
	char oldflag;
	char flag;
...
static void
convert_title(struct oldboardheader *oldbrd, struct boardheader *brd)
{
	int i;
	...
	for (i = 0; i < SECNUM; i++)
	{
		if (strchr(oldseccode[i], oldbrd->title[0]) != NULL)
		{
			...
			snprintf(brd->title+1, STRLEN-1, "%6.6s      %s", 
					oldbrd->title+1, oldbrd->title+11);
		}
	}
}

static void
convert_permission(struct oldboardheader *oldbrd, struct boardheader *brd)
{
...
static void
set_article_ids(struct oldfileheader *base, int id, 
		struct fileheader *fh, struct oldfileheader *oldfh)
{
	int i;
...
static void
convert_article_flag(struct oldfileheader *oldfh, struct fileheader *fh)
{
	char flag1, flag2;
	char oldflag;
...
static int
convert_board_articles(struct boardheader *brd)
{
	char dir[256];
	struct stat fs;
	int records;
...
convert_board(struct oldboardheader *oldbrd, struct boardheader *brd)
{
	...
	bzero(brd->unused, sizeof(brd->unused));
}
