	...
	char *argv[];
{
	...
	for (j = 0; j < 24; j++) {
		...
		/* Ã¿Ò»Ç§»»Ò»ÖÖÑÕÉ« Czz 020507 */
		if (st.no[j] >= 5000)
		    /*fprintf(fp, "[35m###[33m"); Leeward 97.12.08 */
		    fprintf(fp, "\033[1m[36m%-3d\033[m[33m", (st.no[j]) % 1000);
		else if (st.no[j] >= 4000)
		    fprintf(fp, "\033[1m[35m%-3d\033[m[33m", (st.no[j]) % 1000);
		else if (st.no[j] >= 3000)
		    fprintf(fp, "\033[1m[34m%-3d\033[m[33m", (st.no[j]) % 1000);
		else if (st.no[j] >= 2000)
		    fprintf(fp, "\033[1m[33m%-3d\033[m[33m", (st.no[j]) % 1000);
		else if (st.no[j] >= 1000)
		    fprintf(fp, "\033[1m[32m%-3d\033[m[33m", (st.no[j]) % 1000);
		else
		    fprintf(fp, "[31m%-3d[33m", (st.no[j]));
		continue;
	    }
