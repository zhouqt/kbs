/*仅仅修改过sturct fileheader_v1_2的转化fileheader的程序*/
#include "bbs.h"
...
static void strip_fileheader(const fileheader_v1_2 * oldfh, fileheader * fh, const char *bname, const char *dir)
{
    ...
    fh->reid = oldfh->reid;
#if defined(FILTER) || defined(COMMEND_ARTICLE)
    if (strcmp(bname, FILTER_BOARD) == 0 || strcmp(bname, COMMEND_ARTICLE) == 0) {
        if (oldfh->o_board[0] != '\0' && (fh->o_bid = getboardnum(oldfh->o_board, NULL)) == 0) {
            fprintf(stderr, "Warning: Cannot find original board <%s>: %s/%s/id<%d>.\n", oldfh->o_board, bname, dir, oldfh->id);
        }
        fh->o_id = oldfh->o_id;
    ...
    fh->accessed[sizeof(fh->accessed) - 1] = oldfh->accessed[sizeof(oldfh->accessed) - 1];
}
