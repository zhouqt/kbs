...
/* from bbs.c */
static void 
record_exit_time(char* userid)   /* ¼ÇÂ¼ÀëÏßÊ±¼ä  Luzi 1998/10/23 */
{
    char path[80];
    FILE *fp;
    time_t now;
...
int full_utmp(struct user_info* uentp,int* count)
{
    ...
    if(!HAS_PERM(currentuser,PERM_SEECLOAK) && uentp->invisible 
		&& strcmp(uentp->userid,getcurruserid()))/*Haohmaru.99.4.24.ÈÃÒşÉíÕßÄÜ¿´¼û×Ô¼º*/
    {
        return 0;
    }
