					...
					if (!strcasecmp(UBBCode,"upload")){
						if ( (UBBArg1>0) && (UBBArg1<=attachmatched)) {
							...
							case ATTACH_IMG:
								...
								break;
							case ATTACH_FLASH:
				                snprintf(outbuf, 511, "<br>Flash¶¯»­: " "<a href='%s&ap=%d'>%s</a> (%d ×Ö½Ú)<br>" "<OBJECT><PARAM NAME='MOVIE' VALUE='%s&amp;ap=%d'>" "<EMBED SRC='%s&amp;ap=%d'></EMBED></OBJECT><br />", attachlink, attachPos[UBBArg1-1], attachFileName[UBBArg1-1], attachLen[UBBArg1-1], attachlink, attachPos[UBBArg1-1], attachlink, attachPos[UBBArg1-1]);
								break;
							case ATTACH_OTHERS:
	...
	for ( i = 0; i<attachmatched ; i++ ){
		if (!attachShowed[i]) { 
			...
			case ATTACH_IMG:
				...
				break;
			case ATTACH_FLASH:
		        snprintf(outbuf, 511, "<br>Flash¶¯»­: " "<a href='%s&ap=%d'>%s</a> (%d ×Ö½Ú)<br>" "<OBJECT><PARAM NAME='MOVIE' VALUE='%s&amp;ap=%d'>" "<EMBED SRC='%s&amp;ap=%d'></EMBED></OBJECT><br />", attachlink, attachPos[i], attachFileName[i], attachLen[i], attachlink, attachPos[i], attachlink, attachPos[i]);
				break;
			case ATTACH_OTHERS:
...
/* added by roy 2003.7.17 generate .ORIGIN index file*/
{
    ...
    setboardorigin(board, 0);   /* ±ê¼Çflag*/
