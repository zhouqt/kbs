...
int post_article(char *board, char *title, char *file, struct userec *user, char *ip, int sig, int local_save, int anony)
{
    ...
    fclose(fp);
    add_loginfo2(filepath, board, user, anony);	/*Ìí¼Ó×îºóÒ»ĞĞ */
...
{
    int x;			/* °æÃæ²»´æÔÚ·µ»Ø0, pºÍz°æÃæ·µ»Ø1, ÓĞÈ¨ÏŞ°æÃæ·µ»Ø1. */
...
int full_utmp(struct user_info *uentp, int *count)
{
    ...
    if (!HAS_PERM(currentuser, PERM_SEECLOAK) && uentp->invisible && strcmp(uentp->userid, getcurruserid())) {	/*Haohmaru.99.4.24.ÈÃÒşÉíÕßÄÜ¿´¼û×Ô¼º */
	return 0;
    }
...
*/
/*
 * get_unifile() ÓÃÓÚ»ñµÃÒ»¸ö¶ÀÒ»ÎŞ¶şµÄÎÄ¼ş.
 *     filename  ÎÄ¼şÃû»º³åÇø
 *     key       ¹Ø¼ü×Ö, ¿ÉÒÔÊÇ°æÃû»òÕßÊÇÓÃ»§Ãû
 *     mode      == 0, ±íÊ¾ filename ÊÇ·¢±íÎÄÕÂÓÃµÄÎÄ¼şÃû
 *                     ´ËÊ± key ÊÇ°æÃû
 *               == 1, ±íÊ¾ filename ÊÇÍ¶µİÓÊ¼şÓÃµÄÎÄ¼şÃû
 *                     ´ËÊ± key ÊÇÓÃ»§Ãû
 */
...
/* WWW GUESTÕâÑù×öÓĞ¸öÍ¬²½ÎÊÌâ£¬¾ÍÊÇµ±±»Çå³ıÒ»¸ö
  GUESTµÄÊ±ºòÈç¹ûÕıºÃÕâ¸öguestË¢ĞÂÁË£¬ÄÇÃ´»áÖØĞ´Êı¾İ½á¹¹
  ËùÒÔ£¬Òª×¢Òâ³ıÁËkeyÖ®ÍâµÄÊı¾İÈç¹ûÒª±ä¶¯£¬±ØĞë±£Ö¤
  ÔÚÕâÖÖÇé¿öÏÂ²»»áÂß¼­»ìÂÒ¡£freshtimeÒòÎª¼´Ê¹±»´íÎóµÄ
  client¸üĞÂÁË£¬Îó²îÔÚÃë¼¶£¬¶øÇÒÒ²²»»áÔì³ÉÓÃ»§ÎÊÌâ£¬ËùÒÔ
  ²»»á³ö´í¡£µ«ÊÇÈç¹ûĞèÒª¸üĞÂkey£¬ÄÇÃ´¾ÍÓĞ¿ÉÄÜµ¼ÖÂÏÂÒ»¸ö
  guestÓÃ»§µÄkey±»´íÎóµÄ¸²¸Ç¡£Õâ¸öÊÇ¸öÎÊÌâ
  */
struct WWW_GUEST_S {
    int key;
    time_t freshtime;
...
#define MAX_WWW_MAP_ITEM (MAX_WWW_GUEST/32)	/* ³ıÒÔ32 */
#define MAX_WWW_GUEST_IDLE_TIME 3600	/* www guest·¢´ôÊ±¼äÉèÎª1Ğ¡Ê± */
...
int www_data_init()
{
    ...
    /* www_guest_infoÄ¿Ç°ÏÈÊ¹ÓÃÒ»¸öÈ«¾Ö±äÁ¿À´×ö£¬ÕâÑù
       »áµ¼ÖÂÏß³Ì²»°²È«:P   µ«ÊÇ¶ÔÓÚ½ø³ÌÄ£ĞÍµÄcgi ºÍphp
       ×ã¹»ÁË
     */
...
int www_user_logoff(struct userec *user, int useridx, struct user_info *puinfo, int userinfoidx)
{
    ...
    /* ÉÏÕ¾Ê±¼ä³¬¹ı 2 Ğ¡Ê±°´ 2 Ğ¡Ê±¼Æ */
    if (stay > 7200)
	stay = 7200;
    user->stay += stay;
    record_exit_time(user->userid);
    if (strcasecmp(user->userid, "guest")) {
	bbslog("1system", "EXIT: Stay:%3ld (%s)[%d %d](www)", stay / 60, user->username, get_curr_utmpent(), useridx);
	if (!puinfo->active)
	    return 0;
	...
	bbslog("1system", "EXIT: Stay:%3ld (guest)[%d %d](www)", stay / 60, puinfo->destuid, useridx);
