...
int user_init(struct userec *x, struct user_info **y)
{
	...
	/* ’‚¿Ô”–Œ Ã‚, (*y)‘⁄∫Û√ÊΩ´“ª÷±÷∏œÚ&(utmpshm_ptr->uinfo[i]),
	 * ≤ªπ‹∫Û√Êµƒƒ«–©≈–∂œ «∑Ò≥…π¶ */
	(*y) = get_user_info(i);
...
char *checked;          /* ∏ƒ∂Ø±æ∫Ø ˝±ÿ–ÎÕ¨≤Ω bbs.c ∫Õ bbssnd.c (4 WWW) */
{
    struct stat st;
    char        buf[STRLEN];
...
int deny_me(char *board)   /* ≈–∂œµ±«∞”√ªß  «∑Ò±ªΩ˚÷π∑¢±ÌŒƒ’¬ */
{
    char buf[STRLEN];
...
void add_loginfo2(char *filepath, char *board, struct userec *user, int anony)
{
    ...
    color=(user->numlogins%7)+31; /* —’…´ÀÊª˙±‰ªØ */
	...
	{ /*Bigman 2000.8.10–ﬁ∏ƒ,ºı…Ÿ¥˙¬Î */
        fprintf(fp,"\n");
	}
    /* ”…Bigman‘ˆº”:2000.8.10 Announce∞Êƒ‰√˚∑¢ŒƒŒ Ã‚ */
    if (!strcmp(board,"Announce"))
        fprintf(fp, "[m[%2dm°˘ ¿¥‘¥:°§%s %s°§[FROM: %s][m\n",
				color, "BBS "NAME_BBS_CHINESE"’æ", NAME_BBS_ENGLISH,
                NAME_BBS_CHINESE" BBS’æ");
    else
        fprintf(fp, "\n[m[%2dm°˘ ¿¥‘¥:°§%s %s°§[FROM: %s][m\n",
				color, "BBS "NAME_BBS_CHINESE"’æ", NAME_BBS_ENGLISH,
					...
					struct userec *user, int anony)
{
	...
	fprintf(fp, 
			"±Í  Ã‚: %s\n∑¢–≈’æ: %s (%24.24s)\n\n",
			title, "BBS "NAME_BBS_CHINESE"’æ", wwwCTime(time(0)));
}
		...
		char *ip, int sig, int local_save, int anony)
{
    ...
    anonyboard = seek_in_file("etc/anonymous",board); /*  «∑ÒŒ™ƒ‰√˚∞Ê */
	...
	fclose(fp);
    add_loginfo2(filepath, board, user, anony); /*ÃÌº”◊Ó∫Û“ª––*/
    ...
    /* ‘⁄boards∞Ê∞Ê÷˜∑¢Œƒ◊‘∂ØÃÌº”Œƒ’¬±Íº« Bigman:2000.8.12*/
...
int has_read_perm(struct userec *user, char *board) {  
	bcache_t *x;	/* ∞Ê√Ê≤ª¥Ê‘⁄∑µªÿ0, p∫Õz∞Ê√Ê∑µªÿ1, ”–»®œﬁ∞Ê√Ê∑µªÿ1. */
...
char msgstr[256];
{
    ...
    *msgbak = 0;	/* period 2000-11-20 may be used without init */
    ...
    if (NA==canIsend2(uin->userid))/*Haohmaru.06.06.99.ºÏ≤È◊‘º∫ «∑Ò±ªignore*/
        return -1;	/* ∂‘∑Ωæ‹æ¯ƒ„µƒ—∂œ¢ */
...
{
	unsigned char *ppt = s; /* ’€––¥¶ */
	unsigned char *pp  = ppt; /* –– ◊ */
	unsigned int  LLL = 78; /* ’€––Œª÷√ */
...
time_t get_exit_time(id,exittime) /* ªÒ»°¿Îœﬂ ±º‰£¨id:”√ªßID,
                                   exittime:±£¥Ê∑µªÿµƒ ±º‰£¨Ω· ¯∑˚Œ™\n
                                            Ω®“È∂®“ÂŒ™ char exittime[40]
                                   Luzi 1998/10/23 */
...
int valid_filename(char *file)
{
	...
	if(strncmp(file, "M.", 2) && strncmp(file, "G.", 2))
		/*http_fatal("¥ÌŒÛµƒ≤Œ ˝1");*/
		return -1;
	if(strstr(file, "..") || strstr(file, "/"))
		/*http_fatal("¥ÌŒÛµƒ≤Œ ˝2");*/
		return -2;
	return 0;
}
...
int chk_currBM(char *BMstr)   /* ∏˘æ› ‰»Îµƒ∞Ê÷˜√˚µ• ≈–∂œµ±«∞user «∑Ò «∞Ê÷˜ */
{
    char *ptr;
...
/* from bbs.c */
void 
record_exit_time()   /* º«¬º¿Îœﬂ ±º‰  Luzi 1998/10/23 */
{
    char path[80];
    FILE *fp;
    time_t now;
...
int full_utmp(struct user_info* uentp,int* count)
{
    ...
    if(!has_perm(PERM_SEECLOAK) && uentp->invisible 
		&& strcmp(uentp->userid,getcurruserid()))/*Haohmaru.99.4.24.»√“˛…Ì’ﬂƒ‹ø¥º˚◊‘º∫*/
    {
        return 0;
    }
...
void load_favboard(int dohelp)
{
    ...
    if( (fd = open( fname, O_RDONLY, 0600 )) != -1 ) {
        ...
        int savmode;
        savmode = uinfo.mode;
        modify_user_mode(CSIE_ANNOUNCE);	// √ª∫œ  µƒmode.æÕœ»”√"º≥»°æ´ª™"∞…. 
