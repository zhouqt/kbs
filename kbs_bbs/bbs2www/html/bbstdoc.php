<?php
	/**
	 * This file lists threaded articles to user.
	 * $Id$
	 */
	require("funcs.php");
login_init();
	...
	else
	{
		...
		if ($start < 0)
			$start = 1;
		$articles = bbs_getarticles($board, $start, $artcnt, $dir_modes["THREAD"]);
		if ($articles == FALSE)
...
<td>
<a href="/bbsocon.php?board=<?php echo $brd_encode; ?>&file=<?php echo $article["FILENAME"]; ?>&num=<?php echo $start + $i; ?>"><?php echo htmlspecialchars($title); ?> </a>
</td>
</tr>
<?php
			$i++;
