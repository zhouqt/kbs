...
/*	if (yank_flag == 0)
	...
	"<a target=\"f3\" href=\"bbsfav?select=%d\">返回上一级</a></td>"
		...
		if(ptr->flag==-1){
			...
			if(j!=-1){
		     		printf("<tr><td>%d</td><td><img src=\"/images/folder.gif\"></td><td>"
						...
						"<td><a target=\"f3\" href=\"bbsfav?select=%d&delete=%d\">删除</a></td></tr>\n",
			...
			printf("<td>%c<a href=\"bbsdoc?board=%s\">%s</a></td>",
				   ptr->zap ? '*' : ' ', buf, ptr->name);
			printf("<td>%6.6s</td>", ptr->title+1);
			printf("<td><a href=\"bbsdoc?board=%s\">%s</a></td>",
					buf, ptr->title+7);
	...
	printf("</table>");
	printf("<form action=bbsfav>增添目录<input name=dname size=24 maxlength=20 type=text value=''> "
"<input type=submit value=确定><input type=hidden name=select value=%d></form>", sec);
	printf("<form action=bbsfav>增添版面<input name=bname size=24 maxlength=20 type=text value=''> "
"<input type=submit value=确定><input type=hidden name=select value=%d></form>", sec);
	printf("<hr></center>\n");
	return brdnum;
}
