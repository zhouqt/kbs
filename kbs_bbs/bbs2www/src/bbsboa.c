...
int brd_show_boards(int sec, char *cgi, int yank)
{
    ...
    if (yank_flag == 0) {
	printf("[<a href=\"bbsboa?group=%d&yank=1\">本区所有看版</a>]", sec, yank_flag);
    } else
	...
	printf("<td>%c<a href=\"%s?board=%s\">%s</a></td>", ptr->zap ? '*' : ' ', cgi, buf, ptr->name);
	printf("<td>%6.6s</td>", ptr->title + 1);
	printf("<td><a href=\"%s?board=%s\">%s</a></td>", cgi, buf, ptr->title + 7);
