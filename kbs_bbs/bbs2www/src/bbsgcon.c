...
int main()
{
    ...
    if (num > 1) {
        ...
        printf("[<a href=\"bbsgcon?board=%s&file=%s&num=%d\">上一篇</a>]", board_url, x.filename, num - 1);
    }
    printf("[<a href=\"/bbsdoc.php?board=%s\">本讨论区</a>]", board_url);
    if (num < total) {
        fseek(fp, sizeof(x) * ((num-1) + 1), SEEK_SET);
        fread(&x, sizeof(x), 1, fp);
        printf("[<a href=\"bbsgcon?board=%s&file=%s&num=%d\">下一篇</a>]", board_url, x.filename, num + 1);
    }
    fclose(fp);
    ptr = x.title;
    if (!strncmp(ptr, "Re: ", 4))
