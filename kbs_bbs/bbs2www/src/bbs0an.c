...
int main() {
	...
	if(total<=0) {
		...
		http_quit();
       	...
       	printf("<tr><td>编号<td>类别<td>标题<td>作者<td>日期");
	for(i=0; i<total; i++) {
		char *id;
		...
		printf("<tr><td>%d", index+1);
		sprintf(buf, "0Announce%s%s", path, file[i]);
		if(!file_exist(buf)) {
			printf("<td>[错误] <td>%s", nohtml(name[i]));
		} else if(file_isdir(buf)) {
			printf("<td>[目录] <td><a href=bbs0an?path=%s%s>%s</a>", path, file[i], nohtml(name[i]));
		} else {
			printf("<td>[文件] <td><a href=bbsanc?path=%s%s>%s</a>", path, file[i], nohtml(name[i]));
		}
		if(id[0]) {
