...
int main()
{
	...
	if(fp)
	{
      	...
      	for(i=0; i<my_t_lines; i++)
		{
			...
			/* 去掉统计字节数的功能, 浪费系统资源 */
			printf("<td><a href=\"bbscon?board=%s&file=%s&num=%d\">%s%s </a></td>",
	...
	if(start>0)
		printf("<a href=\"bbsdoc?board=%s&start=%d\">上一页</a> ",
				brdencode, start-my_t_lines);
	if(start<total-20)
		printf("<a href=\"bbsdoc?board=%s&start=%d\">下一页</a> ",
				brdencode, start+my_t_lines);
	/*
