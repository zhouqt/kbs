...
int inform(bcache_t * bp, char *user, char *exp, int dt)
{
    ...
    sprintf(buf, "%s 取消 %s 在 %s 的 POST 权力", usr->userid, user, board);
    report(buf);
...
int main()
{
    ...
    sprintf(buf, "%-12.12s %2d月%2d日解", getcurruserid(), tmtime->tm_mon + 1, tmtime->tm_mday);
    ...
    printf("[<a href=\"bbsdenyall?board=%s\">返回被封帐号名单</a>]", http_encode_string(board, sizeof(board)));
    http_quit();
}
