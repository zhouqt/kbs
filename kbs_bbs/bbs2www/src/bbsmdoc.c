...
int main()
{
	...
	printf("<td><a href=\"bbscon?board=%s&file=%s&num=%d\">%s%36.36s </a></td></tr>", brdencode, x.filename, start + i, strncmp(x.title, "Re: ", 4) ? "○ " : "", void1(nohtml(x.title)));
    }
    printf("</table>\n");
    ...
    printf("</form>\n");
    if (start > 0) {
	printf("<a href=bbsmdoc?board=%s&start=%d>上一页</a> ", brdencode, start < 20 ? 0 : start - 20);
    }
    if (start < total - 20) {
