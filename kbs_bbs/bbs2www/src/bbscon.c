...
int main()
{
	...
	{
        printf("[<a href=\"bbspst?board=%s&file=%s&userid=%s&title=Re: %s&refilename=%s\">回文章</a>]", 
			   buf2, records[1].filename, records[1].owner, 
			   encode_url(buf, void1(ptr), sizeof(buf)), records[1].filename);
	}
