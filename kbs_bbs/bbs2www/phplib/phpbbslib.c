...
static PHP_FUNCTION(bbs_changemaillist)
{
    ...
    if (bAdd)                   //add
    {
        if (maillist.mail_list_t == MAILBOARDNUM)
            RETURN_LONG(MAILBOARDNUM);  //◊Ó¥Û÷µ¡À
        i = 0;
...
static PHP_FUNCTION(bbs_sendwebmsg)
{
    ...
    if (!msg_can_sendmsg(destid, destutmp)) {
        ZVAL_STRING(z_errmsg, "Œﬁ∑®∑¢ÀÕ—∂œ¢", 1);
        RETURN_FALSE;
    }
    if (!strcasecmp(destid, currentuser->userid)) {
        ZVAL_STRING(z_errmsg, "ƒ„≤ªƒ‹∏¯◊‘º∫∑¢—∂œ¢", 1);
        RETURN_FALSE;
    }
    if ((result = send_msg(currentuser->userid, get_utmpent_num(u_info), destid, destutmp, msg)) == 1) {
        ZVAL_STRING(z_errmsg, "“—æ≠∞Ôƒ„ÀÕ≥ˆ—∂œ¢", 1);
        RETURN_TRUE;
    } else if (result == -1) {
        char buf[STRLEN];

        snprintf(buf, sizeof(buf), "∑¢ÀÕ—∂œ¢ ß∞‹£¨%s", msgerr);
        ZVAL_STRING(z_errmsg, buf, 1);
        RETURN_FALSE;
    } else {
        ZVAL_STRING(z_errmsg, "∑¢ÀÕ—∂œ¢ ß∞‹£¨¥À»Àƒø«∞≤ª‘⁄œﬂªÚ’ﬂŒﬁ∑®Ω” ’—∂œ¢", 1);
        RETURN_FALSE;
    }
}
...
static PHP_FUNCTION(bbs_createregform)
{
	...
	if(!bAuto)
	{
        //ºÏ≤È”√ªß «∑Ò“—æ≠Õ®π˝◊¢≤·ªÚ’ﬂªπ≤ªµΩ ±º‰(œ»∑≈µΩ’‚¿Ô,◊Ó∫√∑≈µΩphp¿Ô√Ê)
...
/**
 *  Function: ∏˘æ›◊¢≤·–’√˚∫Õemail…˙≥…–¬µƒ√‹¬Î.»Áπ˚”√ªß√˚Œ™ø’,‘Ú…˙≥…“ª∏ˆ√‹¬Î.
...
static int bbs_bm_change(char *board, struct boardheader *newbh, struct boardheader *oldbh)
{
	...
	for( i=0; i<obmnum; i++ ){
		...
		sprintf(buf,"√‚»• %s µƒ∞ﬂ÷Ò %s", board, lookupuser->userid);
	...
	for( i=0; i<nbmnum; i++ ){
		...
		newlevel |= PERM_BOARDS;
		mail_file(currentuser->userid, "etc/forbm", lookupuser->userid, "–¬»Œ∞ﬂ÷Ò±ÿ∂¡", BBSPOST_LINK, NULL);

		sprintf(buf,"»Œ√¸ %s µƒ∞ﬂ÷Ò %s", board, lookupuser->userid);
		//securityreport(buf, lookupuser, NULL);
		lookupuser->userlevel = newlevel;
	}
}
...
static PHP_FUNCTION(bbs_new_board)
{
	...
	if( oldbnum ){	//∏¸∏ƒ∞Ê√Ê Ù–‘
		...
		sprintf(vbuf, "∏¸∏ƒÃ÷¬€«¯ %s µƒ◊ ¡œ --> %s", oldboard.filename, newboard.filename);
		bbslog("user","%s",vbuf);
...
static PHP_FUNCTION(bbs_start_vote)
{
	...
	if(type == 1){
		ball.maxtkt = 1;
		ball.totalitems = 3;
		strcpy(ball.items[0], "‘ﬁ≥…  £® «µƒ£©");
		strcpy(ball.items[1], "≤ª‘ﬁ≥…£®≤ª «£©");
		strcpy(ball.items[2], "√ª“‚º˚£®≤ª«Â≥˛£©");
	...
	if((fp=fopen(buf,"w"))==NULL){
		sprintf(buff,"[Õ®÷™] %s æŸ∞ÏÕ∂∆±: %s",board,ball.title);
		fprintf(fp,"%s",buff);
		fclose(fp);
#ifdef NINE_BUILD
...
static PHP_FUNCTION(bbs_sysconf_str)
{
        ...
        char_result=sysconf_str(char_conf);//ªÒ»°≈‰÷∆≤Œ ˝
        RETURN_STRING(char_result,1);
}

/****
 * add by stiger, template, √˛∞Â
 */
