...
char *filename, *userid;
{
    ...
    if (fd < 0) {
        innbbsdlog("<bbslink> Err: can't open %s\n", filename);
        return 0;
    }
    ...
    for (buffer = FD_BUF, artend = FD_BUF + st.st_size, artback = strchr(buffer, '\n'); buffer && buffer < artend && *buffer; artback = strchr(buffer, '\n')
        ) {
        /*
...
void save_outgoing(soverview_t *sover,const char *filename,const char *userid,const char *poster,time_t mtime){
    ...
    if (nf == NULL) {
        innbbsdlog("<bbslink> save_outgoing: No such board %s\n", board);
        return;
    } else {
        group = nf->newsgroups;
        server = nf->path;
        ...
        *ptr2 = savech;
        ...
        if (nl->host && *nl->host) {
            if (nl->feedfp == NULL) {
                ...
                if (nl->feedfp == NULL) {
                    innbbsdlog("<save outgoing> append failed for %s/%s.link", INNDHOME, nl->node);
                }
            }
            if (nl->feedfp != NULL) {
        ...
        *NNTPHOST_BUF = '\0';
        ...
        if (bufferp)
            for (buffer = bufferp + 1, bufferp = strchr(buffer, '\n'); buffer && *buffer; bufferp = strchr(buffer, '\n')) {
                 ...
                 */
                if (NEWSFEED == REMOTE && strncmp(NEWSCONTROL, "cancel", 5) == 0 && strncmp(buffer, "------------------", 18) == 0) {
                    break;
                }
                /*
                 * $BODY[ @BODY ] = "$_\r\n"; 
...
char *textline;
{
    ...
    if (USEIHAVE) {
        if (status == 235) {
            if (NEWSFEED == LOCAL) {
                innbbsdlog("Sendout <%s> from %s/%s\n", msgid, board, filename);
            }
        } else if (status == 437 || status == 435) {
            innbbsdlog("<bbslink> :Warn: %d %s <%s>\n", status, (char *) tcpmessage(), msgid);
            if (Verbose)
                printf(":Warn: %d %s <%s>\n", status, (char *) tcpmessage(), msgid);
            return 0;
        } else {
            innbbsdlog("<bbslink> :Err: %d %s of <%s>\n", status, (char *) tcpmessage(), msgid);
            if (Verbose)
                printf(":Err: %d %s of <%s>\n", status, (char *) tcpmessage(), msgid);
        ...
        if (status == 250) {
            innbbsdlog("<bbslink> DATA Sendout <%s> from %s/%s\n", msgid, board, filename);
            if (Verbose)
    ...
    if (!USEDATA) {
        ...
        if (!NoAction) {
            ...
            if ((NNTP = inetclient(hostname, hostport, "tcp")) < 0) {
                innbbsdlog("<bbslink> :Err: server %s %s error: cant connect\n", hostname, hostport);
                if (Verbose)
            ...
            if (atoi(NNTPbuffer) != 200) {
                innbbsdlog("<bbslink> :Err: server error: %s", NNTPbuffer);
                if (Verbose)
        ...
        if (!NoAction) {
            ...
            if ((NNTP = inetclient("localhost", hostport, "tcp")) < 0) {
                innbbsdlog("<bbslink> :Err: server %s port %s error: cant connect\n", hostname, hostport);
                if (Verbose)
    ...
    if (BODY != NULL && !NoAction) {
        if (USEIHAVE) {
            ...
            if (status == 335) {
                ...
                innbbsdlog("<bbslink> :Warn: %d %s, IHAVE <%s>\n", status, (char *) tcpmessage(), msgid);
                if (Verbose)
                    printf(":Warn: %d %s, IHAVE <%s>\n", status, (char *) tcpmessage(), msgid);
                returnstatus = 0;
            } else {
                innbbsdlog("<bbslink> :Err: %d %s, IHAVE <%s>\n", status, (char *) tcpmessage(), msgid);
                if (Verbose)
            ...
            if (status == 340) {
                ...
                innbbsdlog("<bbslink> :Warn: %d %s, POST <%s>\n", status, (char *) tcpmessage(), msgid);
                if (Verbose)
                    printf(":Warn: %d %s, POST <%s>\n", status, (char *) tcpmessage(), msgid);
                returnstatus = 0;
            } else {
                innbbsdlog("<bbslink> :Err: %d %s, POST <%s>\n", status, (char *) tcpmessage(), msgid);
                if (Verbose)
            ...
            if (status == 354) {
                ...
                innbbsdlog("<bbslink> :Err: %d %s, DATA <%s>\n", status, (char *) tcpmessage(), msgid);
                if (Verbose)
...
char *board, *filename, *from, *subject;
{
    ...
    innbbsdlog("<cancel_outgoing> Try to move moderated post from %s to deleted\n", board);
    if (Verbose)
        printf("Try to move moderated post from %s to deleted\n", board);
    FN = popen(fileglue("%s/bbspost post %s/boards/deleted > %s", INNDHOME, BBSHOME, TMPFILE), "w");
    if (FN == NULL) {
        innbbsdlog("<cancel_outgoing> can't run %s/bbspost\n", INNDHOME);
        if (Verbose)
...
//    fprintf(FN, "你的文章 \"%s\" 已经送往审核中. 请等待回覆.\n", subject);
    ...
    innbbsdlog("%s/bbspost cancel %s %s %s moderate\n", INNDHOME, BBSHOME, board, filename);
    if (Verbose)
        printf("%s/bbspost cancel %s %s %s moderate\n", INNDHOME, BBSHOME, board, filename);
    system(fileglue("%s/bbspost cancel %s %s %s moderate", INNDHOME, BBSHOME, board, filename));
    return 1;
}
    ...
    while (fgets(textline, sizeof textline, POSTS) != NULL) {
      ...
      try_read_outgoing:
        ...
        if (read_outgoing(&soverview) == 0) {
            int sendresult = send_outgoing(node, site, hostname, &soverview,
                                           baktextline);
...
void close_link()
{
    ...
    if (status != 205 && status != 221) {
        innbbsdlog("<bbslink> :Err: Cannot quit message '%d %s'\n", status, (char *) tcpmessage());
        if (Verbose)
...
void send_article()
{
    ...
    for (nlcount = 0; nlcount < NLCOUNT; nlcount++) {
        ...
        if (op && (strncasecmp(op, "ihave", 5) == 0 || strncasecmp(op, "post", 4) == 0 || strncasecmp(op, "data", 4) == 0)) {
            char *left, *right;
...
//    strcpy(BBSHOME, argv[0]);
    ...
    for (nlcount = 0; nlcount < NLCOUNT; nlcount++) {
        ...
        if (localsendout || remotesendout || localfailed || remotefailed)
            innbbsdlog("<bbslink> [%s]%s lsend:%d rsend:%d lfail:%d rfail:%d\n", NODELIST[nlcount].node, NoAction ? "NoAction" : "", localsendout, remotesendout, localfailed, remotefailed);
        if (NODELIST[nlcount].feedfp != NULL) {
            fclose(NODELIST[nlcount].feedfp);
            NODELIST[nlcount].feedfp = NULL;
        }
