...
const struct royalset royallist[] = {
	...
	{"A", "ĞÇ¿ÕÆï±øÁ¬", 1, 10, 15, 100, "ÄãÕæºÃ£¬À´ÅãÎÒÁÄÌì..",
	 "ÊØÎÀĞÇ¿ÕµÄ°²È«ÊÇºÜĞÁ¿àµÄ.."},
	{"B", "ĞÇ¿Õ£°£°£·", 1, 100, 25, 200, "ÕæÊÇÀñÃ²µÄĞ¡¼¦..ÎÒÏ²»¶...",
	 "ÌØÎñ¾ÍÊÇÃØÃÜ±£»¤Õ¾³¤°²È«µÄÈË.."},
	{"C", "Õò¹ú´ó½«¾ü", 1, 200, 30, 250, "¸æËßÄãà¡£¡µ±ÄêÄÇ¸öÕ½ÒÛºÜ¾«²Êà¸..",
	 "ÄãÕæÊÇ¸ß¹óÓÅÑÅµÄĞ¡¼¦..."},
	{"D", "²ÎÄ±×ÜÎñ³¤", 1, 300, 35, 300, "ÎÒ°ïÕ¾³¤¹ÜÀíÕâ¸ö¹ú¼Òà¡..",
	 "ÄãµÄÉùÒôºÜºÃÌıÒ®..ÎÒºÜÏ²»¶à¸...:)"},
	{"E", "Ğ¡ÌìÊ¹Õ¾³¤", 1, 400, 35, 300, "ÄãºÜÓĞ½ÌÑøà¡£¡ºÜ¸ßĞËÈÏÊ¶Äã...",
	 "ÓÅÑÅµÄÄã£¬ÇëÈÃÎÒ°ïÄãÆí¸£...."},
	{"F", "·çóİÊÖÕ¾³¤", 1, 500, 40, 350, "ÄãºÃ¿É°®à¸..ÎÒÏ²»¶Äãà¡....",
	 "¶ÔÀ²..ÒÔºóÒª¶à¶àÀ´ºÍÎÒÍæà¸..."},
	{"G", "¹ÔĞ¡º¢Õ¾³¤", 1, 550, 40, 350,
	 "¸úÄã½²»°ºÜ¿ìÀÖà¸..²»ÏñÕ¾³¤Ò»ÑùÎŞÁÄ..",
	 "À´£¬×øÎÒÏ¥¸ÇÉÏ£¬ÌıÎÒ½²¹ÊÊÂ.."},
	{"H", "Ğ¡Ã×¿ËÕ¾³¤", 1, 600, 50, 400, "Ò»Õ¾Ö®³¤ÔğÈÎÖØ´óÑ½..:)..",
	 "Ğ»Ğ»ÄãÌıÎÒ½²»°..ÒÔºóÒª¶àÀ´à¸..."},
	{"I", "ĞÇ¿Õ¹àË®Èº", 2, 60, 0, 0, "²»´íà¡..Âù»úÁéµÄà¸..ºÜ¿É°®....",
	 "À´  ÎÒÃÇÒ»ÆğÀ´¹àË®°É...."},
	{"J", "ÇàÄêË§Îä¹Ù", 0, 0, 0, 0,
	 "ÄãºÃ£¬ÎÒÊÇÎä¹Ù£¬¸Õ´ÓÒøºÓ±ß¾³»ØÀ´ĞİÏ¢..",
	 "Ï£ÍûÏÂ´Î»¹ÄÜ¼ûµ½Äã...:)"},
...
pip_change_weight()
{
...
#else
	getdata(b_lines - 1, 1, "ÄãµÄÑ¡ÔñÊÇ? [Q]Àë¿ª:", genbuf, 2, DOECHO, true);
#endif				// END MAPLE
	if (genbuf[0] == '1' || genbuf[0] == '2' || genbuf[0] == '3'
		...
		case '1':
			if (d.money < 80) {
				pressanykey
				    ("´«Í³ÔöÅÖÒª80Ôªà¸....Äã²»¹»Ç®À²...");
			} else {
#ifdef MAPLE
				getdata(b_lines - 1, 1,
					"Ğè»¨·Ñ80Ôª(3¡«5¹«½ï)£¬ÄãÈ·¶¨Âğ? [y/N]",
					genbuf, 2, 1, 0);
#else
				getdata(b_lines - 1, 1,
					"Ğè»¨·Ñ80Ôª(3¡«5¹«½ï)£¬ÄãÈ·¶¨Âğ? [y/N]",
		...
		case '2':
#ifdef MAPLE
			getdata(b_lines - 1, 1,
				"ÔöÒ»¹«½ïÒª30Ôª£¬ÄãÒªÔö¶àÉÙ¹«½ïÄØ? [ÇëÌîÊı×Ö]:",
				genbuf, 4, 1, 0);
#else
			getdata(b_lines - 1, 1,
				"ÔöÒ»¹«½ïÒª30Ôª£¬ÄãÒªÔö¶àÉÙ¹«½ïÄØ? [ÇëÌîÊı×Ö]:",
			...
			if (weightmp <= 0) {
				...
				sprintf(inbuf,
					"Ôö¼Ó%d¹«½ï£¬×Ü¹²Ğè»¨·ÑÁË%dÔª£¬È·¶¨Âğ? [y/N]",
...
#ifdef MAPLE
			getdata(b_lines - 1, 1,
				"¼õÒ»¹«½ïÒª30Ôª£¬ÄãÒª¼õ¶àÉÙ¹«½ïÄØ? [ÇëÌîÊı×Ö]:",
				genbuf, 4, 1, 0);
#else
			getdata(b_lines - 1, 1,
				"¼õÒ»¹«½ïÒª30Ôª£¬ÄãÒª¼õ¶àÉÙ¹«½ïÄØ? [ÇëÌîÊı×Ö]:",
...
struct royalset *p;
{
	...
	do {
		...
		move(13, 4);
		sprintf(buf,
			"[1;31m©°¡ª¡ª¡ª¡ª¡ª¡ª©È[37;41m À´µ½×ÜË¾Áî²¿ÁË  ÇëÑ¡ÔñÄãÓû°İ·ÃµÄ¶ÔÏó [0;1;31m©À¡ª¡ª¡ª¡ª¡ª¡ª©´[0m");
		prints(buf);
		move(14, 4);
		sprintf(buf,
			"[1;31m©¦                                                                  ©¦[0m");
		...
		for (n = 0; n < 5; n++) {
			...
			if ((d.seeroyalJ == 1 && n == 4) || (n != 4))
				sprintf(buf,
					"[1;31m©¦ [36m([37m%s[36m) [33m%-10s  [37m%-14s     [36m([37m%s[36m) [33m%-10s  [37m%-14s[31m©¦[0m",
					p[a].num, p[a].name, inbuf1, p[b].num,
					p[b].name, inbuf2);
			else
				sprintf(buf,
					"[1;31m©¦ [36m([37m%s[36m) [33m%-10s  [37m%-14s                                   [31m©¦[0m",
		...
		move(21, 4);
		sprintf(buf,
			"[1;31m©¸¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©¼[0m");
		prints(buf);
		...
		clrtoeol();
		move(b_lines - 1, 0);
		prints("[1;33m [ÉúÃüÁ¦] %d/%d  [Æ£ÀÍ¶È] %d [0m", d.hp,
		       d.maxhp, d.tired);
			...
			if (d.death == 1) {
				...
				if ((p[choice].needmode == 0) ||
					...
					if (choice >= 1 && choice <= 9
						...
						if (rand() % 2 > 0)
							sprintf(buf,
								"ÄÜºÍÕâ÷áÎ°´óµÄÄã½²»°ÕæÊÇÈÙĞÒ¨Ú...");
						else
							sprintf(buf,
								"ºÜ¸ßĞËÄãÀ´°İ·ÃÎÒ£¬µ«ÎÒ²»ÄÜ¸øÄãÊ²÷áÁË..");
					} else {
						change = 0;
						if (choice >= 1 && choice <= 8) {
							...
							if (change >
								...
								change =
								    ...
								    addtoman;
							/*
							 * Èç¹û¼ÓÉÏÔ­ÏÈµÄÖ®ºó´óì¶ËùÄÜ¸øµÄËùÓĞÖµÊ±
							 */
							if ((change +
							     save[choice]) >=
							    p[choice].maxtoman)
...
char *genbuf;
{
	 ...
	 */
	const static char yo[12][5] = { "µ®Éú", "Ó¤¶ù", "Ó×¶ù", "¶ùÍ¯", "ÉÙÄê", "ÇàÄê",
...
*/
		...
		if (d1.death == 0) {
			prints
			    ("[1;32mName£º%-10s[m  ÉúÈÕ£º%02dÄê%02dÔÂ%2dÈÕ   ÄêÁä£º%2dËê  ×´Ì¬£º%s  Ç®Ç®£º%d\n"
			...
			if (d1.shit >= 80 && d1.shit < 100)
				prints("[1;34m¿ì³ôËÀÁË..[m");
			if (d1.shit >= 100) {
				prints("[1;31m³ôËÀÁË..[m");
				return -1;
			}
			...
			if (pc1 < 20)
				prints("[1;35mÈ«ÉíÎŞÁ¦ÖĞ.¿ì¶öËÀÁË.[m");
			if (pc1 < 40 && pc1 >= 20)
				prints("ÌåÁ¦²»Ì«¹»..Ïë³Ôµã¶«Î÷..");
			if (pc1 < 100 && pc1 >= 80)
				prints("àÅ¡«¶Ç×Ó±¥±¥ÓĞÌåÁ¦..");
			if (pc1 >= 100)
				prints("[1;34m¿ì³ÅËÀÁË..[m");

			pc1 = d1.tired;
			if (pc1 < 20)
				prints("¾«Éñ¶¶¶¶ÖĞ..");
			if (pc1 < 80 && pc1 >= 60)
				prints("[1;34mÓĞµãĞ¡ÀÛ..[m");
			if (pc1 < 100 && pc1 >= 80) {
				prints("[1;31mºÃÀÛà¸£¬¿ì²»ĞĞÁË..[m");
			}
			if (pc1 >= 100) {
				prints("ÀÛËÀÁË...");
				return -1;
