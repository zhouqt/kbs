...
pip_new_game()
{
...
#ifdef MAPLE
			getdata(10, 6, "ÄãÒªÎÒÃÇÈÃËûÖØÉúÂğ? [y/N]:", buf, 2, 1,
				0);
#else
			getdata(10, 6, "ÄãÒªÎÒÃÇÈÃËûÖØÉúÂğ? [y/N]:", buf, 2,
				DOECHO, true);
#endif				// END MAPLE
			if (buf[0] == 'y' || buf[0] == 'Y') {
				pip_live_again();
	...
	if (d.death != 0 || !d.name[0]) {
		...
		strcpy(buf, "¶¾ÖĞÖ®¶¾");
#ifdef MAPLE
		getdata(2, 3, "°ïĞ¡¼¦È¡¸öºÃÌıµÄÃû×Ö°É(Çë²»ÒªÓĞ¿Õ¸ñ):", buf, 11,
			1, 0);
#else
		getdata(2, 3, "°ïĞ¡¼¦È¡¸öºÃÌıµÄÃû×Ö°É(Çë²»ÒªÓĞ¿Õ¸ñ):", buf, 11,
			DOECHO, false);
		 ...
		 */
#ifdef MAPLE
		getdata(4, 3, "[Boy]Ğ¡¹«¼¦¡á or [Girl]Ğ¡Ä¸¼¦¡â [b/G]", buf, 2,
			1, 0);
#else
		getdata(4, 3, "[Boy]Ğ¡¹«¼¦¡á or [Girl]Ğ¡Ä¸¼¦¡â [b/G]", buf, 2,
			DOECHO, true);
		...
		move(7, 3);
		prints("Ñ¡ÓĞ½á¾Ö»áÔÚĞ¡¼¦20ËêÊ±½áÊøÓÎÏ·£¬²¢¸æÖªĞ¡¼¦ºóĞøµÄ·¢Õ¹");
		move(8, 3);
		 ...
		 */
#ifdef MAPLE
		getdata(9, 3, "ÄãÏ£ÍûĞ¡¼¦ÓÎÏ·ÊÇ·ñÒªÓĞ20Ëê½á¾Ö? [Y/n]", buf, 2,
			1, 0);
#else
		getdata(9, 3, "ÄãÏ£ÍûĞ¡¼¦ÓÎÏ·ÊÇ·ñÒªÓĞ20Ëê½á¾Ö? [Y/n]", buf, 2,
			DOECHO, true);
