...
int mode;
{
	...
	if (d.sex == 1)
		sprintf(buf,
			"[1;41m  ÐÇ¿ÕÕ½¶·¼¦ ¡« [32m¡á [37m%-15s                                             [0m",
			d.name);
	else if (d.sex == 2)
		sprintf(buf,
			"[1;41m  ÐÇ¿ÕÕ½¶·¼¦ ¡« [33m¡â [37m%-15s                                             [0m",
			d.name);
	else
		sprintf(buf,
			"[1;41m  ÐÇ¿ÕÕ½¶·¼¦ ¡« [34m£¿ [37m%-15s                                             [0m",
			d.name);
	prints(buf);
	...
	if (mode == 0) {	/*Ö÷Òª»­Ãæ */
		...
		if (anynum == 0)
			sprintf(buf,
				" [1;35m[Õ¾³¤Ô»]:[31mºìÉ«[36m±íÊ¾Î£ÏÕ  [33m»ÆÉ«[36m±íÊ¾¾¯¸æ  [37m°×É«[36m±íÊ¾°²È«[0m");
		else if (anynum == 1)
			sprintf(buf,
				" [1;35m[Õ¾³¤Ô»]:[37mÒª¶à¶à×¢ÒâÐ¡¼¦µÄÆ£ÀÍ¶ÈºÍ²¡Æø  ÒÔÃâÀÛËÀ²¡ËÀ[0m");
		else if (anynum == 2)
			sprintf(buf,
				" [1;35m[Õ¾³¤Ô»]:[37mËæÊ±×¢ÒâÐ¡¼¦µÄÉúÃüÊýÖµà¡![0m");
		else if (anynum == 3)
			sprintf(buf,
				" [1;35m[Õ¾³¤Ô»]:[37m¿ì¿ìÀÖÀÖµÄÐ¡¼¦²ÅÊÇÐÒ¸£µÄÐ¡¼¦.....[0m");
		prints(buf);
	} else if (mode == 1) {	/*ÒûÊ³ */
		move(4, 0);
		if (d.food == 0)
	...
	move(5, 0);
	sprintf
	    (linebuf, "[1;%dm©°¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©´[m",
	     color);
	prints(linebuf);
	move(6, 0);
	switch (age) {
	...
	pc = d.satisfy;
	if (pc < 20)
		prints("[1;35mºÜ²»Âú×ã..[m  ");
	if (pc < 40 && pc >= 20)
		prints("[1;33m²»Ì«Âú×ã[m  ");
	if (pc < 95 && pc >= 80)
		prints("Âú×ãÀ²..  ");
	if (pc <= 100 && pc >= 95)
		prints("ºÜÂú×ã..  ");
