...
pip_ending_screen()
{
	...
	move(7, 8);
	prints
	    ("[1;31m¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª[41;37m ĞÇ¿ÕÕ½¶·¼¦½á¾Ö±¨¸æ [0;1;31m¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª[0m");
	move(9, 10);
	prints("[1;36mÕâ¸öÊ±¼ä²»Öª²»¾õµØ»¹ÊÇµ½ÁÙÁË...[0m");
	move(11, 10);
	prints
	    ("[1;37m[33m%s[37m µÃÀë¿ªÄãµÄÎÂÅ¯»³±§£¬×Ô¼ºÒ»Ö»¼¦ÔÚÍâÃæÇóÉú´æÁË.....[0m",
	     d.name);
	move(13, 10);
	prints
	    ("[1;36mÔÚÄãÕÕ¹Ë½Ìµ¼ËûµÄÕâ¶ÎÊ±¹â£¬ÈÃËû½Ó´¥ÁËºÜ¶àÁìÓò£¬ÅàÑøÁËºÜ¶àµÄÄÜÁ¦....[0m");
	move(15, 10);
	prints
	    ("[1;37mÒòÎªÕâĞ©£¬ÈÃĞ¡¼¦ [33m%s[37m Ö®ºóµÄÉú»î£¬±äµÃ¸ü¶à²É¶à×ËÁË........[0m",
	     d.name);
	move(17, 10);
	...
	move(7, 8);
	prints
	    ("[1;34m¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª[44;37m ĞÇ¿ÕÕ½¶·¼¦Î´À´·¢Õ¹ [0;1;34m¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª[0m");
	move(9, 10);
	prints
	    ("[1;36mÍ¸¹ıË®¾§Çò£¬ÈÃÎÒÃÇÒ»ÆğÀ´¿´ [33m%s[36m µÄÎ´À´·¢Õ¹°É.....[0m",
	     d.name);
	move(11, 10);
	prints("[1;37mĞ¡¼¦ [33m%s[37m ºóÀ´%s....[0m", d.name, endbuf1);
	move(13, 10);
	prints("[1;36mÒòÎªËûµÄÖ®Ç°µÄÅ¬Á¦£¬Ê¹µÃËûÔÚÕâÒ»·½Ãæ%s....[0m",
	       endbuf2);
	move(15, 10);
	prints("[1;37mÖÁì¶Ğ¡¼¦µÄ»éÒö×´¿ö£¬ËûºóÀ´%s£¬»éÒöËãÊÇºÜÃÀÂú.....[0m",
	       endbuf3);
	move(17, 10);
	...
	now = time(0);
	sprintf(buf,
		"[1;35m¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª[0m\n");
	pip_log_record(buf);
	sprintf(buf,
...
{
	const static char *name[8][2] = { {"ÄĞµÄ", "Å®µÄ"},
	{"¼Ş¸øÍõ×Ó", "È¢ÁË¹«Ö÷"},
	{"¼Ş¸øÄã", "È¢ÁËÄã"},
...
int endgrade;
{
	...
	move(12, 17);
	prints("[1;36mÄúµÄĞ¡¼¦ [37m%s [36m×ÜµÃ·Ö£½ [1;5;33m%d [0m", d.name,
	       gradeall);
	return gradeall;
}
...
#else
		getdata(12, 14, buf, ans, 2, DOECHO, true);
#endif				// END MAPLE
		if (ans[0] != 'N' && ans[0] != 'n') {
			...
			if (randvalue == 0)
				sprintf(buf,
					"[1;37m  ÄãµÄĞ¡¼¦%sÒÔºó¿ÉÄÜµÄÉí·İÊÇ%s  [0m",
					d.name,
