...
pip_ending_screen()
{
	...
	move(15, 10);
	prints
	    ("[1;37mÒòÎªÕâĞ©£¬ÈÃĞ¡¼¦ [33m%s[37m Ö®ááµÄÉú»î£¬±äµÃ¸ü¶à²É¶à×ËÁË........[0m",
	     d.name);
	move(17, 10);
	...
	prints("[1;36mÒòÎªËûµÄÖ®Ç°µÄÅ¬Á¦£¬Ê¹µÃËûÔÚÕâÒ»·½Ãæ%s....[0m",
	       endbuf2);
	move(15, 10);
	prints("[1;37mÖÁì¶Ğ¡¼¦µÄ»éÒö×´¿ö£¬ËûááÀ´%s£¬»éÒöËãÊÇºÜÃÀÂú.....[0m",
	       endbuf3);
	move(17, 10);
	...
	pip_log_record(buf);
	sprintf(buf,
		"[1;37mÖÁì¶»éÒö×´¿ö£¬ËûááÀ´%s£¬»éÒöËãÊÇºÜÃÀÂú.....[0m\n\n[1;37mĞ¡¼¦ [32n%s[37m µÄ×Ü»ı·Ö£½ [33m%d[0m\n",
...
{				/*Õ¼²·Ê¦À´·Ã */
	...
	if (d.money >= money) {
		...
		if (ans[0] != 'N' && ans[0] != 'n') {
			...
			if (randvalue == 0)
				sprintf(buf,
					"[1;37m  ÄãµÄĞ¡¼¦%sÒÔáá¿ÉÄÜµÄÉí·İÊÇ%s  [0m",
					d.name,
