...
const struct goodsofpip pipfoodlist[] = {
	...
	1, "ºÃ³ÔµÄÊ³Îï", "ÌåÁ¦»Ö¸´50", "Ã¿³ÔÒ»´ÎÊ³Îï»á»Ö¸´ÌåÁ¦50à¸!", 50, 50, 1,
	1,
	2, "ÃÀÎ¶µÄÁãÊ³", "ÌåÁ¦»Ö¸´100", "³ıÁË»Ö¸´ÌåÁ¦£¬Ğ¡¼¦Ò²»á¸ü¿ìÀÖ", 120,
	    100,
	2, 3,
...
const struct goodsofpip pipmedicinelist[] = {
	0, "ÎïÆ·Ãû", "ËµÃ÷buy", "ËµÃ÷feed", 0, 0, 0, 0,
	1, "ºÃÓÃ´ó²¹Íè", "ÌåÁ¦»Ö¸´600", "»Ö¸´´óÁ¿Á÷Ê§ÌåÁ¦µÄÁ¼·½", 500, 600, 4,
	    4,
	2, "Õä¹óµÄÁéÖ¥", "·¨Á¦»Ö¸´50", "Ã¿³ÔÒ»´ÎÁéÖ¥»á»Ö¸´·¨Á¦50à¸!", 100, 50,
	    7,
	7,
	3, "Ç§ÄêÈË²ÎÍõ", "·¨Á¦»Ö¸´500", "»Ö¸´´óÁ¿Á÷Ê§·¨Á¦µÄÁ¼·½", 800, 500, 7,
	    7,
...
const struct goodsofpip pipotherlist[] = {
	...
	2, "°Ù¿ÆÈ«Êé", "ÖªÊ¶µÄÀ´Ô´", "Êé±¾ÈÃĞ¡¼¦¸ü´ÏÃ÷¸üÓĞÆøÖÊÀ²...", 100, 0, 6,
	6,
	0, NULL, NULL, NULL, 0, 0, 0, 0
};
...
struct goodsofpip *p;
{
	const static char *shopname[4] = { "µêÃû", "±ãÀûÉÌµê", "ĞÇ¿ÕÒ©ÆÌ", "Ò¹ÀïÊé¾Ö" };
	char inbuf[256];
	char genbuf[20];
	long smoney;
	int oldmoney;
	...
	do {
		...
		move(6, 0);
		sprintf(inbuf,
			"[1;31m  ¡ª[41;37m ±àºÅ [0;1;31m¡ª[41;37m ÉÌ      Æ· [0;1;31m¡ª¡ª[41;37m Ğ§            ÄÜ [0;1;31m¡ª¡ª[41;37m ¼Û     ¸ñ [0;1;31m¡ª[37;41m ÓµÓĞÊıÁ¿ [0;1;31m¡ª[0m  ");
		prints(inbuf);
		...
		case 'b':
			move(b_lines - 1, 1);
			sprintf(inbuf,
				"ÏëÒªÂòÈëÉ¶ÄØ? [0]·ÅÆúÂòÈë [1¡«%d]ÎïÆ·ÉÌºÅ",
				oldnum[0]);
#ifdef MAPLE
			...
			if (choice >= 1 && choice <= oldnum[0]) {
				...
				if (smoney < 0) {
					...
					sprintf(inbuf,
						"È·¶¨ÂòÈëÎïÆ· [%s] ÊıÁ¿ %d ¸öÂğ?(µê¼ÒÂô¼Û %d) [y/N]:",
		...
		case 's':
			...
			move(b_lines - 1, 1);
			sprintf(inbuf,
				"ÏëÒªÂô³öÉ¶ÄØ? [0]·ÅÆúÂô³ö [1¡«%d]ÎïÆ·ÉÌºÅ",
				oldnum[0]);
#ifdef MAPLE
			getdata(b_lines - 1, 1, inbuf, genbuf, 3, LCECHO, "0");
#else
			...
			if (choice >= 1 && choice <= oldnum[0]) {
				...
				if (smoney < 0) {
					...
					sprintf(inbuf,
						"ÄãµÄ [%s] Ã»ÓĞÄÇ÷á¶à¸öà¸",
						p[choice].name);
					pressanykey(inbuf);
				} else {
					sprintf(inbuf,
						"È·¶¨Âô³öÎïÆ· [%s] ÊıÁ¿ %d ¸öÂğ?(µê¼ÒÂò¼Û %d) [y/N]:",
						p[choice].name, smoney,
		...
		case 'Q':
		case 'q':
			sprintf(inbuf, "½ğÇ®½»Ò×¹² %d Ôª,Àë¿ª %s ",
...
/*   move(10,2); 
   ...
   prints(buf);*/
		...
		move(12, 2);
		sprintf(buf,
			" [1;31m¡ª¡ª[37m°×É« ¿ÉÒÔ¹ºÂò[31m¡ª¡ª[32mÂÌÉ« ÓµÓĞ×°±¸[31m¡ª¡ª[33m»ÆÉ« Ç®Ç®²»¹»[31m¡ª¡ª[35m×ÏÉ« ÄÜÁ¦²»×ã[31m¡ª¡ª[m");
		prints(buf);
		...
		case 'b':
			move(b_lines - 1, 1);
			sprintf(shortbuf, "ÏëÒª¹ºÂòÉ¶ÄØ? ÄãµÄÇ®Ç®[%d]Ôª:[Êı×Ö]",
				d.money);
			prints(shortbuf);
#ifdef MAPLE
			...
			if (choice >= 0 && choice <= n) {
				...
				if (choice == 0) {	/*½â³ı */
					...
					sprintf(shortbuf, "ÄãÔçÒÑ¾­ÓĞ %s ÂŞ",
						p[variance].name);
					pressanykey(shortbuf);
				}
				...
				else if (d.maxhp < p[choice].needmaxhp || d.maxmp < p[choice].needmaxmp || d.speed < p[choice].needspeed) {	/*ÄÜÁ¦²»×ã */
					sprintf(shortbuf,
						"ĞèÒªHP %d MP %d SPEED %d à¸",
						p[choice].needmaxhp,
						p[choice].needmaxmp,
						p[choice].needspeed);
					pressanykey(shortbuf);
