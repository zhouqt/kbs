...
const struct playrule badmanlist[] = {
	...
	"Ä¢¹½Ð¡¹Ö", 80, 0, 40, 0, 50, 35, 60, 250, "11110", 0, 0,
	"¶¾Ð«", 85, 0, 30, 0, 80, 90, 80, 500, "10111", 0, 0,
	...
	"³¬¼¶Åç»ðÁú", 500, 0, 100, 0, 250, 250, 150, 1500, "11000", 0, 0,
	...
	"U2ÍâÐÇÈË", 800, 0, 600, 0, 800, 800, 600, 2000, "11010", 0, 0,
	"ÖÐ¹ú½®  ", 800, 0, 600, 0, 1000, 1000, 500, 2000, "10100", 0, 0,
	...
	"ÈÛ»¯Ñý", 1750, 0, 1300, 0, 1800, 2000, 1000, 4000, "01011", 0, 0,
	"Ê¹Í½", 2500, 0, 2500, 0, 2500, 2500, 2500, 5000, "10001", 0, 0,
	"°£¼°Ä¾ÄËÒÁ", 3500, 0, 3000, 0, 3500, 3500, 2000, 5000, "10110", 0, 0,
	"¹ÅÐ¡ÍÃ", 5000, 0, 4500, 0, 5000, 6000, 4000, 5000, "11100", 0, 0,
	"Ê®×Ö»úÆ÷ÈË", 6000, 0, 5000, 0, 5500, 6500, 5000, 5000, "01001", 0, 0,
	"·ÉÌìÐ¡Î×Ê¦", 6500, 0, 5500, 0, 6500, 6000, 6000, 6000, "01101", 0, 0,
...
const struct magicset treatmagiclist[] = {
	...
	"ÄýÉñ¹éÔª", 20, 1, 100, 0, 40, 0,
	"ÔªÁé¹éÐÄ", 50, 1, 400, 0, 60, 0,
...
const struct magicset firemagiclist[] = {
	...
	"Á¶ÓüÕæ»ð", 40, 0, 100, 1, 5, 0,
	"»ðÁúÕÆ", 100, 0, 250, 1, 10, 0,
	"»ðÁúÕÐ»½", 200, 0, 500, 1, 10, 0,
	"»ðÉñÖ®Îè", 600, 0, 1600, 1, 10, 0,
	"NULL", 0, 0, 0, 0, 0, 0
};
...
const struct magicset windmagiclist[] = {
	"·çÏµ·¨Êõ", 0, 0, 0, 0, 0, 0,
	"·çÖä", 10, 0, 20, 1, 5, 0,
	"Ðý·çÖä", 20, 0, 50, 1, 5, 0,
	"¿ñ·çÊõ", 40, 0, 100, 1, 5, 0,
	"Áú¾í·ç", 100, 0, 250, 1, 10, 0,
	"·ç¾í²ÐÔÆ", 200, 0, 500, 1, 10, 0,
...
int mode;
{
	...
	int mankey;
	int lucky;
	int dinjure = 0;	/*Ð¡¼¦ÉËº¦Á¦ */
	int minjure = 0;	/*¶Ô·½ÉËº¦Á¦ */
	int dresistmore = 0;	/*Ð¡¼¦¼ÓÇ¿·ÀÓù */
	...
	int oldmoney;		/*Î´Õ½¶·Ç°½ðÇ® */
	int oldtired;
	int oldhp;
	int winorlose = 0;	/*1:you win 0:you loss */

	/*
	 * Ëæ»ú²úÉúÈËÎï ²¢ÇÒ´æºÃÕ½¶·Ç°µÄÒ»Ð©ÊýÖµ
	 */
	oldhexp = d.hexp;
	oldmexp = d.mexp;
	oldbrave = d.brave;
	 ...
	 */
	do {
		if (m.hp <= 0) {	/*µÐÈËËÀµôÁË */
			m.hp = 0;
			d.money += m.money;
			m.death = 1;
			d.brave += rand() % 4 + 3;
		}
		if (d.hp <= 0 || d.tired >= 100) {	/*Ð¡¼¦ÕóÍö */
			if (mode == 1) {
				d.hp = 0;
				d.tired = 0;
				d.death = 1;
		...
		if (d.sex == 1)
			sprintf(buf,
				"[1;41m  ÐÇ¿ÕÕ½¶·¼¦ ¡« [32m¡á [37m%-10s                                                  [0m",
				d.name);
		else if (d.sex == 2)
			sprintf(buf,
				"[1;41m  ÐÇ¿ÕÕ½¶·¼¦ ¡« [33m¡â [37m%-10s                                                  [0m",
				d.name);
		else
			sprintf(buf,
				"[1;41m  ÐÇ¿ÕÕ½¶·¼¦ ¡« [34m£¿ [37m%-10s                                                  [0m",
				d.name);
		prints(buf);
		move(6, 0);
		if (mode == 1)
		...
		move(1, 0);
		sprintf(buf,
			"[1;31m©°¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©´[m");
		prints(buf);
		move(2, 0);
		/*
		 * luckyÄÃÀ´µ±colorÓÃ
		...
		sprintf(buf,
			"[1;31m©¦[33mÉú  Ãü:[37m%-12s[33m·¨  Á¦:[37m%-12s[33mÆ£  ÀÍ:[%dm%-12d[33m½ð  Ç®:[37m%-10d[31m©¦[m",
			inbuf1, inbuf2, lucky, d.tired, d.money);
		prints(buf);
		move(3, 0);
		sprintf(inbuf1, "%d/%d", d.hexp, d.mexp);
		sprintf(buf,
			"[1;31m©¦[33m¹¥  »÷:[37m%-10d  [33m·À  Óù:[37m%-10d  [33mËÙ  ¶È:[37m%-5d       [33m¾­  Ñé:[37m%-10s[31m©¦[m",
			d.attack, d.resist, d.speed, inbuf1);
		prints(buf);
		move(4, 0);
		sprintf(buf,
			"[1;31m©¦[33mÊ³  Îï:[37m%-5d       [33m´ó²¹Íè:[37m%-5d       [33mÁã  Ê³:[37m%-5d       [33mÒ©  ²Ý:[37m%-5d     [31m©¦[m",
		...
		move(19, 0);
		sprintf(buf,
			"[1;34m©°¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©´[m");
		prints(buf);
		move(20, 0);
		sprintf(inbuf1, "%d/%d", m.hp, m.maxhp);
		sprintf(inbuf2, "%d/%d", m.mp, m.maxmp);
		sprintf(buf,
			"[1;34m©¦[32mÐÕ  Ãû:[37m%-10s  [32mÉú  Ãü:[37m%-11s [32m·¨  Á¦:[37m%-11s                  [34m©¦[m",
			p[n].name, inbuf1, inbuf2);
		prints(buf);
		move(21, 0);
		sprintf(buf,
			"[1;34m©¦[32m¹¥  »÷:[37m%-6d      [32m·À  Óù:[37m%-6d      [32mËÙ  ¶È:[37m%-6d      [32m½ð  Ç®:[37m%-6d    [34m©¦[m",
		...
		move(b_lines, 0);
		sprintf(buf,
			"[1;44;37m  Õ½¶·ÃüÁî  [46m  [1]ÆÕÍ¨  [2]È«Á¦  [3]Ä§·¨  [4]·ÀÓù  [5]²¹³ä  [6]ÌÓÃü         [m");
		prints(buf);
		...
		if (m.death == 0 && d.death == 0) {
			...
			case '6':
				...
				move(11, 0);
				prints
				    ("            [1;31m©¦  [37mÊµÁ¦²»Ç¿µÄÐ¡¼¦ [33m%-10s                 [31m©¦[m",
				     d.name);
				move(12, 0);
				prints
				...
				sprintf(inbuf1, "%d/%d", d.hexp - oldhexp,
					d.mexp - oldmexp);
				prints
				    ("            [1;31m©¦  [37mÆÀ¼ÛÔö¼ÓÁË [36m%-5s [37mµã  ¼¼ÊõÔö¼ÓÁË [36m%-2d/%-2d [37mµã  [31m©¦[m",
				     inbuf1, d.hskill - oldhskill,
				     d.mskill - oldmskill);
				move(14, 0);
				sprintf(inbuf1, "%d [37mÔª",
					oldmoney - d.money);
				prints
				    ("            [1;31m©¦  [37mÓÂ¸Ò½µµÍÁË [36m%-5d [37mµã  ½ðÇ®¼õÉÙÁË [36m%-13s  [31m©¦[m",
				     oldbrave - d.brave, inbuf1);
				move(15, 0);
				prints
				    ("            [1;31m©¸¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©¼[m");
				pressanykey("ÈýÊ®Áù¼Æ ×ßÎªÉÏ²ß...");
				winorlose = 0;
				break;
		...
		sprintf(inbuf1, "%d/%d", d.hexp, d.mexp);
		sprintf(buf,
			"[1;31m©¦[33m¹¥  »÷:[37m%-6d      [33m·À  Óù:[37m%-6d      [33mËÙ  ¶È:[37m%-5d       [33m¾­  Ñé:[37m%-10s[31m©¦[m",
			d.attack, d.resist, d.speed, inbuf1);
		prints(buf);
		move(4, 0);
		sprintf(buf,
		...
		move(b_lines, 0);
		sprintf(buf,
			"[1;41;37m  [37m¹¥»÷ÃüÁî  [47m  [31m[1][30mÆÕÍ¨  [31m[2][30mÈ«Á¦  [31m[3][30mÄ§·¨  [31m[4][30m·ÀÓù  [31m[5][30mÌÓÃü                     [m");
		prints(buf);
		...
		if ((m.hp > 0) && (pipkey != '6')
			...
			case 4:
				...
				d.brave += rand() % 3 + 2;
				m.death = 1;
				sprintf(buf, "¶Ô·½ÏÈÉÁÁË..µ«µôÁËÒ»Ð©Ç®¸øÄã...");
				pressanykey(buf);
				break;
			}
		}
		...
		if (m.death == 1) {
			...
			if (mode == 1) {
				...
				move(11, 0);
				prints
				    ("            [1;31m©¦  [37mÓ¢ÓÂµÄÐ¡¼¦ [33m%-10s                     [31m©¦[m",
				     d.name);
				move(12, 0);
				prints
				    ("            [1;31m©¦  [37m´ò°ÜÁËÐ°¶ñµÄ¹ÖÎï [32m%-10s               [31m©¦[m",
				     p[n].name);
			} else {
				move(10, 0);
				prints
				...
				move(11, 0);
				prints
				    ("            [1;31m©¦  [37mÎäÊõ´ó»áµÄÐ¡¼¦ [33m%-10s                 [31m©¦[m",
				     d.name);
				move(12, 0);
				prints
				    ("            [1;31m©¦  [37m´ò°ÜÁËÇ¿¾¢µÄ¶ÔÊÖ [32m%-10s               [31m©¦[m",
				     p[n].name);
			}
			move(13, 0);
			sprintf(inbuf1, "%d/%d", d.hexp - oldhexp,
				d.mexp - oldmexp);
			prints
			    ("            [1;31m©¦  [37mÆÀ¼ÛÌáÉýÁË %-5s µã  ¼¼ÊõÔö¼ÓÁË %-2d/%-2d µã  [31m©¦[m",
			     inbuf1, d.hskill - oldhskill,
			     d.mskill - oldmskill);
			move(14, 0);
			sprintf(inbuf1, "%d Ôª", d.money - oldmoney);
			prints
			    ("            [1;31m©¦  [37mÓÂ¸ÒÌáÉýÁË %-5d µã  ½ðÇ®Ôö¼ÓÁË %-9s [31m©¦[m",
			...
			if (m.hp <= 0)
				pressanykey("¶Ô·½ËÀµôÂÞ..ËùÒÔÄãÓ®ÂÞ..");
			else if (m.hp > 0)
				pressanykey("¶Ô·½ÂäÅÜÂÞ..ËùÒÔËãÄãÓ®ÂÞ.....");
			winorlose = 1;
		}
		if (d.death == 1 && mode == 1) {
			clear();
			...
			move(11, 0);
			prints
			    ("            [1;31m©¦  [37m¿ÉÁ¯µÄÐ¡¼¦ [33m%-10s                     [31m©¦[m",
			     d.name);
			move(12, 0);
			prints
			    ("            [1;31m©¦  [37mÔÚÓë [32m%-10s [37mµÄÕ½¶·ÖÐ£¬                [31m©¦[m",
			     p[n].name);
			move(13, 0);
			prints
			    ("            [1;31m©¦  [37m²»ÐÒµØÕóÍöÁË£¬ÔÚ´ËÌØ±ðÄ¬°§..........      [31m©¦[m");
			move(14, 0);
			prints
			    ("            [1;31m©¸¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©¼[m");
			pressanykey("Ð¡¼¦ÕóÍöÁË....");
			...
			move(13, 0);
			prints
			    ("            [1;31m©¦  [37m²»ÐÒµØ´òÊäÁË£¬¼ÇÕßÏÖ³¡ÌØ±ð±¨µ¼.........   [31m©¦[m");
			move(14, 0);
			prints
			    ("            [1;31m©¸¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©¼[m");
			pressanykey("Ð¡¼¦´òÊäÁË....");
...
struct magicset *p;
{
	...
	move(7, 0);
	sprintf(buf,
		"[1;31m©È[37;41m   ¿ÉÓÃ[%s]Ò»ÀÀ±í   [0;1;31m©À¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª[m",
		p[0].name);
	prints(buf);
	while ((s = p[n].name) && strcmp(s, "NULL") && (p[n].needmp <= d.mp)) {
		...
		if (p[n].hpmode == 1) {
			sprintf(buf,
				"[1;37m[[36m%d[37m] [33m%-10s  [37mÐèÒª·¨Á¦: [32m%-6d  [37m»Ö¸´ÌåÁ¦: [32m%-6d [37m»Ö¸´Æ£ÀÍ: [32m%-6d[m   ",
				n, p[n].name, p[n].needmp, p[n].hp, p[n].tired);
			prints(buf);
		} else if (p[n].hpmode == 2) {
			sprintf(buf,
				"[1;37m[[36m%d[37m] [33m%-10s  [37mÐèÒª·¨Á¦: [32m%-6d  [37m»Ö¸´ÌåÁ¦µ½[35m×î´óÖµ[37m »Ö¸´Æ£ÀÍµ½[35m×îÐ¡Öµ[m  ",
				n, p[n].name, p[n].needmp);
			prints(buf);
		} else if (p[n].hpmode == 0) {
			sprintf(buf,
				"[1;37m[[36m%d[37m] [33m%-10s  [37mÐèÒª·¨Á¦: [32m%-6d [m             ",
				n, p[n].name, p[n].needmp);
			prints(buf);
		}
		n++;
...
#else
		getdata(16, 4, buf, ans, 2, DOECHO, true);
#endif				// END MAPLE
		if (ans[0] != 'q' && ans[0] != 'Q') {
			pipkey = atoi(ans);
		}
	...
	if (ans[0] != 'q' && ans[0] != 'Q') {
#ifdef MAPLE
		getdata(17, 4, "È·¶¨Ê¹ÓÃÂð? [Y/n]", ans, 2, 1, 0);
#else
		getdata(17, 4, "È·¶¨Ê¹ÓÃÂð? [Y/n]", ans, 2, DOECHO, true);
#endif				// END MAPLE
		if (ans[0] != 'n' && ans[0] != 'N') {
