...
#else
	getdata(b_lines - 1, 1, "ÕæµÄÒª·ÅÉúÂð£¿(y/N)", buf, 2, DOECHO, true);
#endif				// END MAPLE
	if (buf[0] != 'y' && buf[0] != 'Y')
		return 0;
	sprintf(buf, "%s ±»ºÝÐÄµÄ %s ¶ªµôÁË~", d.name, userid);
	pressanykey(buf);
	d.death = 2;
...
pip_system_service()
{
	...
	move(b_lines, 0);
	prints
	    ("[1;44m  ·þÎñÏîÄ¿  [46m[1]ÃüÃû´óÊ¦ [2]±äÐÔÊÖÊõ [3]½á¾ÖÉè¾Ö                                [0m");
	pipkey = egetch();
	...
	case '1':
		...
		clrtobot();
#ifdef MAPLE
		getdata(b_lines - 1, 1, "°ïÐ¡¼¦ÖØÐÂÈ¡¸öºÃÃû×Ö£º", buf, 11,
			DOECHO, NULL);
#else
		getdata(b_lines - 1, 1, "°ïÐ¡¼¦ÖØÐÂÈ¡¸öºÃÃû×Ö£º", buf, 11,
			DOECHO, true);
	...
	case '2':		/*±äÐÔ */
		...
		if (d.sex == 1) {
			...
			move(b_lines - 1, 0);
			prints
			    ("[1;33m½«Ð¡¼¦ÓÉ[32m¡á[33m±äÐÔ³É[35m¡â[33mµÄÂð£¿ [37m[y/N][0m");
		} else {
			oldchoice = 1;	/*Ä¸-->¹« */
			move(b_lines - 1, 0);
			prints
			    ("[1;33m½«Ð¡¼¦ÓÉ[35m¡â[33m±äÐÔ³É[35m¡á[33mµÄÂð£¿ [37m[y/N][0m");
		}
		move(b_lines, 0);
		prints
		    ("[1;44m  ·þÎñÏîÄ¿  [46m[1]ÃüÃû´óÊ¦ [2]±äÐÔÊÖÊõ [3]½á¾ÖÉè¾Ö                                [0m");
		...
		if (pipkey == 'Y' || pipkey == 'y') {
			...
			if (d.sex == 1)
				sprintf(buf,
					"[1;37m%s %-11s°ÑÐ¡¼¦ [%s] ÓÉ¡á±äÐÔ³É¡âÁË[0m\n",
					Cdate(&now), userid, d.name);
			else
				sprintf(buf,
					"[1;37m%s %-11s°ÑÐ¡¼¦ [%s] ÓÉ¡â±äÐÔ³É¡áÁË[0m\n",
					Cdate(&now), userid, d.name);
			pip_log_record(buf);
	...
	case '3':
		...
		if (d.wantend == 1 || d.wantend == 2 || d.wantend == 3) {
			...
			move(b_lines - 1, 0);
			prints
			    ("[1;33m½«Ð¡¼¦ÓÎÏ·¸Ä³É[32m[ÓÐ20Ëê½á¾Ö][33m? [37m[y/N][0m");
			sprintf(buf, "Ð¡¼¦ÓÎÏ·Éè¶¨³É[ÓÐ20Ëê½á¾Ö]..");
		} else {
			oldchoice -= 3;	/*ÓÐ-->Ã»ÓÐ */
			move(b_lines - 1, 0);
			prints
			    ("[1;33m½«Ð¡¼¦ÓÎÏ·¸Ä³É[32m[Ã»ÓÐ20Ëê½á¾Ö][33m? [37m[y/N][0m");
...
{				/*¿´Ð¡¼¦¸öÈËÏêÏ¸×ÊÁÏ */
	...
	do {
		...
		move(b_lines, 0);
		sprintf(buf,
			"[1;44;37m  ×ÊÁÏÑ¡µ¥  [46m  [¡ü/PAGE UP]ÍùÉÏÒ»Ò³ [¡ý/PAGE DOWN]ÍùÏÂÒ»Ò³ [Q]Àë¿ª:            [m");
		prints(buf);
		pipkey = egetch();
		switch (pipkey) {
		case KEY_UP:
