...
pip_write_backup()
{
	...
	do {
		...
		move(b_lines - 1, 1);
		prints
		    ("´¢´æ [1]½ø¶ÈÒ» [2]½ø¶È¶ş [3]½ø¶ÈÈş [Q]·ÅÆú [1/2/3/Q]£º");
		pipkey = igetkey();
	...
	move(b_lines - 2, 1);
	prints("´¢´æµµ°¸»á¸²¸Ç´æ´¢´æì¶ [%s] µÄĞ¡¼¦µÄµµ°¸à¸£¡Çë¿¼ÂÇÇå³ş...",
	       files[num]);
...
pip_read_backup()
{
	...
	do {
		...
		move(b_lines - 1, 1);
		prints
		    ("¶ÁÈ¡ [1]½ø¶ÈÒ» [2]½ø¶È¶ş [3]½ø¶ÈÈş [Q]·ÅÆú [1/2/3/Q]£º");
		pipkey = igetkey();
		...
		if (num > 0) {
			...
			if ((fs = fopen(buf, "r")) == NULL) {
				...
				move(b_lines - 2, 1);
				prints
				    ("¶ÁÈ¡³öµµ°¸»á¸²¸ÇÏÖÔÚÕıÔÚÍæµÄĞ¡¼¦µÄµµ°¸à¸£¡Çë¿¼ÂÇÇå³ş...");
				sprintf(buf,
					"È·¶¨Òª¶ÁÈ¡³ö [%s] µµ°¸Âğ£¿ [y/N]:",
					files[num]);
#ifdef MAPLE
...
pip_live_again()
{
	...
	clear();
	showtitle("Ğ¡¼¦¸´»îÊÖÊõÖĞ", BBS_FULL_NAME);
