...
pip_marriage_offer()
{
	...
	int money;
	int who;
	const static char *name[5][2] = { {"Å®ÉÌÈË£Á", "ÉÌÈË£Á"},
	{"Å®ÉÌÈË£Â", "ÉÌÈË£Â"},
	{"Å®ÉÌÈË£Ã", "ÉÌÈË£Ã"},
	{"Å®ÉÌÈË£Ä", "ÉÌÈË£Ä"},
	{"Å®ÉÌÈË£Å", "ÉÌÈË£Å"}
	...
	sprintf(buf, "%s´øÀ´ÁË½ğÇ®%d£¬ÒªÏòÄãµÄĞ¡¼¦Çó»é£¬ÄúÔ¸ÒâÂğ£¿[y/N]",
	...
	if (ans[0] == 'y' || ans[0] == 'Y') {
		if (d.wantend != 1 && d.wantend != 4) {
			sprintf(buf,
				"¨Ú¡«Ö®Ç°ÒÑ¾­ÓĞ»éÔ¼ÁË£¬ÄúÈ·¶¨Òª½â³ı¾É»éÔ¼£¬¸Ä¶¨Á¢»éÔ¼Âğ£¿[y/N]");
#ifdef MAPLE
			getdata(b_lines - 1, 1, buf, ans, 2, 1, 0);
#else
			getdata(b_lines - 1, 1, buf, ans, 2, DOECHO, true);
...
{				/*ÊÕ»ñ¼¾ */
	...
	sprintf(buf, "[A]%s [B]%s [C]%s [D]%s [Q]·ÅÆú:", showname[1],
	...
	case 'a':
		...
		for (i = 0; i < 3; i++) {
			a = 0;
			b[i][1] = 0;
			sprintf(buf, "ÄãµÄµÚ%d¸ö¶ÔÊÖÊÇ%s", i + 1,
