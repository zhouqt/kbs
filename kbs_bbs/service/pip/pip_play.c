...
{				/*½¼ÓÎ */
	...
	if (d.money < 250) {
		...
		if (lucky == 0) {
			...
			if (rand() % 2 > 0)
				pressanykey
				    ("ÈÃÎÒÃÇ·è¿ñÔÚÒ¹ÀïµÄº£Ì²°É....ºôºô..");
			else
				pressanykey
				    ("Á¹Ë¬µÄº£·çÓ­ÃæÏ®À´  ×îÏ²»¶ÕâÖÖ¸Ğ¾õÁË....");
		}
		if ((rand() % 301 + rand() % 200) % 100 == 12) {
			...
			move(18, 10);
			prints
			    ("[1;37m¿´µ½ÄãÕâÑùÅ¬Á¦µÄÅàÑø×Ô¼ºµÄÄÜÁ¦  ÈÃÎÒĞÄÖĞÊ®·ÖµÄ¸ßĞËà¸..[m");
			move(19, 10);
			prints
			    ("[1;36mĞ¡ÌìÊ¹ÎÒ¾ö¶¨¸øÄã½±ÉÍ¹ÄÀø¹ÄÀø  ÍµÍµµØ°ïÖúÄãÒ»ÏÂ....^_^[0m");
			move(20, 10);
			lucky = rand() % 7;
			if (lucky == 6) {
				prints
				    ("[1;33mÎÒ½«°ïÄãµÄ¸÷ÏîÄÜÁ¦È«²¿ÌáÉı°Ù·ÖÖ®Îåà¸......[0m");
				...
				prints
				    ("[1;33mÎÒ½«°ïÄãµÄÕ½¶·ÄÜÁ¦È«²¿ÌáÉı°Ù·ÖÖ®Ê®à¸.......[0m");
				...
				prints
				    ("[1;33mÎÒ½«°ïÄãµÄÄ§·¨ÄÜÁ¦ºÍÉúÃüÁ¦È«²¿ÌáÉı°Ù·ÖÖ®Ê®à¸.......[0m");
				...
				prints
				    ("[1;33mÎÒ½«°ïÄãµÄ¸ĞÊÜÄÜÁ¦È«²¿ÌáÉı°Ù·ÖÖ®¶şÊ®à¸....[0m");
