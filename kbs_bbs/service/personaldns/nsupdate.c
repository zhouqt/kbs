				...
				const char *key, const char *host, const char *ip, int ttl)
{
	...
	argv[0] = malloc(strlen("nsupdate")+1);
	strcpy(argv[0], "nsupdate");
	/* emulate the cmdline file */
	cmds = malloc(sizeof(char*)*8);	/* 7 lines + null */
	snprintf(buf, sizeof(buf)-1, "server %s\n", server);
	buf[sizeof(buf)-1] = '\0';
	cmds[0] = strdup(buf);
	snprintf(buf, sizeof(buf)-1, "zone %s\n", zone);
	buf[sizeof(buf)-1] = '\0';
	cmds[1] = strdup(buf);
	snprintf(buf, sizeof(buf)-1, "key %s %s\n", keyname, key);
	buf[sizeof(buf)-1] = '\0';
	cmds[2] = strdup(buf);
	snprintf(buf, sizeof(buf)-1, "update delete %s A\n", host);
	buf[sizeof(buf)-1] = '\0';
	cmds[3] = strdup(buf);
	snprintf(buf, sizeof(buf)-1, "update add %s %d A %s\n", host, ttl, ip);
	buf[sizeof(buf)-1] = '\0';
	cmds[4] = strdup(buf);
	cmds[5] = strdup("send\n");
	cmds[6] = strdup("quit\n");
	cmds[7] = NULL;
	cmdsno = 0;	/* set to the firest line */
...
int main(int argc, char **argv)
{
	argc = argc;
	argv = argv;
	return update_dns("166.111.4.19", "thu.dlg.cn", "update-thu-dlg", "wSSyXVra+MdYoJU3l3FbiQ==",
					"lqx", "166.111.3.34", 60);
}
#endif
#endif