...
void start_game()
{
    ...
    for(i=0;i<myroom->people;i++)
        if(!(inrooms.peoples[i].flag&PEOPLE_SPECTATOR)) 
            total++;
    if(total<6) {
        send_msg(inrooms.peoples+me, "\x1b[31mÖÁÉÙ6ÈË²Î¼Ó²ÅÄÜ¿ªÊ¼ÓÎÏ·\x1b[m");
    ...
    if(totalk>total) {
        send_msg(inrooms.peoples+me, "\x1b[31m×ÜÈËÊýÉÙÓÚÒªÇóµÄ»µÈËÈËÊý,ÎÞ·¨¿ªÊ¼ÓÎÏ·\x1b[m");
        kill(inrooms.peoples[me].pid, SIGUSR1);
        end_change_inroom();
        return;
    }
    ...
    for(i=0;i<totalk;i++) {
        ...
        send_msg(inrooms.peoples+j, "Äã×öÁËÒ»¸öÎÞ³ÜµÄ»µÈË\nÏÂÃæÓÃÄãµÄ¼âµ¶(\x1b[31;1mCtrl+S\x1b[m)Ñ¡ÔñÄãÒª²Ðº¦µÄÈË°É...");
    }
    for(i=0;i<totalk;i++) {
        inrooms.peoples[j].flag |= PEOPLE_ALIVE;
        if(!(inrooms.peoples[j].flag&PEOPLE_KILLER))
            send_msg(inrooms.peoples+j, "ºÚÉ«µÄÒ¹Ä»½µÁÙÁË...");
    }
    for(i=0;i<myroom->people;i++)
