...
void start_game()
{
    ...
    sprintf(buf, "\x1b[31;1mÓÎÏ·¿ªÊ¼À²!\x1b[m");
    for(i=0;i<myroom->people;i++)
    ...
    for(i=0;i<totalk;i++) {
        ...
        send_msg(inrooms.peoples+j, "Äã×öÁËÒ»¸öÎÞ³ÜµÄ»µÈË\nÓÃÄãµÄ¼âµ¶(\x1b[31;1mCtrl+S\x1b[m)Ñ¡ÔñÄãÒª²Ðº¦µÄÈË°É...");
    }
    for(i=0;i<myroom->people;i++) 
    if(!(inrooms.peoples[i].flag&PEOPLE_SPECTATOR))
    {
        ...
        case '\r':
            switch(sel) {
                ...
                case 2:
                    ...
                    resetcolor();
                    clrtoeol();
                    getdata(t_lines-1, 0, "ÇëÊäÈëÃû×Ö:", buf, 30, 1, 0, 1);
                    if(buf[0]) {
                        for(me=0;me<myroom->people;me++)
                            if(inrooms.peoples[me].pid==uinfo.pid) break;
                        j=0;
                ...
                case 4:
                    ...
                    resetcolor();
                    clrtoeol();
                    getdata(t_lines-1, 0, "ÇëÊäÈë»°Ìâ:", buf, 30, 1, 0, 1);
                    if(buf[0]) {
                        start_change_inroom(myroom);
...
void join_room(struct room_struct * r)
{
    ...
    while(1){
        do{
            int ch;
            ch=-getdata(t_lines-1, 0, "ÊäÈë:", buf, 75, 1, NULL, 1);
...
quitgame2:
    getdata(t_lines-1, 0, "¼Ä»Ø±¾´ÎÈ«²¿ÐÅÏ¢Âð?[y/N]", buf, 3, 1, 0, 1);
    if(toupper(buf[0])=='Y') {
        sprintf(buf, "home/%c/%s/.INROOMMSG%d", toupper(currentuser->userid[0]), currentuser->userid, uinfo.pid);
