        ...
        do{
            ...
            if(ch==KEY_UP) {
                ...
                if(inrooms[myroom].peoples[me].flag&PEOPLE_TESTED) {
                    send_msg(pid, "\x1b[31;1m±æ¬÷ƒ„“—æ≠’Ï≤Èπ˝¡À\x1b[m");
                    refreshit();
                    continue;
                }
                if(inrooms[myroom].peoples[sel].flag&PEOPLE_SPECTATOR)
                    send_msg(pid, "\x1b[31;1m¥À»À «≈‘π€’ﬂ\x1b[m");
                else if(!(inrooms[myroom].peoples[sel].flag&PEOPLE_ALIVE))
                    send_msg(pid, "\x1b[31;1m¥À»À“—À¿\x1b[m");
                else if(!(inrooms[myroom].peoples[sel].flag&PEOPLE_KILLER)) {
                    inrooms[myroom].peoples[me].flag|=PEOPLE_TESTED;
                    send_msg(pid, "\x1b[31;1mæ≠π˝ƒ„µƒ’Ï≤‚, ∑¢œ÷¥À»À «ªµ»À!!!\x1b[m");
                }
                else {
                    inrooms[myroom].peoples[me].flag|=PEOPLE_TESTED;
                    send_msg(pid, "\x1b[31;1mæ≠π˝ƒ„µƒ’Ï≤‚, ∑¢œ÷¥À»À «∫√»À\x1b[m");
                }
                refreshit();
            }
            else if(ch==Ctrl('S')) {
