...
static int room_list_refresh(struct _select_def *conf)
{
    ...
    move(2, 0);
    prints("[0;1;37;44m    %4s %-14s %-12s %4s %4s %4s %4s", "±àºÅ", "ÓÎÏ·ÊÒÃû³Æ", "´´½¨Õß", "ÀàÐÍ", "ÈËÊý", "×î¶à", "Ëø¶¨");
    clrtoeol();
    resetcolor();
    update_endline();
...
static int room_list_show(struct _select_def *conf, int i)
{
    ...
    if(r)
        prints("  %3d  %-14s %-12s %4s %3d  %3d   %2s", i, r->name, r->creator, "É±ÈË", r->people, r->maxpeople, (r->flag&ROOM_LOCKED)?"ÊÇ":"·ñ");
    return SHOW_CONTINUE;
}
