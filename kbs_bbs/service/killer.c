...
void kill_msg(int u)
{
    ...
    if(u==-1||i==u) {
        ...
        if(j==-1) {
            sprintf(buf, "%sµÙœﬂ¡À", inrooms[myroom].peoples[i].nick[0]?inrooms[myroom].peoples[i].nick:inrooms[myroom].peoples[i].id);
            send_msg(-1, buf);
            start_change_inroom();
            ...
            if(inrooms[myroom].peoples[i].flag&PEOPLE_ROOMOP) {
                ...
                if(inrooms[myroom].peoples[k].style!=-1&&!(inrooms[myroom].peoples[k].flag&PEOPLE_SPECTATOR))
                {
                    ...
                    sprintf(buf, "%s≥…Œ™–¬∑ø÷˜", inrooms[myroom].peoples[k].nick[0]?inrooms[myroom].peoples[k].nick:inrooms[myroom].peoples[k].id);
                    send_msg(-1, buf);
                    break;
                }
            }
