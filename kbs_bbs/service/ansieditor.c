...
char dirt_str[2][2] = {" ", "*"};
char drawing_str[3][7] = {"Ì§±Ê", "Âä±Ê", "¸üÐÂ"};
char linemode_str[4][7]={"ÐÐ²ÁÐ´","ÐÐ²åÈë","ÁÐ²ÁÐ´","ÁÐ²åÈë"};
char state_str[8][10]={"±à¼­","¿éÑ¡ÖÐ","¿é±à¼­","´øÉ«Õ³Ìù", "ÃüÁî°ïÖú", "GBKÂë°ïÖú", "¼ÄÓÊ¼þ"};
...
int confirm_quit()
{
	...
	save_cur();
	gotoxy(20, 10);
	prints("[0mÄãËù×÷µÄ¸Ä¶¯Î´±£´æ,È·¶¨ÒªÍË³öÂð(Y/N)?");	
	while (1) {
		ch = getch();
		if (toupper(ch) == 'Y')
			return 1;
...
void help_code(int page)
{
	int code;
	int i,j;
	char* url = "¸ü¶àÇë²Î¼û:http://www.khngai.com/chinese/charmap/tblgbk.php?page=0";
...
void _absob()
{
	...
	chkpt_commit();
	strcpy(message, "[0mµ±Ç°ÎÄ±¾µÄÊôÐÔ±»²ÉÓÃ.");
	draw_sbar();
}
void _reset()
{
...
void _apen()
{
	...
	if (num_pens == MAX_PENS)
		sprintf(message, "[31m10Ö§±ÊÒÑÂú,ÎÞ·¨ÔÙÌí¼Ó.[0m");
	else {
		ci = &graph[cur_y][cur_x];
		assert(!ci->chinese || ci->chinese && !ci->half);
...
void _dpen()
{
	...
	if (!cur_pen) 
		strcpy(message, "[31m´Ë0ºÅ±ÊÊÇÏðÆ¤²Á,²»ÄÜ±»É¾³ý.[0m");
	else {
		chkpt_begin(-1, -1, 3, &cur_pen, sizeof(char), &num_pens, sizeof(char), pens, sizeof(pens));
		...
		if (cur_pen >= num_pens)
			cur_pen = num_pens-1;
		sprintf(message, "[0m»­±Ê%sÒÑ±»É¾³ý.",s);
		chkpt_commit();
	}
	draw_sbar();
}
...
{
	if (!hcmd_total_pages) {
		strcpy(message, "[31mÎÞ·¨×°ÔØÃüÁî°ïÖúÎÄ¼þ.[0m");
		state = prev_state;
		draw_sbar();
		return;
	}
	...
	while (1) {
		...
		if (n < 15 && (ch>='0' && ch<='9' || ch>='a' && ch<='z' || ch>='A' && ch<='Z' || ch=='_'|| ch=='-' || ch=='.')) {
			...
			if (!n) 
				...
				anchor_dirt = dirt = 0;
				sprintf(message, "[0m½ø¶ÈÎÄ¼þ%s¶ÁÈ¡³É¹¦.", save_file);
	...
	while (1) {
		...
		if (n < 15 && (ch>='0' && ch<='9' || ch>='a' && ch<='z' || ch>='A' && ch<='Z' || ch=='_'|| ch=='-' || ch=='.')) {
			...
			if (!n) 
				...
				anchor_dirt = dirt;
				sprintf(message, "[0m½ø¶ÈÎÄ¼þ%s±£´æ³É¹¦.", save_file);
			}
			else
...
void _export()
{
	...
	while (1) {
		sprintf(message, "[0mÊä³öµ½ÎÄ¼þ(CTRL_QÈ¡Ïû):%s", export_file);
		draw_sbar();
		...
		if (n < 15 && (ch>='0' && ch<='9' || ch>='a' && ch<='z' || ch>='A' && ch<='Z' || ch=='_'|| ch=='-' || ch=='.')) {
			...
			if (!n) 
				...
				sprintf(message, "[0mÎÄ¼þ%sÊä³ö³É¹¦.", export_file);
			else
...
void _redo()
{
	...
	char r1, r2;
	if (!redo(&r1, &r2))
		strcpy(message, "[31mÎÞ·¨½øÐÐREDO.[0m");
	else {
		if (r1!=-1 && r2!=-1) {
			save_cur();
			for (i=r1; i<=r2; i++) {
...
void _undo()
{
	...
	char r1, r2;
	if (!undo(&r1, &r2))
		strcpy(message, "[31mÎÞ·¨½øÐÐUNDO.[0m");
	else {
		if (r1!=-1 && r2!=-1) {
			save_cur();
			for (i=r1; i<=r2; i++) {
...
void _sskey()
{
	...
	*ci1 = *ci;
	...
	sprintf(message, "[0m%s±»ÉèÎªµÚ%dºÅ¿ì½Ý¼ü.", get_char_str(&sc_keys[n][0],0), n);
	chkpt_commit();
	draw_sbar();
}
void _lskey()
...
void _rpst()
{
	...
	prints(get_row_str(0, cur_y, 0));
	load_cur();
	sprintf(message, "[0m¸´ÖÆÎÄ±¾ÒÑÕ³ÌùÖÁ±¾ÐÐ.");
	dirt++;
	chkpt_commit();
	draw_sbar();
}
...
void _mail()
{
	...
	mail_step = ML_ADDR;
	mail_sent = 0;
	strcpy(message, "[0mÈ¡Ïû¼ÄÐÅÓÃCTRL_Q.");
	draw_sbar();
}
void _ret()
{
...
void _blk2()
{
	...
	strcpy(message, "[0mÇëÉèÖÃ¿éµÄÊôÐÔ,CTRL_Q½áÊø,<ESC>h°ïÖú.");
	clr_reverse();
	draw_graph();
	draw_sbar();
}
...
{
	if (!mail_sent)
		strcpy(message, "[0mÈ¡Ïû¼ÄÐÅ.");
	else if (mail_sent<0)
		strcpy(message, "[31m·¢ÐÅÊ§°Ü.[0m");
	else
		strcpy(message, "[0mÐÅÒÑ¼Ä³ö.");
	draw_graph();
	draw_sbar();
...
void _mconf()
{
	char *str1 = "ÇëÊäÈëÓÊ¼þ±êÌâ:";
    struct userec *u = NULL;
	switch (mail_step) {
		case ML_ADDR: 
			 if (!strchr(mail_addr, '@')) {
			    if (HAS_PERM(currentuser,PERM_DENYMAIL)) {
					strcpy(message, "[31mÄãÎÞÈ¨¸ø±¾Õ¾ÓÃ»§¼ÄÐÅ.[0m");
					draw_sbar();
					break;
				}
...
int editor_main()
{
	...
	//tty_raw(0);
	clr_scr();
	sprintf(message, "»¶Ó­Ê¹ÓÃ%s,<ESC>h°ïÖú.", EDITOR_NAME);
	draw_sbar();
	gotoxy(0,0);
	state=S_EDIT;
	while (1) {
