...
{
	if (conf->item_count<=0) return SHOW_QUIT;
...
static int show_item(struct _select_def* conf,int item,bool clear)
{
	...
	if (conf->show_data)
		if ((*conf->show_data)(conf,item)==SHOW_QUIT) return SHOW_QUIT;
...
static int refresh_select(struct _select_def* conf)
{
	...
	for (i=conf->page_pos;(i<conf->page_pos+conf->item_per_page)
		&&(i<=conf->item_count);i++)
		if (show_item(conf,i,false)==SHOW_QUIT) return SHOW_QUIT;
	refresh();
	return SHOW_CONTINUE;
}
...
static int select_change(struct _select_def* conf,int new_pos)
{
	...
	if (new_pos<=0||new_pos>conf->item_count) {
		...
		if ((new_pos==conf->item_count+1)&&(conf->flag&LF_LOOP)) {
			new_pos=1;
		}
		else {
			select_wrong(conf);
...
static int do_select_internal(struct _select_def* conf,int key)
{
	...
	if (!(conf->flag&LF_INITED)) {
		...
		if (refresh_select(conf)==SHOW_QUIT) return SHOW_QUIT;
	}
	if (key==KEY_INIT) return SHOW_CONTINUE;
	switch (key) {
	case KEY_REFRESH:
			...
			return SHOW_QUIT;
		{
			...
			for (i=0;i<conf->item_per_page;i++) {
				if (conf->item_pos[i].y==conf->item_pos[idx].y) {
				    if ((conf->item_pos[i].x<conf->item_pos[idx].x)&&
				       (conf->item_pos[i].x>x)) {
				       x=conf->item_pos[i].x;
				       findidx=i;
			            }
				    if ((conf->item_pos[i].x>conf->item_pos[idx].x)&&
				       (conf->item_pos[i].x<x2)) {
...
//	int old_item_count = conf->item_count;
	...
	case SHOW_DIRCHANGE:
		...
		if (check_valid(conf)==SHOW_QUIT) return SHOW_QUIT;
	case SHOW_REFRESH:
		refresh_select(conf);
		break;
	case SHOW_SELCHANGE: 
