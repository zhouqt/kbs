...
int     real, unum;
{
    ...
    case '1':
        ...
        if( real ) {
            ...
            sprintf( genbuf, "½«×¢²áÈÕÆÚÌáÇ°ÈıÌì [Y/N]");
            getdata( i++, 0, genbuf, buf, 16, DOECHO, NULL, YEA);
    ...
    case '2':
        ...
        {
            prints("\n\n´íÎó!ÏµÍ³½ûÖ¹ĞŞ¸ÄSYSOPµÄÃÜÂë,¾¯²ìÕıÔÚÀ´µÄÂ·ÉÏ :)");
            pressreturn();
            clear();
            return 0;
        }
...
x_fillform()
{
    ...
    {
        prints( "ÄúÊ×´ÎµÇÈë±¾Õ¾Î´Âú3Ìì(72¸öĞ¡Ê±)..." );
        prints( "ÇëÏÈËÄ´¦ÊìÏ¤Ò»ÏÂ£¬ÔÚÂú3ÌìÒÔºóÔÙÌîĞ´×¢²áµ¥¡£");
        pressreturn();
        return;
    }
    ...
    getdata(t_lines-1,8,"ÄúÈ·¶¨ÒªÌîĞ´×¢²áµ¥Âğ (Y/N)? [N]: ",ans,3,DOECHO,NULL,YEA);
    if( ans[0] != 'Y' && ans[0] != 'y' )
        return;
    ...
    while( 1 ) {
        ...
        getfield( 14, "Äê.ÔÂ.ÈÕ(¹«Ôª)(Ñ¡ÔñÌîĞ´)",     "³öÉúÄêÔÂ", birth, STRLEN );
