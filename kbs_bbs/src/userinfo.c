...
int     real, unum;
{
    ...
    case '1':
        ...
        if( real ) {
            sprintf( genbuf, "’Ê µEmail[%s]: ", u->realemail );
            getdata( i++, 0, genbuf, buf, STRLEN, DOECHO, NULL ,YEA);
    ...
    case '2':
        if( ! real ) {
            getdata(i++,0,"«Î ‰»Î‘≠√‹¬Î: ",buf,39,NOECHO,NULL,YEA);
        ...
        getdata(i++,0,"«Î…Ë∂®–¬√‹¬Î: ",buf,39,NOECHO,NULL,YEA);
        if( buf[0] == '\0' ) {
            prints("\n\n√‹¬Î…Ë∂®»°œ˚, ºÃ–¯ π”√æ…√‹¬Î\n");
            fail++;
            break;
        }
        getdata(i++,0,"«Î÷ÿ–¬ ‰»Î–¬√‹¬Î: ",genbuf,39,NOECHO,NULL,YEA);
