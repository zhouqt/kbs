...
int m_newbrd()
{
    ...
    getdata(8, 0, "ÊÇ·ñ²»¼ÇÎÄÕÂÊı(Y/N)? [N]: ", ans, 4, DOECHO, NULL, YEA);
    if (ans[0] == 'Y' || ans[0] == 'y')
        newboard.flag|=BOARD_JUNK;
    getdata(9, 0, "ÊÇ·ñ¿ÉÏòÍâ×ªĞÅ(Y/N)? [N]: ", ans, 4, DOECHO, NULL, YEA);
    if (ans[0] == 'Y' || ans[0] == 'y')
...
int m_editbrd()
{
    ...
    getdata(9, 0, "ÊÇ·ñ¸ü¸ÄÒÔÉÏ×ÊÑ¶? (Yes or No) [N]: ", genbuf, 4, DOECHO, NULL, YEA);
...
enterbname:
        ...
        getdata(15, 0, "ÊÇ·ñÒÆ¶¯¾«»ªÇøµÄÎ»ÖÃ (Y/N)? [N]: ", genbuf, 4, DOECHO, NULL, YEA);
        if (*genbuf == 'Y' || *genbuf == 'y')
            a_mv = 2;
        else
            a_mv = 0;
        getdata(16, 0, "ÊÇ·ñ¸ü¸Ä´æÈ¡È¨ÏŞ (Y/N)? [N]: ", genbuf, 4, DOECHO, NULL, YEA);
        if (*genbuf == 'Y' || *genbuf == 'y')
        {
            char            ans[5];
        ...
        {
            getdata(17, 0, "È·¶¨Òª¸ü¸ÄÂğ? (Y/N) [N]: ", genbuf, 4, DOECHO, NULL, YEA);
        }
        if (*genbuf == 'Y' || *genbuf == 'y')
        {
            {
