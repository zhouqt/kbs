...
int m_newbrd()
{
    ...
    getdata(9, 0, "ÊÇ·ñ²»Í³¼ÆÊ®´ó(Y/N)? [N]: ", ans, 4, DOECHO, NULL, true);
    if (ans[0] == 'Y' || ans[0] == 'y')
        newboard.flag |= BOARD_POSTSTAT;
    getdata(10, 0, "ÊÇ·ñ¿ÉÏòÍâ×ªÐÅ(Y/N)? [N]: ", ans, 4, DOECHO, NULL, true);
    if (ans[0] == 'Y' || ans[0] == 'y')
        newboard.flag |= BOARD_OUTFLAG;
    getdata(11, 0, "ÊÇ·ñ²»¿ÉreÎÄÕÂ(Y/N)? [N]: ", ans, 4, DOECHO, NULL, true);
    if (ans[0] == 'Y' || ans[0] == 'y')
...
int m_editbrd()
{
    ...
    prints("ÄäÃûÌÖÂÛÇø: %s  ²»¼ÇÎÄÕÂÊý: %s  ²»Í³¼ÆÊ®´ó: %s  ÊÇ·ñÊÇÄ¿Â¼: %s\n", 
    ...
    prints("ËùÊôÄ¿Â¼£º%s\n",bh?groupname:"ÎÞ");
    prints("¿ÉÏòÍâ×ªÐÅ: %s  ¿ÉÕ³Ìù¸½¼þ: %s ÔÊÐíEmail·¢ÎÄ: %s ²»¿ÉreÎÄ: %s\n", 
    ...
    getdata(10, 0, "ÊÇ·ñ¸ü¸ÄÒÔÉÏ×ÊÑ¶(°´S»Ø³µÐÞ¸ÄÃèÊö)? (Yes or No) [N]: ", genbuf, 4, DOECHO, NULL, true);
	if (*genbuf == 's' || *genbuf == 'S'){

		move(11,0);
		prints("ÇëÊäÈëÐÂµÄÃèÊö:");
		multi_getdata(12, 0, 79, NULL, newfh.des, 195, 8, false, 0);
		if( newfh.des[0] ){
        	getdata(21, 0, "È·¶¨Òª¸ü¸ÄÂð? (Y/N) [N]: ", genbuf, 4, DOECHO, NULL, true);
        	if (*genbuf == 'Y' || *genbuf == 'y') {
            	set_board(pos, &newfh, &fh);
            	sprintf(genbuf, "¸ü¸ÄÌÖÂÛÇø %s µÄÃèÊö --> %s", fh.filename, newfh.filename);
            	bbslog("user", "%s", genbuf);
			}
		}
      ...
      enterbname:
        ...
        sprintf(buf, "²»Í³¼ÆÊ®´ó (Y/N)? [%c]: ", (newfh.flag & BOARD_POSTSTAT) ? 'Y' : 'N');
        getdata(line++, 0, buf, genbuf, 4, DOECHO, NULL, true);
        if (*genbuf == 'y' || *genbuf == 'Y' || *genbuf == 'N' || *genbuf == 'n') {
            if (*genbuf == 'y' || *genbuf == 'Y')
        ...
        sprintf(buf, "²»¿ÉreÎÄ (Y/N)? [%c]: ", (newfh.flag & BOARD_NOREPLY) ? 'Y' : 'N');
        getdata(line++, 0, buf, genbuf, 4, DOECHO, NULL, true);
        if (*genbuf == 'y' || *genbuf == 'Y' || *genbuf == 'N' || *genbuf == 'n') {
            if (*genbuf == 'y' || *genbuf == 'Y')
