...
int x_deny()
{
    ...
    const struct _select_item level_conf[] = {
        ...
        {3, 11, -1, SIT_SELECT, (void *) "6)»»Ò»¸öID"},
    ...
    if (flag>0) {
		...
		move(18,0);
    		 ...
    		 else
   		{
			...
			if (flag ==1 ) sprintf(genbuf, "ÈÎÃü %s µÄ°æÖ÷Îª %s µÄÈ¨ÏŞXPERM%d %d", newfh.filename, lookupuser->userid, lookupuser->userlevel, newlevel);
			else if (flag ==2 ) sprintf(genbuf, "ÃâÈ¥ %s µÄ°æÖ÷ %s µÄÈ¨ÏŞXPERM%d %d", newfh.filename, lookupuser->userid, lookupuser->userlevel, newlevel);
