...
int
a_Save( path, key, fileinfo ,nomsg)  /* ½«ÎÄ¼ş ·ÅÈë ¾«»ªÇøÔİ´æµµ */
char    *path, *key;
struct fileheader *fileinfo;
int    nomsg;
{
...
/*      case ADDGOPHER:
        {
            ...
            a_prompt( -2, "Á¬ÏßµÄÎ»ÖÃ£º", ghost );
            if(ghost[0]=='\0')
                return ;
            a_prompt( -2, "Á¬ÏßµÄÄ¿Â¼£º", gfname );
            if(gfname[0]=='\0')
                return ;
            a_prompt( -2, "Á¬ÏßµÄPort£º", ans );
            if(ans[0]=='\0')
                return ;
            a_prompt( -2, "±êÌâ£º",gtitle );
            if(gtitle[0]=='\0')
                return ;
            gport=atoi(ans);
...
ÔÚ¾«»ªÇøÔÄ¶ÁÎÄÕÂµ½Ä©Î²Ê±£¬ÓÃÉÏ£¯ÏÂ¼ıÍ·Ö±½ÓÌø×ªµ½Ç°£¯ºóÒ»Ïî */
                        ansimore( fname, NA );
...
char group[STRLEN],bname[STRLEN],title[STRLEN],gname[STRLEN];
{
    ...
    if(!dashd(bpath))
    {
        if(dashd(old_bpath)) /*¼ì²éÊÇ·ñ´æÔÚÒÔÇ°µÄpalmbbsµÄ¾«»ªÇø£¬×Ô¶¯mvµ½ºÏÊÊÎ»ÖÃ */
        {
