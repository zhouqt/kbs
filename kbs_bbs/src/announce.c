    ...
    if (ann_get_path(key, buf, sizeof(buf)) == 0)
	{
		...
		*/
		...
		if (a_savenames(&pm)==0) {
			...
			sprintf(buf, " ÊÕÈë¾«»ªÇøÊ§°Ü£¬¿ÉÄÜÓĞÆäËû°æÖ÷ÔÚ´¦ÀíÍ¬Ò»Ä¿Â¼£¬°´ Enter ¼ÌĞø ", /*fileinfo->title, */ pm.path);
			if (!nomsg)
				a_prompt(-1, buf, ans);
                     a_loadnames(&pm);
		}
    ...
    int mode;
{
    ...
    if (!valid_fname(fname)) {
	    ...
	    sprintf(buf, " ÕûÀí¾«»ªÇøÊ§°Ü£¬¿ÉÄÜÓĞÆäËû°æÖ÷ÔÚ´¦ÀíÍ¬Ò»Ä¿Â¼£¬°´ Enter ¼ÌĞø ", /*fileinfo->title, */ pm.path);
           a_prompt(-1, buf, ans);
           a_loadnames(pm);
       }
  }
    ...
    MENU *pm;
{
    ...
    if (a_savenames(pm)==0) {
       ...
       char buf[80],ans[40];
       sprintf(buf, " É¾³ıÊ§°Ü£¬¿ÉÄÜÓĞÆäËû°æÖ÷ÔÚ´¦ÀíÍ¬Ò»Ä¿Â¼£¬°´ Enter ¼ÌĞø ", /*fileinfo->title, */ pm.path);
       a_prompt(-1, buf, ans);
       a_loadnames(pm);
   }
}
