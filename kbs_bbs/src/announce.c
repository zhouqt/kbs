...
MENU *pm;
{
    ...
    for (n = pm->page; n < pm->page + BBS_PAGESIZE && n < pm->num; n++) {
        ...
        /* etnlegend, 2006.04.29, ÌìÄÄÕâÔ­À´¶¼ÊÇÊ²Ã´Ğ´·¨... */
        ...
        if(!(pm->item[n]->host)){
            switch(st.st_mode&S_IFMT){
                case S_IFDIR:
                    ...
                    break;
                case S_IFREG:
                    snprintf(kind,32,"%s","[\033[0;36mÎÄ¼ş\033[m]");
                    break;
                case S_IFLNK:
                    if(stat(genbuf,&st)==-1||!(S_ISDIR(st.st_mode)||S_ISREG(st.st_mode)))
                        ...
                        snprintf(kind,32,"%s","[\033[0;33mÎÄ¼ş\033[m]");
                    break;
                default:
                    snprintf(kind,32,"%s","[\033[0;32m´íÎó\033[m]");
                    break;
        ...
        if (!strncmp(title,"[Ä¿Â¼] ",7)||!strncmp(title,"[ÎÄ¼ş] ",7)||!strncmp(title,"[Á¬Ïß] ",7))
        ...
        /* --END--, etnlegend, 2006.04.29, ÌìÄÄÕâ¶¼ÊÇÊ²Ã´Ğ´·¨... */
...
/* etnlegend, 2006.04.29, [¾«»ªÇø] ¼ôÇĞ/¸´ÖÆ/Õ³Ìù²Ù×÷ */
...
#define ACP_ANY_RETURN(msg) do{prints("\033[1;37m%s\033[0;33m<Any>\033[m",(msg));igetkey();pm->page=9999;return;}while(0)
    ...
    if(!(fp=fopen(genbuf,(!(mode==0||mode==1)?"r":"w"))))
        ACP_ANY_RETURN("Ê¹ÓÃÕ³ÌùÃüÁîÇ°Ó¦ÏÈÊ¹ÓÃ¼ôÇĞ»ò¸´ÖÆÃüÁî...");
    if(mode==0||mode==1){
        item=pm->item[pm->now];
        ...
        fclose(fp);
        ACP_ANY_RETURN((!mode?"¸´ÖÆ±êÊ¶Íê³É, µ«ÔÚÕ³Ìùºó·½¿ÉÉ¾³ıÔ´ÎÄ¼ş»òÄ¿Â¼!":"¼ôÇĞ±êÊ¶Íê³É, µ«ÔÚÕ³Ìùºó·½¿ÉÉ¾³ıÔ´ÎÄ¼ş»òÄ¿Â¼!"));
    }
    do{
...
#undef ACP_DUMPS
    ...
    if(!((access(newpath,F_OK)==-1)&&(errno==ENOENT)))
        ACP_ANY_RETURN("µ±Ç°Â·¾¶ÏÂÒÑ´æÔÚÍ¬ÃûÎÄ¼ş»òÄ¿Â¼...");
    if(!strncmp(path,newpath,(len=strlen(path)))&&newpath[len]=='/')
        ACP_ANY_RETURN("µ±Ç°²Ù×÷¼ôÇĞ»ò¸´ÖÆÄ¿Â¼ÖÁÆä×ÓÄ¿Â¼ÖĞ, ½«µ¼ÖÂËÀÑ­»·...");
    if(stat(path,&st)==-1||!(S_ISDIR(st.st_mode)||S_ISREG(st.st_mode)))
        ACP_ANY_RETURN("Ô´ÎÄ¼ş»òÄ¿Â¼²»´æÔÚ...");
    if(type==PASTE_COPY)
        snprintf(genbuf,STRLEN,"¸´ÖÆ·½Ê½Õ³Ìù%s %.30s, È·ÈÏ? (C-¸´ÖÆ/L-Á´½Ó/N) [N]: ",S_ISDIR(st.st_mode)?"Ä¿Â¼":"ÎÄ¼ş",filename);
    else
        snprintf(genbuf,STRLEN,"¼ôÇĞ·½Ê½Õ³Ìù%s %.38s, È·ÈÏ? (Y/N) [N]: ",S_ISDIR(st.st_mode)?"Ä¿Â¼":"ÎÄ¼ş",filename);
    ...
    if(ans[0]=='Y'||(type==PASTE_COPY&&ans[0]=='C')){
        if(!(type==PASTE_CUT&&!rename(path,newpath))){
            if(S_ISDIR(st.st_mode)){
                ...
                if(system(genbuf))
                    ACP_ANY_RETURN("¸´ÖÆÄ¿Â¼¹ı³ÌÖĞ·¢Éú´íÎó...");
            }
            else{
                if(f_cp(path,newpath,0))
                    ACP_ANY_RETURN("¸´ÖÆÎÄ¼ş¹ı³ÌÖĞ·¢Éú´íÎó...");
            }
            copy=1;
        }
    }
        ...
        if(S_ISDIR(st.st_mode)){
            if(symlink(genbuf,newpath)==-1)
                ACP_ANY_RETURN("Á´½ÓÄ¿Â¼¹ı³ÌÖĞ·¢Éú´íÎó...");
        }
        else{
            if(link(path,newpath)==-1&&symlink(genbuf,newpath)==-1)
                ACP_ANY_RETURN("Á´½ÓÎÄ¼ş¹ı³ÌÖĞ·¢Éú´íÎó...");
        }
    }
    else{
        pm->page=9999;
    ...
    if(type==PASTE_CUT){
        ...
        a_freenames(&menu);
        if(ret)
            ACP_ANY_RETURN("¼ôÇĞÎÄ¼ş¹ı³ÌÖĞ·¢Éú´íÎó...");
    }
    pm->page=9999;
...
/* etnlegend, 2006.04.29, [¾«»ªÇø] É¾³ı²Ù×÷ */
void a_delete(MENU *pm){
    ...
    if(!lstat(path,&st)&&(S_ISDIR(st.st_mode)||S_ISREG(st.st_mode)||S_ISLNK(st.st_mode))){
        sprintf(genbuf,"\033[1;37mÈ·ÈÏÉ¾³ı¸Ã%s? (Y/N) [N]: \033[m",
            (S_ISLNK(st.st_mode)?"Á´½Ó":(S_ISDIR(st.st_mode)?"Ä¿Â¼":"ÎÄ¼ş")));
        getdata(t_lines-1,0,genbuf,ans,2,DOECHO,NULL,true);
        if(toupper(ans[0])!='Y'){
            pm->page=9999;
            return;
...
void a_manager(MENU *pm,int ch)
{
    ...
    if (pm->num > 0)
        ...
        case '`':
            ...
            clrtoeol();
            if(a_savenames(pm)){
                prints("\033[1;31;47m%s\033[K\033[m","²Ù×÷¹ı³ÌÖĞ·¢Éú´íÎó, °´»Ø³µ¼ü¼ÌĞø...");
                WAIT_RETURN;
                a_loadnames(pm,getSession());
            }
            else{
                prints("\033[1;34;47m%s\033[K\033[m","²Ù×÷[Çå¿ÕÕûÀíÕß]³É¹¦, °´»Ø³µ¼ü¼ÌĞø...");
                WAIT_RETURN;
            }
            pm->page=9999;
            break;
        case '~':
            ...
            if(a_savenames(pm)){
                ...
                prints("\033[1;34;47m%s\033[K\033[m","²Ù×÷[ÉèÖÃÕûÀíÕßÎª (BM: SYSOPS)]³É¹¦, °´»Ø³µ¼ü¼ÌĞø...");
                WAIT_RETURN;
            }
            pm->page=9999;
            break;
...
void ann_get_current_url(char* buf,int buf_len,char *ext, int len,void* arg)
{
	...
	int sz;
 	...
 	if(dashd(path))
 	{
 	    ...
 	    /* Èç¹ûbbs0an.phpÖ§³ÖÊı×Ö´®·½Ê½µÄ¾«»ªÇøÂ·¾¶£¬ÔòÏÂÃæÒ»¶Î¿ÉÉ¾³ı */
