...
#undef ACP_DUMPS
    ...
    if(type==PASTE_COPY)
        snprintf(genbuf,STRLEN,"¸´ÖÆ·½Ê½Õ³Ìù%s %s, È·ÈÏ? (C-¸´ÖÆ/L-Á´½Ó/N) [N]: ",S_ISDIR(st.st_mode)?"Ä¿Â¼":"ÎÄ¼þ",filename);
    else
        snprintf(genbuf,STRLEN,"¼ôÇÐ·½Ê½Õ³Ìù%s %s, È·ÈÏ? (Y/N) [N]: ",S_ISDIR(st.st_mode)?"Ä¿Â¼":"ÎÄ¼þ",filename);
...
void a_delete(MENU *pm){
    ...
    if(stat(path,&st)==-1||!(S_ISDIR(st.st_mode)||S_ISREG(st.st_mode)))
        ...
        sprintf(genbuf,"\033[1;37mÈ·ÈÏÉ¾³ý¸Ã%s? (Y/N) [N]: \033[m",S_ISDIR(st.st_mode)?"Ä¿Â¼":"ÎÄ¼þ");
        getdata(t_lines-1,0,genbuf,ans,2,DOECHO,NULL,true);
        if(toupper(ans[0])!='Y'){
            pm->page=9999;
            return;
