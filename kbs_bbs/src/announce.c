...
void a_prompt2(int bot,char* pmt,char* buf)    /* ¾«»ªÇø×´Ì¬ÏÂ ÊäÈë ,°üº¬Ô­À´µÄÄÚÈİ*/
{
    move(t_lines + bot, 0);
    clrtoeol();
...
a_select_path_refresh(struct _select_def *conf)
{
    ...
    docmdtitle("[Ë¿Â·Ñ¡Ôñ²Ëµ¥]",
               "ÍË³ö[\x1b[1;32m¡û\x1b[0;37m,\x1b[1;32me\x1b[0;37m] ½øÈë[\x1b[1;32mEnter\x1b[0;37m] Ñ¡Ôñ[\x1b[1;32m¡ü\x1b[0;37m,\x1b[1;32m¡ı\x1b[0;37m] Ìí¼Ó[\x1b[1;32ma\x1b[0;37m] ¸ÄÃû[\x1b[1;32mT\x1b[0;37m] É¾³ı[\x1b[1;32md\x1b[0;37m]\x1b[m ÒÆ¶¯[\x1b[1;32mm\x1b[0;37m]\x1b[m");
    move(2, 0);
...
int a_SeSave(char *path, char *key, struct fileheader *fileinfo, int nomsg)
{
    ...
    if (dashf(filepath)) {
        if (!nomsg) {
            sprintf(buf, "Òª¸½¼ÓÔÚ¾ÉÔİ´æµµÖ®ááÂğ?(Y/N/C) [Y]: ");
            a_prompt(-1, buf, ans);
        }
    ...
    unlink(filepath);
    sprintf(buf, "½« %s ´æÈëÔİ´æµµ", filepath);
    a_report(buf);
    if (!nomsg) {
        sprintf(buf, " ÒÑ½«¸ÃÎÄÕÂ´æÈëÔİ´æµµ, Çë°´ÈÎºÎ¼üÒÔ¼ÌĞø << ");
        a_prompt(-1, buf, ans);
    }
    return 1;
}
...
int a_Save(char *path, char *key, struct fileheader *fileinfo, int nomsg, char *direct, int ent)
{
    ...
    sprintf(buf, "½« boards/%s/%s ´æÈëÔİ´æµµ", key, fileinfo->filename);
    ...
    int ent;
{
    ...
    if (ann_get_path(key, buf, sizeof(buf)) == 0)
	{
		...
		/* Leeward: 97.12.17: ¶Ô°æÖ÷µÄ¶à¸ö´°¿ÚÍ¬²½Ë¿Â· 
		    KCN: 2003.1.17:Ó¦¸ÃÓÃ°æÖ÷Ä¿Â¼ÏÂµÄÎÄ¼ş¡£¶ø²»ÊÇ
		    ¶à¸ö°ßÖñ¹«ÓÃ¡£
		*/
		...
		if (import_path[i][0] != '\0') {
			...
			sprintf(buf, "½«¸ÃÎÄÕÂ·Å½ø %s,È·¶¨Âğ?(Y/N) [N]: ", Importname);
			if (!nomsg)
				a_prompt(-1, buf, ans);
			if (ans[0] == 'Y' || ans[0] == 'y' || nomsg) {
				pm.path = Importname;
			} else {
				sprintf(buf, "Äã¸Ä±äĞÄÒâÁË?? ,Çë°´ÈÎºÎ¼üÒÔ½áÊø << ");
				a_prompt(-1, buf, ans);
				return 1;
			}
		}
		...
		if (a_savenames(&pm)==0) {
			...
			/* Leeward 98.04.15 */
			sprintf(buf, "½« boards/%s/%s ÊÕÈëÄ¿Â¼ %s", key, fileinfo->filename, pm.path + 17);
			a_report(buf);
			sprintf(buf, " ÊÕÈë¾«»ªÇøÄ¿Â¼ %s, Çë°´ Enter ¼ÌĞø << ", /*fileinfo->title, */ pm.path);
			if (!nomsg)
				a_prompt(-1, buf, ans);
    ...
    int mode;
{
    ...
    switch (mode) {
    case ADDITEM:
        mesg = "ÇëÊäÈëĞÂÎÄ¼şÖ®Ó¢ÃûÃû³Æ(¿Éº¬Êı×Ö)£º";
        break;
    case ADDGROUP:
        mesg = "ÇëÊäÈëĞÂÄ¿Â¼Ö®Ó¢ÃûÃû³Æ(¿Éº¬Êı×Ö)£º";
        break;
    case ADDMAIL:
        sprintf(board, "tmp/bm.%s", currentuser->userid);
        if (!dashf(board)) {
