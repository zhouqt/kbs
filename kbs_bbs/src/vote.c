...
char *vote_type[] = { "ÊÇ·Ç", "µ¥Ñ¡", "¸´Ñ¡", "Êý×Ö", "ÎÊ´ð"};
struct votebal currvote;
struct votelimit currlimit;/*Haohmaru.99.11.17.¸ù¾Ý°åÖ÷ÉèµÄÏÞÖÆÌõ¼þÅÐ¶ÏÊÇ·ñÈÃ¸ÃÊ¹ÓÃÕßÍ¶Æ±*/
...
b_notes_edit()
{
    ...
    getdata(1,0,"(E)±à¼­ (D)É¾³ý ±¾ÌÖÂÛÇøµÄ±¸ÍüÂ¼? [E]: ",ans,2,DOECHO,NULL,YEA);
    if (ans[0] == 'D' || ans[0] == 'd')
    {
        move(2,0);
        if(askyn("ÕæµÄÒªÉ¾³ý±¾ÌÖÂÛÇøµÄ±¸ÍüÂ¼",0))
        {
            move(3,0);
            prints("±¸ÍüÂ¼ÒÑ¾­É¾³ý...\n");
            pressanykey();
...
get_result_title()
{
    ...
    if(currlimit.numlogins || currlimit.numposts || currlimit.stay || currlimit.
            day){
        fprintf( sug, "¡Ñ ´Ë´ÎÍ¶Æ±µÄÔÊÐí×Ê¸ñÎª:\n");
        fprintf( sug, "1. ÉÏÕ¾´ÎÊýÐè´óÓÚ %d ´Î .\n",currlimit.numlogins);
        fprintf( sug, "2. ÎÄÕÂÊýÄ¿Ðè´óÓÚ %d Æª.\n",currlimit.numposts);
        fprintf( sug, "3. ÉÏÕ¾×ÜÊ±ÊýÐè´óÓÚ %d Ð¡Ê±.\n",currlimit.stay);
        fprintf( sug, "4. ÉÏÕ¾ÎïÀíÊ±¼äÐè´óÓÚ %d Ìì.\n",currlimit.day);
    }
    fprintf( sug, "¡Ñ Í¶Æ±¿ªÆôì¶£º%.24s  Àà±ð£º%s\n", ctime( &currvote.opendate )
...
int num;
{
    ...
    if(currvote.type==VOTE_VALUE)
    {
        ...
        for(i=0;i<10;i++)
        {
            fprintf(sug, "[1m  %4d[m µ½ [1m%4d[m Ö®¼äÓÐ [1m%4d[m Æ±  Ô¼Õ¼ [1m%d%%[m\n",
    ...
    if (strcmp(currboard,"sys_discuss"))/*Èç¹ûÊÇsys_discuss°æ,Í¶Æ±½á¹û²»½øvote°æ,Haohmaru,98.9.4*/
    {
...
char    *bname;
{
    ...
    for(;;)
    {
        getdata(2,0,"(1)ÊÇ·Ç, (2)µ¥Ñ¡, (3)¸´Ñ¡, (4)ÊýÖµ (5)ÎÊ´ð (6)È¡Ïû ? : "
    ...
    clear();
    getdata( 0,0,"´Ë´ÎÍ¶Æ±ËùÐëÌìÊý (²»¿É£°Ìì): ",buf, 4, DOECHO, NULL ,YEA);
    ...
    case VOTE_VALUE:
        for(;;) {
            getdata(3,0,"ÊäÈëÊýÖµ×î´ó²»µÃ³¬¹ý [100] : ",buf,7,DOECHO,NULL,YEA);
            ball->maxtkt = atoi(buf);
        ...
        break;
    case VOTE_ASKING:
        /*                    getdata(3,0,"´ËÎÊ´ðÌâ×÷´ðÐÐÊýÖ®ÏÞÖÆ :",buf,3,DOECHO,NULL,YEA) ;
    ...
    {
        getdata(1,0,"ÊÇ·ñ¶ÔÍ¶Æ±×Ê¸ñ½øÐÐÏÞÖÆ(Y/N) [Y]:",buf,3,DOECHO,NULL,YEA) ;
        if ( buf[0] != 'N' && buf[0] != 'n' )
        {
            getdata(2,0,"ÇëÊäÈë¶ÔÉÏÕ¾´ÎÊýµÄÏÞÖÆ(0ÎªÃ»ÓÐÏÞÖÆ),ÉÏÕ¾´ÎÊý´óÓÚ:",buf,5,DOECHO,NULL,YEA) ;
            v_limit->numlogins = atoi(buf);
            getdata(3,0,"ÇëÊäÈë¶ÔÎÄÕÂÊýÄ¿µÄÏÞÖÆ(0ÎªÃ»ÓÐÏÞÖÆ),ÎÄÕÂÊýÄ¿´óÓÚ:",buf,5,DOECHO,NULL,YEA) ;
            v_limit->numposts = atoi(buf);
            getdata(4,0,"ÇëÊäÈë¶ÔÉÏÕ¾×ÜÊ±ÊýµÄÏÞÖÆ(0ÎªÃ»ÓÐÏÞÖÆ),ÉÏÕ¾×ÜÊ±Êý´óÓÚ(Ð¡Ê±):",buf,5,DOECHO,NULL,YEA) ;
            v_limit->stay = atoi(buf);
            getdata(5,0,"ÇëÊäÈë¶ÔÉÏÕ¾ÎïÀíÊ±¼äµÄÏÞÖÆ(0ÎªÃ»ÓÐÏÞÖÆ),ÉÏÕ¾ÎïÀíÊ±¼ä´óÓÚ(Ìì):",buf,5,DOECHO,NULL,YEA) ;
    ...
    else
    {
        ...
        if( (sug=fopen(votename,"w")) !=NULL)
        {
            ...
            fclose(sug);
            sug = NULL;
            if(!strcmp(currboard,"sys_discuss"))/*sys_discussµÄÍ¶Æ±²»½øvote°æ,Haohmaru,98.9.4*/
            {
...
show_voteing_title()
{
    ...
    if(currvote.type!=VOTE_VALUE&&currvote.type!=VOTE_ASKING)
        sprintf(buf,"¿ÉÍ¶Æ±Êý: %d Æ±",currvote.maxtkt);
    else
        buf[0]='\0';
    closedate=currvote.opendate+currvote.maxdays*86400;
    prints( "Í¶Æ±½«½áÊøì¶: %24s  %s  %s\n",
            ctime(&closedate),buf,(voted_flag)?"(ÐÞ¸ÄÇ°´ÎÍ¶Æ±)":"");
...
int num;
{
    ...
    /*Haohmaru.99.11.17.¸ù¾Ý°åÖ÷ÉèµÄÏÞÖÆÌõ¼þÅÐ¶ÏÊÇ·ñÈÃ¸ÃÊ¹ÓÃÕßÍ¶Æ±*/
    clear();
    ...
    {
        prints("¶Ô²»Æð,Äã²»Âú×ã°åÖ÷¹æ¶¨µÄ´Ë´ÎÍ¶Æ±ËùÐèÌõ¼þ,ÎÞ·¨²Î¼ÓÍ¶Æ±,Ð»Ð»²ÎÓë,ÏÂ´ÎÔÙ¼û! :)");
        pressanykey();
        return ;
    }
...
vote_title()
{
    ...
    docmdtitle("[Í¶Æ±ÏäÁÐ±í]",
               "[¡û,e] Àë¿ª [h] ÇóÖú [¡ú,r <cr>] ½øÐÐÍ¶Æ± [¡ü,¡ý] ÉÏ,ÏÂÑ¡Ôñ [1m¸ßÁÁ¶È[m±íÊ¾ÉÐÎ´Í¶Æ±");
    update_endline();
}
...
case 'O': case 'o':
        ...
        ans = askyn("ÄãÈ·¶¨ÒªÌáÔç½áÊøÕâ¸öÍ¶Æ±Âð",0);
        if (ans != 1)
        {
            move(2, 0);
            prints("È¡ÏûÉ¾³ýÐÐ¶¯\n");
...
case 'D': case 'd':
        ...
        prints("[31m¾¯¸æ!![m\n");
        prints("Í¶Æ±Ïä±êÌâ£º[1m%s[m\n", currvote.title);
        ans = askyn("ÄãÈ·¶¨ÒªÇ¿ÖÆ¹Ø±ÕÕâ¸öÍ¶Æ±Âð",0);
