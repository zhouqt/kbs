...
int user_vote(int num)
{
    ...
    if ((currentuser->numposts < userlimit.numposts
        ...
        prints
            ("¶Ô²»Æğ,Äã²»Âú×ã°åÖ÷¹æ¶¨µÄ´Ë´ÎÍ¶Æ±ËùĞèÌõ¼ş,ÎŞ·¨²Î¼ÓÍ¶Æ±,Ğ»Ğ»²ÎÓë,ÏÂ´ÎÔÙ¼û! :)");
        pressanykey();
        return -1;
    }
    clrtobot();
...
int allnum, pagenum;
{
    ...
    case 'o':
        ...
        if (!strcmp(currboard, "Birthday"))
            /*Haohmaru.99.3.29.Ó¦ÉúÈÕ°åÇ°°åÖ÷µÄÒªÇó¶øÉè,½áÊøÓÚDec 28 20:52:29 2000 */
            break;
        clear();
        deal = 1;
    ...
    case 'd':
        ...
        if (!strcmp(currboard, "Birthday"))
            /*Haohmaru.99.3.29.½áÊøÓÚDec 28 20:52:29 2000 */
            break;
        deal = 1;
        get_record(controlfile, &currvote, sizeof(struct votebal),
                   allnum + 1);
