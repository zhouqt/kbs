...
int change_mode(struct _select_def* conf,struct fileheader *fileinfo,int newmode)
{
    ...
    if (newmode==0) {
        move(t_lines - 2, 0);
        clrtoeol();
        prints("ÇĞ»»Ä£Ê½µ½: 0)È¡Ïû 1)ÎÄÕª 2)Í¬Ö÷Ìâ 3)±»mÎÄÕÂ 4)Ô­×÷ 5)Í¬×÷Õß 6)±êÌâ¹Ø¼ü×Ö ");
        move(t_lines - 1, 0);
        clrtoeol();
...
#define ANONY_KEYS "M"
    ...
    while (1) {                 /* ·¢±íÇ°ĞŞ¸Ä²ÎÊı£¬ ¿ÉÒÔ¿¼ÂÇÌí¼Ó'ÏÔÊ¾Ç©Ãûµµ' */
        ...
        sprintf(buf2, "%s£¬\033[1;32mb\033[m»Ø¸´µ½ĞÅÏä£¬\033[1;32mT\033[m¸Ä±êÌâ£¬%s%s%s\033[1;32mEnter\033[m¼ÌĞø: ", 
				...
				(currboard->flag&BOARD_ATTACH)?"\033[1;32mu\033[m´«¸½¼ş, ":"",
#ifdef POST_QUIT
...
int del_ding(struct _select_def* conf,struct fileheader *fileinfo,void* extraarg)
{
    ...
    a_prompt(-1, "É¾³ıÖÃ¶¥£¬È·ÈÏÂğ£¿(Y/N) [N] ", buf);
                ...
                switch(toupper(key=igetkey())){
                    ...
                    case ' ':
                        if(read_top(index,0)<0){
                            move(t_lines-1,4);
                            clrtoeol();
                            prints("\033[1;31m%s\033[0;33m<Enter>\033[m","»ñÈ¡Ê®´óĞÅÏ¢Ê±·¢Éú´íÎó!");
                            WAIT_RETURN;
                        }
                        update=1;
                        break;
