...
{                               /* ×ªÌù Ò»Æª ÎÄÕÂ */
	...
	digestmode=0;
        if (post_cross(currentuser, bname, currboard, quote_title, q_file, Anony, in_mail, ispost[0], 0) == -1) { /* ×ªÌù */
            pressreturn();
            move(2, 0);
            return FULLUPDATE;
        }
...
int read_hot_info(int ent, struct fileheader *fileinfo, char *direct)
{
    ...
    move(t_lines - 1, 0);
    clrtoeol();
    getdata(t_lines - 1, 0, "Ñ¡ÔñÔÄ¶Á: 1)±¾ÈÕÊ®´óĞÂÎÅ 2)±¾ÈÕÊ®´ó×£¸£ 3)½üÆÚÈÈµã [1]: ", ans, 3, DOECHO, NULL, true);
    switch (ans[0])
	{
    case '2':
...
{                               /*ÓÃ»§ POST ÎÄÕÂ */
    ...
    strcpy(quote_board, currboard);
    aborted = vedit(filepath, true, &eff_size);    /* ½øÈë±à¼­×´Ì¬ */
    ...
    while ((fgets(buf, 256, fp)) != NULL) {
        if (mode == 1) {
            ...
            if (Origin2(buf)) {
                now = time(0);
                if(uinfo.mode == RMAIL)
                    fprintf(out, "[36m¡ù ĞŞ¸Ä:¡¤%s ÓÚ %15.15s ĞŞ¸Ä±¾ĞÅ¡¤[FROM: %15.15s][m\n", currentuser->userid, ctime(&now) + 4, fromhost);
                else
                    fprintf(out, "[36m¡ù ĞŞ¸Ä:¡¤%s ÓÚ %15.15s ĞŞ¸Ä±¾ÎÄ¡¤[FROM: %15.15s][m\n", currentuser->userid, ctime(&now) + 4, fromhost);
                step = 3;
		added = 1;
            }
            fputs(buf, out);
         ...
         */
{
    ...
    getdata(t_lines - 1, 0, "ĞÂÎÄÕÂ±êÌâ: ", buf, 50, DOECHO, NULL, false);      /*ÊäÈë±êÌâ */
    if (buf[0] != '\0') {
        char tmp[STRLEN * 2], *t;
 ...
 */
    ...
    if ( /*strcmp(currentuser->username,"guest")&& */ stay <= Time) {
        ...
        if (fp) {
            fputs(lbuf, fp);
            fclose(fp);
            mail_file(currentuser->userid, tmpfile, "surr", "×ÔÊ×", BBSPOST_MOVE);
        }
    }
    /*
