...
char *filemargin();
#define ARG_NOPROMPT_FLAG   1 /*²Ù×÷²»ÌáÊ¾*/
#define ARG_DELDECPOST_FLAG 2 /*É¾³ý²Ù×÷Òª¼õÎÄÕÂÊý*/
#define ARG_BMFUNC_FLAG 4 /*°æÖ÷²Ù×÷±êÖ¾*/
...
{                               /* ×ªÌù Ò»Æª ÎÄÕÂ */
    ...
    {                           /* Leeward 98.01.13 ¼ì²é×ªÌùÕßÔÚÆäÓû×ªµ½µÄ°æÃæÊÇ·ñ±»½ûÖ¹ÁË POST È¨ */
        ...
        if (deny_me(currentuser->userid, bname) ) {    /* °æÖ÷½ûÖ¹POST ¼ì²é */
			if( HAS_PERM(currentuser, PERM_SYSOP) ){
				char buf[3];

				clear();
            	getdata(3, 0, "Äú±»°æÖ÷Í£Ö¹ÁËPOSTÈ¨Á¦,Ç¿ÖÆ×ªÔØÂð£¿[y/N]: ", buf, 2, DOECHO, NULL, true);
				if( buf[0]!='y' && buf[0]!='Y' ){
					clear();
					return FULLUPDATE;
				}
...
//	sprintf(TITLE,"%s(%d)",ent->title,ent->eff_size);
    ...
    if (uinfo.mode != RMAIL && arg->mode != DIR_MODE_DIGEST && arg->mode != DIR_MODE_DELETED && arg->mode != DIR_MODE_JUNK
        && strcmp(currboard->filename, "sysmail")) { /* ÐÂ·½·¨±È½Ï*/
            if ((ent->groupid != ent->id)&&(arg->mode==DIR_MODE_THREAD||!strncasecmp(TITLE,"Re:",3)||!strncmp(TITLE,"»Ø¸´:",5))) {      /*ReµÄÎÄÕÂ */
                if ((readfh&&readfh->groupid == ent->groupid))     /* µ±Ç°ÔÄ¶ÁÖ÷Ìâ ±êÊ¶ */
                    if (DEFINE(currentuser, DEF_HIGHCOLOR))
                        sprintf(buf, " [1;36m%4d[m %s%c%s %-12.12s %s[1;36m.%c%-44.44s[m ", num, typeprefix, type, typesufix, ent->owner, date, attachch, TITLE);
                    else
                        sprintf(buf, " [36m%4d[m %s%c%s %-12.12s %s[36m.%c%-44.44s[m ", num, typeprefix, type, typesufix, ent->owner, date, attachch, TITLE);
                else
        ...
        if (!strncmp("Re:", ent->title, 3)) {   /*ReµÄÎÄÕÂ */
            if (readfh&&isThreadTitle(readfh->title, ent->title)) /* µ±Ç°ÔÄ¶ÁÖ÷Ìâ ±êÊ¶ */
                if (DEFINE(currentuser, DEF_HIGHCOLOR))
                    sprintf(buf, " [1;36m%4d[m %s%c%s %-12.12s %s[1;36m.%c%-44.44s[m ", num, typeprefix, type, typesufix, ent->owner, date, attachch, TITLE);
                else
                    sprintf(buf, " [36m%4d[m %s%c%s %-12.12s %s[36m.%c%-44.44s[m ", num, typeprefix, type, typesufix, ent->owner, date, attachch, TITLE);
...
int read_hot_info(struct _select_def* conf,struct fileheader *fileinfo,void* extraarg)
{
    ...
    move(t_lines - 1, 0);
    clrtoeol();
    getdata(t_lines - 1, 0, "Ñ¡ÔñÔÄ¶Á: 1)±¾ÈÕÊ®´óÐÂÎÅ 2)×£¸£¡¤Ð£ÄÚ 3)½üÆÚÈÈµã 4)ÏµÍ³ÈÈµã 5)ÈÕÀúÈÕ¼Ç[1]: ", ans, 3, DOECHO, NULL, true);
    switch (ans[0])
	{
    case '2':
...
//	char tmplate[STRLEN];
    ...
    if (!haspostperm(currentuser, currboard->filename)) { /* POSTÈ¨ÏÞ¼ì²é */
        ...
        clear();
        return FULLUPDATE;
    } else if (deny_me(currentuser->userid, currboard->filename)) { /* °æÖ÷½ûÖ¹POST ¼ì²é */
		if( !HAS_PERM(currentuser, PERM_SYSOP) ){
        	move(3, 0);
        	clrtobot();
			...
			clear();
            getdata(3, 0, "Äú±»°æÖ÷Í£Ö¹ÁËPOSTÈ¨Á¦,Ç¿ÖÆ·¢ÎÄÂð£¿[y/N]: ", buf, 2, DOECHO, NULL, true);
			if( buf[0]!='y' && buf[0]!='Y' ){
				clear();
				return FULLUPDATE;
			}
         ...
         */
{
    ...
    if ((arg->mode>= DIR_MODE_THREAD) && (arg->mode<= DIR_MODE_WEB_THREAD)) /*·ÇÔ´direct²»ÄÜÐÞ¸Ä*/
        return DONOTHING;
    if (true == check_readonly(currboard->filename))      /* Leeward 98.03.28 */
        return FULLUPDATE;
...
int set_article_flag(struct _select_def* conf,struct fileheader *fileinfo,int flag)
{
    ...
    static const struct actionlist flaglist[]= {
            ...
            {FILE_DELETE_FLAG,1,FILE_DEL,"±ê¼Ç#"},
...
#ifdef COMMEND_ARTICLE
	if( mode & 0x1 )
    	getdata(t_lines - 1, 0, "ÇÐ»»: 0)È¡Ïû 1)²»¿Ére±ê¼Ç 2)ÖÃ¶¥±ê¼Ç 3)ÍÆ¼ö [1]: ", ans, 3, DOECHO, NULL, true);
	else
    	getdata(t_lines - 1, 0, "3) ÍÆ¼ö [0]: ", ans, 3, DOECHO, NULL, true);
#else
...
/*³ýÁËÕý³£Ä£Ê½,Ö÷ÌåÄ£Ê½,ÓÊ¼þÄ£Ê½,¶¼²»Ó¦¸ÃÄÜÇø¶Î*/
...
static struct key_command read_comms[] = { /*ÔÄ¶Á×´Ì¬£¬¼ü¶¨Òå */
    ...
    {'H', (READ_KEY_FUNC)read_hot_info,NULL},   /* flyriver: 2002.12.21 Ôö¼ÓÈÈÃÅÐÅÏ¢ÏÔÊ¾ */
