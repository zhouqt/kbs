...
{                               /* ÔÚÎÄÕÂÁÐ±íÖÐ ÏÔÊ¾ Ò»ÆªÎÄÕÂ±êÌâ */
    ...
    if ((ent->accessed[0] & FILE_DIGEST) /*&& HAS_PERM(currentuser,PERM_MARKPOST) */ ) {        /* ÎÄÕªÄ£Ê½ ÅÐ¶Ï */
        if (type == ' ')
            type = 'g';
        else
            type = 'G';
...
*/
    ...
    move(t_lines - 1, 0);
    clrtoeol();                 /* ÇåÆÁµ½ÐÐÎ² */
    if (haspostperm(currentuser, currboard)) {  /* ¸ù¾ÝÊÇ·ñÓÐPOSTÈ¨ ÏÔÊ¾×îÏÂÒ»ÐÐ */
...
{                               /*ÓÃ»§ POST ÎÄÕÂ */
    ...
    if (!haspostperm(currentuser, currboard)) { /* POSTÈ¨ÏÞ¼ì²é */
        move(3, 0);
        clrtobot();
        if (digestmode == false) {
    ...
    while (1) {                 /* ·¢±íÇ°ÐÞ¸Ä²ÎÊý£¬ ¿ÉÒÔ¿¼ÂÇÌí¼Ó'ÏÔÊ¾Ç©Ãûµµ' */
        ...
        sprintf(buf2, "°´[1;32m0[m~[1;32m%d/V/L[mÑ¡/¿´/Ëæ»úÇ©Ãûµµ%s£¬[1;32mT[m¸Ä±êÌâ£¬%s[1;32mEnter[m½ÓÊÜËùÓÐÉè¶¨: ", numofsig,
                (replymode) ? "£¬[1;32mS/Y[m/[1;32mN[m/[1;32mR[m/[1;32mA[m ¸ÄÒýÑÔÄ£Ê½" : "", (anonyboard) ? "[1;32mM[m ÄäÃû£¬" : "");
    ...
    add_loginfo(filepath, currentuser, currboard, Anony);       /*Ìí¼Ó×îºóÒ»ÐÐ */
  ...
  THERE:
    ...
    if (*num1 == 'Y' || *num1 == 'y') {
        ...
        if (result) {           /* prints("´íÎó´úÂë: %d;%s Çë±¨¸æÕ¾³¤£¬Ð»Ð»£¡", result,direct);
            ...
            getdata(8, 0, "Çø¶ÎÉ¾³ý´íÎó,Èç¹ûÏëÐÞ¸´,ÇëÈ·¶¨[35mÎÞÈËÔÚ±¾°æÖ´ÐÐÇø¶ÎÉ¾³ý²Ù×÷²¢°´'Y'[0m (Y/N)? [N]: ", num1, 10, DOECHO, NULL, true);
            if (*num1 == 'Y' || *num1 == 'y') {
                if (!mailmode) {
 ...
 */
    ...
    if (num_user_logins(currentuser->userid) == 0 || !strcmp(currentuser->userid, "guest")) {   /*¼ì²é»¹ÓÐÃ»ÓÐÈËÔÚÏßÉÏ */
        ...
        if (DEFINE(currentuser, DEF_MAILMSG /*ÀëÕ¾Ê±¼Ä»ØËùÓÐÐÅÏ¢ */ ) && dashf(fname)) {
            char title[STRLEN];
            time_t now;
