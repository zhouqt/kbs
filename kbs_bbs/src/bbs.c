...
//prompt...
    		...
    		clrtoeol();
    		move(t_lines -1, 0);
			prints("²Ù×÷Ê§°Ü, ÎÄÕªÇøÒÑÂú£¬ÈÎÒâ¼ü¼ÌĞø");
			pressanykey();
			ret = FULLUPDATE;
        } else {
            char buf[STRLEN];
...
{                               /* ÔÚÎÄÕÂÁĞ±íÖĞ ÏÔÊ¾ Ò»ÆªÎÄÕÂ±êÌâ */
   ...
   typeprefix[0]='\0';
    ...
    if (uinfo.mode != RMAIL && arg->mode != DIR_MODE_DIGEST && arg->mode != DIR_MODE_DELETED && arg->mode != DIR_MODE_JUNK
        ...
        if ((ent->groupid != ent->id)&&(arg->mode==DIR_MODE_THREAD||!strncasecmp(ent->title,"Re:",3)||!strncmp(ent->title,"»Ø¸´:",5))) {
			isreply=1;
		}
    ...
    if(!mode){
        ...
        switch(ans[0]){
            ...
            case '5':
                ...
                clrtoeol();
                strcpy(buf,fh->owner);
                getdata(t_lines-1,0,"ÄúÏ£Íû²éÕÒÄÄÎ»ÓÃ»§µÄÎÄÕÂ: ",buf,IDLEN+2,DOECHO,NULL,false);
                if(!buf[0])
                    return FULLUPDATE;
                break;
            case '6':
                ...
                clrtoeol();
                strcpy(buf,title);
                getdata(t_lines-1,0,"ÄúÏ£Íû²éÕÒµÄ±êÌâ¹Ø¼ü×Ö: ",buf,32,DOECHO,NULL,false);
                if(!buf[0])
                    return FULLUPDATE;
                strcpy(title,buf);
                break;
            ...
            case '8':
                ...
                clrtoeol();
                strcpy(buf,title);
                getdata(t_lines-1,0,"ÄúÏ£Íû²éÕÒµÄÈ«ÎÄ¹Ø¼ü×Ö: ",buf,64,DOECHO,NULL,false);
                if(!buf[0])
                    return FULLUPDATE;
                    strcpy(title,buf);
                strcpy(title,buf);
...
//	char tmplate[STRLEN];
    ...
    if(!((cmdmode==DIR_MODE_MARK)||(cmdmode==DIR_MODE_THREAD)||(cmdmode==DIR_MODE_NORMAL)||(cmdmode==DIR_MODE_TOP10))){
        move(3,0);
        clrtobot();
        prints("\n\n\t%s","\033[1;37mµ±Ç°Ä£Ê½ÎªÎÄÕª, Ö÷Ìâ»òËÑË÷Ä£Ê½, ÎŞ·¨·¢±íÎÄÕÂ,\n"
            "\t°´»Ø³µ¼üºóÊ¹ÓÃ \033[1;32m<q>\033[1;37m ¼ü¿É·µ»ØÎÄÕÂÁĞ±í¶ÔÓ¦µÄÎ»ÖÃ...\033[m");
        move(t_lines-1,0);
        prints("\033[1;34;47m\t%s\033[K\033[m","°´»Ø³µ¼ü¼ÌĞø...");
        WAIT_RETURN;
        clear();
        return FULLUPDATE;
    }
