...
char *
sethomefile( buf, userid, filename )  /*È¡Ä³ÓÃ»§ÎÄ¼þ Â·¾¶*/
...
char *
setuserfile( buf, filename )    /* È¡µ±Ç°ÓÃ»§ÎÄ¼þ Â·¾¶*/
char    *buf, *filename;
{
    if (isalpha(currentuser->userid[0]))  /* ¼ÓÈë´íÎóÅÐ¶Ï,Ìá¸ßÈÝ´íÐÔ, alex 1997.1.6*/
...
char *
setmailfile( buf, userid, filename )    /* È¡Ä³ÓÃ»§mailÎÄ¼þ Â·¾¶*/
char    *buf, *userid, *filename;
{
    if (isalpha(userid[0]))  /* ¼ÓÈë´íÎóÅÐ¶Ï,Ìá¸ßÈÝ´íÐÔ, alex 1997.1.6*/
...
char *
setbpath( buf, boardname )   /* È¡Ä³°æ Â·¾¶ */
char *buf, *boardname;
{
    strcpy( buf, "boards/" );
...
char *
setbfile( buf, boardname, filename )  /* È¡Ä³°æÏÂÎÄ¼þ */
...
int
uleveltochar( buf, lvl ) /* È¡ÓÃ»§È¨ÏÞ×Ö·û´®,ÈçCVBS */
char    *buf;
unsigned lvl;
{
    if( !(lvl &  PERM_BASIC) ) {
