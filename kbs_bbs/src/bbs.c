...
char CurArticleFileName[STRLEN]; /* ±£´æµ±Ç°ÎÄÕÂµÄÎÄ¼şÃû£¬ÓÃÓÚclear_new_flag, added by dong, 1999.1.21 */
...
{ /* Leeward 98.06.16 For AIX only. Please man psdanger */
    ...
    if ( safe < free - safe ) {
        ...
        clrtobot();
        move(8, 0);
        prints("[1m[33mºÜ±§Ç¸£º[31mÄ¿Ç° RAM ±»¹ı¶ÈÊ¹ÓÃ, ÇëÉÔºòÔÙ·¢±í»òÕßĞŞ¸ÄÎÄÕÂ[33m\n\nRAM µ±Ç°¿ÕÏĞÒ³Êı¸ß³ö¾¯½äãĞÖµ %d (¾¯½äãĞÖµ = %d)[0m\n", safe, free - safe);
        pressreturn();
        clear();
...
int
chk_currBM(BMstr)   /* ¸ù¾İÊäÈëµÄ°æÖ÷Ãûµ¥ ÅĞ¶Ïµ±Ç°userÊÇ·ñÊÇ°æÖ÷ */
char BMstr[STRLEN-1];
{
    char *ptr;
    char BMstrbuf[STRLEN-1];
...
int
chk_currBM1(BMstr)   /* Bigman:2001.2.19 ¸ù¾İÊäÈëµÄ°æÖ÷Ãûµ¥ ÅĞ¶Ïµ±Ç°userÊÇ·ñÊÇ°æÖ÷ */
char BMstr[STRLEN-1];
{
    char *ptr;
    char BMstrbuf[STRLEN-1];
...
char *
sethomefile( buf, userid, filename )  /*È¡Ä³ÓÃ»§ÎÄ¼ş Â·¾¶*/
...
char *
setuserfile( buf, filename )    /* È¡µ±Ç°ÓÃ»§ÎÄ¼ş Â·¾¶*/
char    *buf, *filename;
{
    if (isalpha(currentuser.userid[0]))  /* ¼ÓÈë´íÎóÅĞ¶Ï,Ìá¸ßÈİ´íĞÔ, alex 1997.1.6*/
...
char *
setmailfile( buf, userid, filename )    /* È¡Ä³ÓÃ»§mailÎÄ¼ş Â·¾¶*/
char    *buf, *userid, *filename;
{
    if (isalpha(userid[0]))  /* ¼ÓÈë´íÎóÅĞ¶Ï,Ìá¸ßÈİ´íĞÔ, alex 1997.1.6*/
...
char *
setbdir( buf, boardname )  /* ¸ù¾İÔÄ¶ÁÄ£Ê½ È¡Ä³°æ Ä¿Â¼Â·¾¶ */
char *buf, *boardname;
{
    char dir[STRLEN];
...
char *
setbfile( buf, boardname, filename )  /* È¡Ä³°æÏÂÎÄ¼ş */
...
int
uleveltochar( buf, lvl ) /* È¡ÓÃ»§È¨ÏŞ×Ö·û´®,ÈçCVBS */
char    *buf;
unsigned lvl;
{
    if( !(lvl &  PERM_BASIC) ) {
    ...
    if( lvl & PERM_SYSOP ) strcpy(buf,"Õ¾Îñ");
...
int
junkboard()  /* ÅĞ¶Ïµ±Ç°°æÊÇ·ñÎª junkboards */
{
...
char *
readdoent(num,ent)  /* ÔÚÎÄÕÂÁĞ±íÖĞ ÏÔÊ¾ Ò»ÆªÎÄÕÂ±êÌâ */
int     num;
struct fileheader *ent ;
{
    ...
    */
    ...
    /******* ÒÔÏÂÎª×ª»¯Ô­À´ÔÄ¶Á¼ÆÊıµÄ³ÌĞò Luzi 1999/1/13 *****/
    /**** Ò»ÖÜºó¿ÉÒÔÉ¾³ı :) ****/
    long        ldReadCount=0;
    ...
    if((HAS_PERM(PERM_OBOARDS)||(chk_currBM(currBM))) && ent->accessed[1] & FILE_DEL) /* Èç¹ûÎÄ¼ş±»mark delete×¡ÁË£¬ÏÔÊ¾X*/
    {
        type = 'X';
    }
...
int
cmpfilename(fhdr)  /* ±È½Ï Ä³ÎÄ¼şÃûÊÇ·ñºÍ µ±Ç°ÎÄ¼ş ÏàÍ¬ */
...
int
cmpname(fhdr,name)  /* Haohmaru.99.3.30.±È½Ï Ä³ÎÄ¼şÃûÊÇ·ñºÍ µ±Ç°ÎÄ¼ş ÏàÍ¬ */
struct fileheader *fhdr ;
char  name[STRLEN];
{
...
int mode;
{
    ...
    setbfile( filepath, currboard, fname ); /* µÃµ½ Ä¿±êPOSTÎÄ¼şÃû */

    /*  ip = strrchr(fname,'A') ;
        while((fp = open(filepath,O_CREAT|O_EXCL|O_WRONLY,0644)) == -1) {
            if(*ip == 'Z')                            POSTÎÄ¼şÃû½áÎ²ÓÉ A->Z±ä»¯£¬À´±ÜÃâÖØ¸´ 
                ip++,*ip = 'A', *(ip + 1) = '\0' ;
            else
                (*ip)++ ;
    ...
    if(mode != 1)
{
    ...
    /*   setbdir( buf, currboard );Haohmaru.99.11.26.¸Ä³ÉÏÂÃæÒ»ĞĞ£¬ÒòÎª²»¹ÜÊÇ×ªÌù»¹ÊÇ×Ô¶¯·¢ÎÄ¶¼²»»áÓÃµ½ÎÄÕªÄ£Ê½*/
...
post_article()                         /*ÓÃ»§ POST ÎÄÕÂ */
{
    ...
    substitute_record(PASSFILE, &currentuser, sizeof(currentuser), usernum); /* ¸üĞÂÓÃ»§ĞÅÏ¢ */
    return FULLUPDATE ;
}
...
int
del_range(ent,fileinfo,direct,mailmode)   /* ÇøÓòÉ¾³ı */
int ent ;
struct fileheader *fileinfo ;
char *direct ;
{
    ...
    getdata(1,0,"É¾³ıÄ£Ê½ [0]±ê¼ÇÉ¾³ı [1]Ç¿ÖÆÉ¾³ı(±»markµÄÎÄÕÂÒ»ÆğÉ¾) [2]ÆÕÍ¨É¾³ı (0): ",del_mode,10,DOECHO,NULL,YEA) ;
    ...
    if( !fail ) {
        ...
        if ((YEA != digestmode) /* ²»¿ÉÒÔÓÃ ¡°NA ==¡± ÅĞ¶Ï£ºdigestmode ÈıÖµ */
                &&!((fileinfo->accessed[0]&FILE_MARKED)
                    &&(fileinfo->accessed[1]& FILE_READ)
                    &&(fileinfo->accessed[0]& FILE_FORWARDED)))
        { /* Leeward 98.06.17 ÔÚÎÄÕªÇøÉ¾ÎÄ²»¼õÎÄÕÂÊıÄ¿ */
            if (owned)
            {
                set_safe_record();
                if ((int)currentuser.numposts > 0 && !junkboard())
                {
                    currentuser.numposts--;/*×Ô¼ºÉ¾³ıµÄÎÄÕÂ£¬¼õÉÙpostÊı*/
                    substitute_record( PASSFILE, &currentuser, sizeof(currentuser), usernum);
                }
            } else if ( !strstr(usrid,".")&&BMDEL_DECREASE&&!B_to_b/*°æÖ÷É¾³ı,¼õÉÙPOSTÊı*/){
...
{
    readpost=0;
    strcpy(CurArticleFileName, fileinfo->filename); /* ±£´æµ±Ç°ÎÄÕÂµÄÎÄ¼şÃû£¬ added by dong, 1999.1.20 */
...
Goodbye()    /*ÀëÕ¾ Ñ¡µ¥*/
{
    ...
    clear() ;
    move(0,0);
    prints("Äã¾ÍÒªÀë¿ª %s £¬¸øÕ¾³¤Ò»Ğ©½¨ÒéÂğ£¿\n",BoardName);
    if(strcmp(currentuser.userid,"guest")!=0)
        prints("[[33m1[m] ¼ÄĞÅ¸øÕ¾³¤ÃÇ\n");
...
void
board_usage( mode, usetime ) /*ÔÚuse_boardÖĞ¼ÇÂ¼boardÊ¹ÓÃÇé¿ö*/
char *mode;
time_t usetime;
{
    time_t      now;
...
int     owned;
{
    ...
    if( (fin = fopen( genbuf, "r" )) != NULL ) {
        brd = owned ? "junk" : "deleted"; /*ÅĞ¶ÏÉ¾³ıºó µ½junk»òdeleted°æ */
        ...
        postfile.filename[ STRLEN - 1 ] = 'D'; /*±êÊ¶ÉÏÉ¾³ı*/
