...
/*Add by SmallPig*/
void
setqtitle(char* stitle)   /* È¡ Reply ÎÄÕÂºóĞÂµÄ ÎÄÕÂtitle */
{
    FFLL=1;
...
void printutitle()  /* ÆÁÄ»ÏÔÊ¾ ÓÃ»§ÁĞ±í title */
{
...
int get_a_boardname(char* bname,char* prompt)  /* ÊäÈëÒ»¸ö°æÃû */
{
...
/* Add by SmallPig */
int
do_cross(int ent,struct fileheader* fileinfo,char* direct)  /* ×ªÌù Ò»Æª ÎÄÕÂ */
{
    char bname[STRLEN];
    char dbname[STRLEN];
    char ispost[10];
    ...
    if(ispost[0]=='s'||ispost[0]=='S'||ispost[0]=='L'||ispost[0]=='l')
    {
        ...
        if(post_cross(currentuser,currboard,quote_board,quote_title,quote_file,Anony,in_mail,ispost[0],0)==-1) /* ×ªÌù */
        {
            pressreturn();
            move(2,0);
...
int
dele_digest(char* dname,char *direc)         /* É¾³ıÎÄÕªÄÚÒ»ÆªPOST, dname=postÎÄ¼şÃû,direc=ÎÄÕªÄ¿Â¼Ãû */
{
...
int garbage_line(char* str )                  /* ÅĞ¶Ï±¾ĞĞÊÇ·ñÊÇ ÎŞÓÃµÄ */
{
    int qlevel = 0;
...
void
do_quote( char    *filepath,char quote_mode)   /* ÒıÓÃÎÄÕÂ£¬ È«¾Ö±äÁ¿quote_file,quote_user, */
{
    FILE        *inf, *outf;
    char        *qfile, *quser;
    char        buf[256], *ptr;
...
int
show_board_notes(char bname[30])     /* ÏÔÊ¾°æÖ÷µÄ»° */
{
    char buf[256];
...
sequent_messages(struct fileheader *fptr,int* continue_flag)
{
    ...
    if(readpost){
        if(idc < sequent_ent)
            return 0;
        if( !brc_unread(FILENAME2POSTTIME( fptr->filename) ) )  return 0; /*ÒÑ¶Á Ôò ·µ»Ø*/
        if (*continue_flag != 0) {
            genbuf[ 0 ] = 'y';
        } else {
            prints("ÌÖÂÛÇø: '%s' ±êÌâ:\n\"%s\" posted by %s.\n",
...
int change_post_flag( int ent, struct fileheader *fileinfo, char *direct, int flag, int prompt)
{
    ...
    if(!dashf( &genbuf[512]) ) ret = 0; /* ½èÓÃÒ»ÏÂnewent :PP   */
    ...
    switch(flag){
    	...
    	case FILE_NOREPLY_FLAG:
		    if (fileinfo->accessed[1] & FILE_READ) {
		        ...
		        if (prompt) a_prompt( -1, " ¸ÃÎÄÕÂÒÑÈ¡Ïû²»¿ÉreÄ£Ê½, Çë°´ Enter ¼ÌĞø << ",ans );
		    }
		    else {
		    	fileinfo->accessed[1] |= FILE_READ;
		    	if (prompt) a_prompt( -1, " ¸ÃÎÄÕÂÒÑÉèÎª²»¿ÉreÄ£Ê½, Çë°´ Enter ¼ÌĞø << ",ans );
    	...
    	case FILE_SIGN_FLAG:
		    if (fileinfo->accessed[0] & FILE_SIGN) {
        		...
        		if (prompt) a_prompt( -1, " ¸ÃÎÄÕÂÒÑ³·Ïû±ê¼ÇÄ£Ê½, Çë°´ Enter ¼ÌĞø << ",ans );
		    }
		    else {
		    	fileinfo->accessed[0] |= FILE_SIGN;
	        	if (prompt) a_prompt( -1, " ¸ÃÎÄÕÂÒÑÉèÎª±ê¼ÇÄ£Ê½, Çë°´ Enter ¼ÌĞø << ",ans );
		   	}
    		break;
