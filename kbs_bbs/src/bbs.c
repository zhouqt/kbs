...
*/
    ...
    if (haspostperm(currentuser, currboard->filename)) {  /* ¸ù¾ÝÊÇ·ñÓÐPOSTÈ¨ ÏÔÊ¾×îÏÂÒ»ÐÐ */
        if (DEFINE(currentuser, DEF_HIGHCOLOR))
            prints("[44m[1;31m[ÔÄ¶ÁÎÄÕÂ] [33m »ØÐÅ R ©¦ ½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,¡ý©¦Ö÷ÌâÔÄ¶Á ^X»òp [m");
        else
        ...
        if (DEFINE(currentuser, DEF_HIGHCOLOR))
            ...
            prints("[44m[1;31m[ÔÄ¶ÁÎÄÕÂ]  [33m½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,<Enter>,¡ý©¦Ö÷ÌâÔÄ¶Á ^X »ò p [m");
    }
...
int generate_title()
{
    ...
    setboardtitle(currboard->filename, 0);        /* ±ê¼Çflag*/
...
int change_mode(int ent, struct fileheader *fileinfo, char *direct)
{
    ...
    move(t_lines - 1, 0);
    clrtoeol();
    getdata(t_lines - 1, 12, "7)³¬¼¶ÎÄÕÂÑ¡Ôñ [1]: ", ans, 3, DOECHO, NULL, true);
    if (ans[0] == ' ') {
        ans[0] = ans[1];
        ans[1] = 0;
    }
...
int range_flag(int ent, struct fileheader *fileinfo, char *direct)
{
    ...
    getdata(4, 0, "1-±£Áô±ê¼Çm, 2-É¾³ý±ê¼Çt, 3-²»¿É»Ø¸´±ê¼Ç;) [0]", ans, 4, DOECHO, NULL, true);
