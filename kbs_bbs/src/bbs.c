...
int scrint = 0;
int local_article = 0;  //0:×ªÐÅ°æÃæÄ¬ÈÏ×ªÐÅ£¬1:·Ç×ªÐÅ°æÃæ£¬2:×ªÐÅ°æÃæÄ¬ÈÏ²»×ªÐÅ - atppp
int readpost;
int helpmode = 0;
...
int selboard = 0;
int check_upload = 0; //·¢±íÎÄÕÂÊ±ÊÇ·ñÒª¼ì²éÌí¼Ó¸½¼þ
...
/* etnlegend, 2006.04.07, ÐÂÑ¡ÔñÔÄ¶ÁÌÖÂÛÇø... */
int Select(void){
...
int top_move(struct _select_def* conf,struct fileheader *fileinfo)
{
	...
	if (alltop <= 0)
		return DONOTHING;
	getdata(t_lines - 1, 0, "ÒÆ¶¯µ½ÐÂÎ»ÖÃ: ", ans, 3, DOECHO, NULL, true);
	if (ans[0] == '\0' || ans[0] == '\n' || ans[0] == '\r')
		return SHOW_REFRESH;
	d = atoi(ans);
	if (d < 1)
	...
	if (ret==0) {
		...
		char buf[STRLEN];
		a_prompt(-1, "²Ù×÷Ê§°Ü, Çë°´ Enter ¼ÌÐø << ", buf);
		ret=SHOW_REFRESH;
		board_update_toptitle(arg->bid, true);
	}
	return ret;
...
#ifdef FILTER
            {FILE_CENSOR_FLAG,0,0,"ÉóºËÍ¨¹ý"},
#endif
#ifdef COMMEND_ARTICLE
            {FILE_COMMEND_FLAG,1,FILE_COMMEND,"ÉóºËÍ¨¹ý"},
#endif
            {0,0,0,NULL}
    };
    if (fileinfo==NULL)
...
int do_commend(struct _select_def* conf,struct fileheader *fileinfo,void* extraarg)
{
    ...
    if ((fileinfo->accessed[1] & FILE_COMMEND) && !HAS_PERM(getCurrentUser(), PERM_SYSOP)) {
        clear();
        move(1, 0);
        prints("±¾ÎÄÕÂÒÑ¾­ÍÆ¼ö¹ý£¬¸ÐÐ»ÄúµÄÈÈÐÄÍÆ¼ö");
        move(2, 0);
        pressreturn();
        return FULLUPDATE;
    }
    if( ! normal_board(currboard->filename) ){
        clear();
        move(1, 0);
        prints("¶Ô²»Æð£¬ÇëÎðÍÆ¼öÄÚ²¿°æÃæÎÄÕÂ");
        move(2, 0);
        pressreturn();
        return FULLUPDATE;
    }
    if ( deny_me(getCurrentUser()->userid, COMMEND_ARTICLE) ) {
        clear();
        move(1, 0);
        prints("¶Ô²»Æð£¬Äú±»Í£Ö¹ÁËÍÆ¼öµÄÈ¨Á¦");
        move(2, 0);
        pressreturn();
        return FULLUPDATE;
    }
...
{                               /* ×ªÌù Ò»Æª ÎÄÕÂ */
    ...
    if (fileinfo==NULL)
        return DONOTHING;
    if (!HAS_PERM(getCurrentUser(), PERM_POST)) {    /* ÅÐ¶ÏÊÇ·ñÓÐPOSTÈ¨ */
        return DONOTHING;
    }
#if 0 //atppp 20051230
    ...
    prints
        ("\033[1m\033[33mÇë×¢Òâ£º\033[31m±¾Õ¾Õ¾¹æ¹æ¶¨£ºÍ¬ÑùÄÚÈÝµÄÎÄÕÂÑÏ½ûÔÚ 5 (º¬) ¸öÒÔÉÏÌÖÂÛÇøÄÚÖØ¸´ÕÅÌù¡£\n\nÎ¥·´Õß\033[33m³ýËùÌùÎÄÕÂ»á±»É¾³ýÖ®Íâ£¬»¹½«±»\033[31m°þ¶á¼ÌÐø·¢±íÎÄÕÂµÄÈ¨Á¦¡£\033[33mÏêÏ¸¹æ¶¨Çë²ÎÕÕ£º\n\n    Announce °æµÄÕ¾¹æ£º¡°¹ØÓÚ×ªÌùºÍÕÅÌùÎÄÕÂµÄ¹æ¶¨¡±¡£\n\nÇë´ó¼Ò¹²Í¬Î¬»¤ BBS µÄ»·¾³£¬½ÚÊ¡ÏµÍ³×ÊÔ´¡£Ð»Ð»ºÏ×÷¡£\n\n\033[m");
#endif
    move(1, 0);
    ...
    if (!haspostperm(getCurrentUser(), bname)) {
        ...
        if (normal_board(bname) && !normal_board(currboard->filename) && (uinfo.mode != RMAIL)) {
            ...
            getdata(5,4,"ÕâÖÖ×ö·¨Í¨³£ÊÇ\033[1;33m²»ÔÊÐí\033[mµÄ»ò\033[1;33m²»½¨Òé\033[mµÄ, ÄúÈ·ÈÏ×ªÔØ²Ù×÷? [y/N]: ",ans,2,DOECHO,NULL,true);
            if(!(ans[0]=='y'||ans[0]=='Y'))
                return FULLUPDATE;
        }
        bh=getbcache(bname);
        ...
        if (deny_me(getCurrentUser()->userid, bname) ) {    /* °æÖ÷½ûÖ¹POST ¼ì²é */
			if( HAS_PERM(getCurrentUser(), PERM_SYSOP) ){
				...
				clear();
                getdata(3,0,"ÄúÒÑ±»È¡ÏûÔÚÄ¿µÄ°æÃæµÄ·¢ÎÄÈ¨ÏÞ, ÊÇ·ñÇ¿ÖÆ×ªÔØ? [y/N]: ",buf,2,DOECHO,NULL,true);
				if( buf[0]!='y' && buf[0]!='Y' ){
					clear();
					return FULLUPDATE;
				}
            	...
            	move(3, 0);
            	clrtobot();
                prints("\n\n\t\tÄúÒÑ±»¹ÜÀíÈËÔ±È¡ÏûÔÚÄ¿µÄ°æÃæµÄ·¢ÎÄÈ¨ÏÞ...\n");
            	pressreturn();
            	clear();
            	return FULLUPDATE;
			}
    ...
    /* ÖÃ¶¥Ìù¿ÉÒÔ¶à´Î×ªÔØÓ¦¸ÃÃ»ÓÐÎÊÌâ°É...
        else
...
{                               /* °æÄÚ ÏÔÊ¾ÎÄÕÂÁÐ±í µÄ title */
    ...
    if (DEFINE(getCurrentUser(), DEF_HIGHCOLOR))
        prints
            ("Àë¿ª[\x1b[1;32m¡û\x1b[m,\x1b[1;32me\x1b[m] Ñ¡Ôñ[\x1b[1;32m¡ü\x1b[m,\x1b[1;32m¡ý\x1b[m] ÔÄ¶Á[\x1b[1;32m¡ú\x1b[m,\x1b[1;32mr\x1b[m] ·¢±íÎÄÕÂ[\x1b[1;32mCtrl-P\x1b[m] ¿³ÐÅ[\x1b[1;32md\x1b[m] ±¸ÍüÂ¼[\x1b[1;32mTAB\x1b[m] ÇóÖú[\x1b[1;32mh\x1b[m]\033[m");
    else
        prints("Àë¿ª[¡û,e] Ñ¡Ôñ[¡ü,¡ý] ÔÄ¶Á[¡ú,r] ·¢±íÎÄÕÂ[Ctrl-P] ¿³ÐÅ[d] ±¸ÍüÂ¼[TAB] ÇóÖú[h]\x1b[m");
    if (arg->mode== DIR_MODE_NORMAL)        /* ÔÄ¶ÁÄ£Ê½ */
        strcpy(readmode, "Ò»°ã");
...
int showinfo(struct _select_def* conf,struct fileheader *fileinfo,void* extraarg)
{
    ...
    if (!deny_modify_article(currboard, fileinfo, DIR_MODE_NORMAL, getSession())
        ...
        snprintf(slink, sizeof(slink), "¸½¼þ±à¼­µØÖ·: (ÇëÎð½«´ËÁ´½Ó·¢ËÍ¸ø±ðÈË)\n\033[4mhttp://%s/bbseditatt.php?bid=%d&id=%d&sid=%s\033[m\n",
...
*/
    ...
    case READ_THREAD:
        if (DEFINE(getCurrentUser(), DEF_HIGHCOLOR))
	        prints("\x1b[44m\x1b[1;31m[Ö÷ÌâÔÄ¶Á] \x1b[33m »ØÐÅ R ©¦ ½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,¡ý");
        else
	        prints("\x1b[44m\x1b[31m[Ö÷ÌâÔÄ¶Á] \x1b[33m »ØÐÅ R ©¦ ½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,¡ý");
        break;
    case READ_AUTHOR:
        if (DEFINE(getCurrentUser(), DEF_HIGHCOLOR))
	        prints("\x1b[44m\x1b[1;31m[Í¬×÷ÕßÔÄ¶Á] \x1b[33m »ØÐÅ R ©¦ ½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,¡ý");
        else
	        prints("\x1b[44m\x1b[31m[Í¬×÷ÕßÔÄ¶Á] \x1b[33m »ØÐÅ R ©¦ ½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,¡ý");
        break;
    default:
        if (DEFINE(getCurrentUser(), DEF_HIGHCOLOR))
            prints("\033[44m\033[1;31m[ÔÄ¶ÁÎÄÕÂ] \033[33m »ØÐÅ R ©¦ ½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,¡ý©¦Ö÷ÌâÔÄ¶Á ^X»òp ");
        else
            prints("\033[44m\033[31m[ÔÄ¶ÁÎÄÕÂ] \033[33m »ØÐÅ R ©¦ ½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,¡ý©¦Ö÷ÌâÔÄ¶Á ^X»òp ");
    }
...
static int sb_refresh(struct _select_def *conf)
{
    clear();
    docmdtitle("[³¬¼¶°æÃæÑ¡Ôñ]",
               "ÍË³ö[\x1b[1;32m¡û\x1b[0;37m] Ñ¡Ôñ[\x1b[1;32m¡ü\x1b[0;37m,\x1b[1;32m¡ý\x1b[0;37m] ½øÈë°æÃæ[\x1b[1;32mENTER\x1b[0;37m]");
    move(2, 0);
    setfcolor(WHITE, DEFINE(getCurrentUser(), DEF_HIGHCOLOR));
    setbcolor(BLUE);
    clrtoeol();
    prints("   %s   ÌÖÂÛÇøÃû³Æ       V Àà±ð ×ªÐÅ  %-24s            ", "È«²¿ " , "ÖÐ  ÎÄ  Ðð  Êö");
    resetcolor();
    update_endline();
    return SHOW_CONTINUE;
}
         ...
         */
{
    ...
    /*
    move(0, 0);
    prints("Ñ¡ÔñÒ»¸öÌÖÂÛÇø (Ó¢ÎÄ×ÖÄ¸´óÐ¡Ð´½Ô¿É, °´\033[1;32m#\033[0m½øÈë\033[1;31m¹Ø¼ü×Ö»ò°æÃæÃû³Æ\033[0mËÑË÷)");
    clrtoeol();
    ...
    */
    ...
    make_blist(addfav);               /* Éú³ÉËùÓÐBoardÃû ÁÐ±í */
	in_do_sendmsg=true;
...
/* ²éÑ¯°æÃæÐÅÏ¢ by pig2532 on 2005-12-10 */
int board_query()
{
	char bname[STRLEN];
	int bid;
...
int change_mode(struct _select_def* conf,struct fileheader *fileinfo,int newmode)
{
    ...
    if (newmode==0) {
        move(t_lines - 2, 0);
        clrtoeol();
        prints("ÇÐ»»Ä£Ê½µ½: 0)È¡Ïû 1)ÎÄÕª 2)Í¬Ö÷Ìâ 3)±»mÎÄÕÂ 4)Ô­×÷ 5)Í¬×÷Õß 6)±êÌâ¹Ø¼ü×Ö ");
        move(t_lines - 1, 0);
        clrtoeol();
...
int read_hot_info()
{
	...
	case '6':
	{
		...
		if(!(currboard->flag & BOARD_RULES) && !HAS_PERM(getCurrentUser(),PERM_SYSOP) && (getCurrentUser()->title==0) && !chk_currBM(currBM, getCurrentUser()) ){
			clear();
			move(3,0);
			prints("%s°æÉÐÎ´ÓÐÍ¨¹ýÉóºËµÄÖÎ°æ·½Õë\n", currboard->filename);
			pressanykey();
			break;
		}
		...
		if(!(currboard->flag & BOARD_RULES)){
			clear();
			move(3,0);
			prints("%s°æÖÎ°æ·½ÕëÉÐÎ´Í¨¹ýÉóºË,ÅúºÅ:%d\n", currboard->filename, st.st_mtime);
			pressanykey();
		}
		show_help(fpath);
		if(!(currboard->flag & BOARD_RULES) && HAS_PERM(getCurrentUser(), PERM_SYSOP) ){
			...
			move(3,0);
			prints("%s°æÖÎ°æ·½ÕëÉÐÎ´Í¨¹ýÉóºË,ÅúºÅ:%d\n", currboard->filename, st.st_mtime);
        	getdata(t_lines - 1, 0, "ÄúÒªÍ¨¹ý¸Ã°æµÄÖÎ°æ·½ÕëÂð (Y/N)? [N] ", ans, 3, DOECHO, NULL, true);
			if(ans[0]=='y' || ans[0]=='Y'){
				int ret;
         ...
         */
{
    ...
    if (fileinfo==NULL)
        return DONOTHING;
    if (!HAS_PERM(getCurrentUser(), PERM_LOGINOK) || !strcmp(getCurrentUser()->userid, "guest"))  /* guest ÎÞÈ¨ */
        return 0;
    /*
...
int process_upload(int nUpload, int maxShow, char *ans, struct ea_attach_info* ai)
{
    ...
    get_telnet_sessionid(ses, getSession()->utmpent);
    snprintf(buf, sizeof(buf), "¸½¼þÉÏ´«µØÖ·: (ÇëÎð½«´ËÁ´½Ó·¢ËÍ¸ø±ðÈË)\n\033[4mhttp://%s/bbsupload.php?sid=%s\033[m\n", get_my_webdomain(0), ses);
    clear();
    prints(buf);
    nUpload = upload_read_fileinfo(ai, getSession());
    prints("%s", "ÒÑÉÏ´«¸½¼þÁÐ±í: (°´ \033[1;32mu\033[m Ë¢ÐÂ, \033[1;32mu<ÐòºÅ>\033[m É¾³ýÏàÓ¦ÐòºÅ¸½¼þ"
#ifdef SSHBBS
    ...
    if (nUpload > maxShow) {
        snprintf(buf, sizeof(buf), "¸½¼þÊýÁ¿³¬¹ý %d ¸ö£¬Ç°ÃæµÄ²»ÏÔÊ¾ÁË£¡\n", maxShow);
        prints("%s", buf);
    }
    snprintf(buf, sizeof(buf), "\n\033[1;36m¹² \033[1;37m%d\033[1;36m ¸½¼þ/¼Æ \033[1;37m%d\033[1;36m ×Ö½Ú\033[m"
             " \033[1;36m(ÉÏÏÞ \033[1;37m%d\033[1;36m ¸½¼þ/ \033[1;37m%d\033[1;36m ×Ö½Ú)\033[m\n",
...
//	char tmplate[STRLEN];
    ...
    if (!haspostperm(getCurrentUser(), currboard->filename)) { /* POSTÈ¨ÏÞ¼ì²é */
		...
		if( !HAS_PERM(getCurrentUser(), PERM_SYSOP) ){
        	move(3, 0);
        	clrtobot();
            prints("\n\n\t\tÄúÒÑ±»¹ÜÀíÈËÔ±È¡ÏûÔÚµ±Ç°°æÃæµÄ·¢ÎÄÈ¨ÏÞ...\n");
        	pressreturn();
        	clear();
        	return FULLUPDATE;
		}else{
			clear();
            getdata(3,0,"ÄúÒÑ±»È¡ÏûÔÚµ±Ç°°æÃæµÄ·¢ÎÄÈ¨ÏÞ, ÊÇ·ñÇ¿ÖÆ·¢ÎÄ? [y/N]: ",buf,2,DOECHO,NULL,true);
			if( buf[0]!='y' && buf[0]!='Y' ){
				clear();
				return FULLUPDATE;
			}
...
#define ANONY_KEYS "M"
    ...
    while (1) {                 /* ·¢±íÇ°ÐÞ¸Ä²ÎÊý£¬ ¿ÉÒÔ¿¼ÂÇÌí¼Ó'ÏÔÊ¾Ç©Ãûµµ' */
        ...
        move(t_lines - 4, 0);
        clrtoeol();
        prints("\033[m·¢±íÎÄÕÂÓÚ %s ÌÖÂÛÇø  %s %s %s\n", currboard->filename, (anonyboard) ? (Anony == 1 ? "\033[1mÒª\033[mÊ¹ÓÃÄäÃû" : "\033[1m²»\033[mÊ¹ÓÃÄäÃû") : "", mailback? "»Ø¸´µ½ÐÅÏä":"",use_tmpl?"Ê¹ÓÃÄ£°å":"");
        clrtoeol();
         ...
         */
        sprintf(buf2, "%s£¬\033[1;32mb\033[m»Ø¸´µ½ÐÅÏä£¬\033[1;32mT\033[m¸Ä±êÌâ£¬%s%s%s\033[1;32mEnter\033[m¼ÌÐø: ", 
                (replymode) ? "\033[1;32mS/Y/N/R/A\033[m ¸ÄÒýÑÔÄ£Ê½" : "\033[1;32mP\033[mÊ¹ÓÃÄ£°å", (anonyboard) ? "\033[1;32m" ANONY_KEYS "\033[mÄäÃû£¬" : "",
				(currboard->flag&BOARD_ATTACH)?"\033[1;32mu\033[m´«¸½¼þ, ":"",
#ifdef POST_QUIT
				"\033[1;32mQ\033[m·ÅÆú, "
#else
				""
#endif
				);
    ...
    if (!strcmp(post_file.title, buf) && q_file[0] != '\0')
        if (q_file[119] == 'L') /* FIXME */
            local_article = 1; //Õâ¸öµØ·½Ì«¹îÒìÁË£¬ÍêÈ«¿´²»¶®£¬²»ÖªµÀ¶Ô local_save ÓÐÊ²Ã´Ó°Ïì¡£
    ...
    if (returnvalue == -2) {
        clear();
        move(3, 0);
        prints("\n\n        ºÜ±§Ç¸£¬±¾ÎÄ¿ÉÄÜº¬ÓÐ²»µ±ÄÚÈÝ£¬Ðè¾­ÉóºË·½¿É·¢±í¡£\n\n"
                   "        ¸ù¾Ý¡¶ÕÊºÅ¹ÜÀí°ì·¨¡·£¬±»ÏµÍ³¹ýÂËµÄÎÄÕÂÊÓÍ¬¹«¿ª·¢±í¡£ÇëÄÍÐÄµÈ´ý\n"
                   "    Õ¾ÎñÈËÔ±µÄÉóºË£¬²»Òª¶à´Î³¢ÊÔ·¢±í´ËÎÄÕÂ¡£\n\n"
                   "        ÈçÓÐÒÉÎÊ£¬ÇëÖÂÐÅ SYSOP ×ÉÑ¯¡£");
        pressreturn();
        return FULLUPDATE;
    }
#endif
...
int noreply_post(struct _select_def* conf,struct fileheader *fileinfo,void* extraarg)
{
	...
	sprintf(buf,"ÇÐ»»: 0)È¡Ïû %s%s%s[%d]", (can&0x1)?"1)²»¿Ére±ê¼Ç ":"", (can&0x2)?"2)ÖÃ¶¥±ê¼Ç ":"", (can&0x4)?"3)ÍÆ¼ö ":"", (can&0x1)?1:0);
		...
		if(ret != 0){
			...
			if(ret){
				...
				if((fp=fopen(buff,"w"))==NULL){
					...
					fprintf(fp,"%s :\n\n",tmpfh.owner);
					fprintf(fp,"ÓÉÓÚÄúÔÚ\033[1;31m%s\033[m°æµÄÄäÃûÎÄÕÂ\033[1;31m%s\033[m,%s¾ö¶¨×·¼ÓÈ¡ÏûÄúµÄÈ«Õ¾postÈ¨ÏÞ1Ìì\n",currboard->filename,fileinfo->title,getCurrentUser()->userid);
					fprintf(fp,"\nÄäÃû·â½û¶ÔÓÚÄú¿´À´Äú½«ÊÇ±»Ç¿ÖÆ½ä·¢ÎÄÈ¨ÏÞ1Ìì£¬ÇëÄú·ÅÐÄ£¬Ã»ÓÐÈËÖªµÀ±»·â½ûµÄÈË¾ßÌåÊÇÄã£¬Õ¾³¤ºÍ°ßÖñÒ²¶¼²»ÖªµÀ\n");
					fclose(fp);

					sprintf(title,"%sÈ¡Ïû%s°æÄäÃû×÷ÕßÔ­id·¢ÎÄÈ¨ÏÞ1Ìì", getCurrentUser()->userid, currboard->filename);
...
int range_flag(struct _select_def* conf,struct fileheader *fileinfo,void* extraarg)
{
    ...
    sprintf(buf, "1-±£Áôm  2-±ê¼ÇÉ¾³ýt  3-ÎÄÕªg  4-²»¿ÉRe  5-±ê¼Ç#  6-±ê¼Ç%%%s:[0]",
#ifdef FILTER
    ...
    k=ans[0]-'0';
    if (k==3) {
        prints("\033[1;31m±¾¹¦ÄÜ±»°µÉ±, ÈçÓÐ²»·þÕßÕÒ atppp @ newsmth.net PK. \033[36m<ENTER>\033[0m");
        WAIT_RETURN;
        return FULLUPDATE;
    }
    ...
    for(i=inum1;i<=inum2;i++) {
        if(i>=1&&i<=total) {
            ...
            data.accessed[1]=0xff; /* TODO: ÊÇ·ñ¸ù¾ÝÊ×ÆªÎÄÕÂµÄ±ê¼ÇÀ´¾ö¶¨¼ÓÉÏ»¹ÊÇÈ¥µô±ê¼Ç? */
...
void notepad()
{
    ...
    if ((in = fopen(tmpname, "w")) != NULL) {
        ...
        if (note1[0] != 'N' && note1[0] != 'n') {
            ...
            fprintf(in, "\033[m\033[31m¡Ñ©Ð¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©È\033[37mËáÌð¿àÀ±°å\033[31m©À¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©Ð¡Ñ\033[m\n");
            ...
            fprintf(in, "\033[31m¡õ©Ð¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©Ð¡õ\033[m\n");
...
/*---	ÏÔÊ¾±¸ÍüÂ¼µÄ¹Øµô¸ÃËÀµÄ»î¶¯¿´°å	2001-07-01	---*/
    ...
    if (choose == 0)
        choose = 2;
    clear();
    if (strcmp(getCurrentUser()->userid, "guest") && choose == 1) {  /* Ð´ÐÅ¸øÕ¾³¤ */
        if (PERM_LOGINOK & getCurrentUser()->userlevel) {    /*Haohmaru.98.10.05.Ã»Í¨¹ý×¢²áµÄÖ»ÄÜ¸ø×¢²áÕ¾³¤·¢ÐÅ */
            prints("        ID        ¸ºÔðµÄÖ°Îñ\n");
            prints("   ============ =============\n");
            ...
            prints("[\033[33m%1d\033[m] »¹ÊÇ×ßÁËÂÞ£¡\n", num_sysop + 1);      /*×îºóÒ»¸öÑ¡Ïî */
...
/* added by stephen 11/13/01 */
    ...
    if (num_user_logins(getCurrentUser()->userid) == 0 || !strcmp(getCurrentUser()->userid, "guest")) {   /*¼ì²é»¹ÓÐÃ»ÓÐÈËÔÚÏßÉÏ */
        FILE *fp;
        char buf[STRLEN], *ptr;

        if (DEFINE(getCurrentUser(), DEF_MAILMSG /*ÀëÕ¾Ê±¼Ä»ØËùÓÐÐÅÏ¢ */ ) && (get_msgcount(0, getCurrentUser()->userid))) {
        ...
        while (fp != NULL && fgets(buf, sizeof(buf), fp) != NULL) {
            ...
            ptr = strstr(uid, "\n");
            *ptr = '\0';
            if (!strcmp(uid, getCurrentUser()->userid))      /*É¾³ý±¾ÓÃ»§µÄ Ñ°ÈËÃûµ¥ */
...
#endif
        prints("  %2d  %-50s %4s", i, buf, acl[i-1].deny?"¾Ü¾ø":"ÔÊÐí");
    }
    return SHOW_CONTINUE;
}
...
static int set_acl_list_refresh(struct _select_def *conf)
{
    ...
    move(2, 0);
    prints("\033[0;1;37;44m  %4s  %-50s %-31s", "¼¶±ð", "IPµØÖ··¶Î§", "ÔÊÐí/¾Ü¾ø");
    clrtoeol();
    update_endline();
    return SHOW_CONTINUE;
}
...
int set_ip_acl()
{
    ...
    //TODO: ´°¿Ú´óÐ¡¶¯Ì¬¸Ä±äµÄÇé¿ö£¿ÕâÀïÓÐbug
    pts = (POINT *) malloc(sizeof(POINT) * BBS_PAGESIZE);
    for (i = 0; i < BBS_PAGESIZE; i++) {
        pts[i].x = 2;
        pts[i].y = i + 3;
...
struct BMFunc_arg {
    bool delpostnum; /*ÊÇ·ñ¼õÎÄÕÂÊý*/
    int action;            /*°æÖ÷²Ù×÷£¬ÎªBM_DELETEµ½BM_TOTALÆäÖÐÖ®Ò»*/
    bool saveorigin;    /*ÔÚºÏ¼¯²Ù×÷µÄÊ±ºò±íÃ÷ÊÇ·ñ±£´æÔ­ÎÄ*/
    char* announce_path; /*ÊÕÂ¼¾«»ªÇøµÄÊ±ºòµÄÎ»ÖÃ*/
    bool setflag; /*ÉèÖÃ»¹ÊÇÈ¡Ïû*/
};

/*°æÖ÷Í¬Ö÷Ìâº¯Êý£¬ÓÃÓÚapply_recordµÄ»Øµ÷º¯Êý*/
static int BM_thread_func(struct _select_def* conf, struct fileheader* fh,int ent, void* extraarg)
{
    ...
    switch (func_arg->action) {
	    ...
	    if (func_arg->setflag)
            ...
            fh->accessed[0] &= ~FILE_MARKED;
            break;
        case BM_DELMARKDEL: /* etnlegend, 2005.11.28, Í¬Ö÷Ìâ±ê¼ÇÉ¾³ý */
...
static int SR_BMFunc(struct _select_def* conf, struct fileheader* fh, void* extraarg)
{
    ...
    if (arg->mode != DIR_MODE_NORMAL && arg->mode != DIR_MODE_DIGEST)     /* KCN:ÔÝ²»ÔÊÐí */
        return DONOTHING;
...
static int split_thread(struct _select_def* conf, struct fileheader* fh, void* extraarg)
{
    ...
    if (arg->mode != DIR_MODE_NORMAL)     /* KCN:ÔÝ²»ÔÊÐí */
        return DONOTHING;
    if (conf->pos>arg->filecount) /*ÖÃ¶¥*/
        return DONOTHING;
    if (fh->id == fh->groupid)
    ...
    a_prompt(-1, "ÇÐ·ÖÖ÷Ìâ£¬²Ù×÷ºóÎÞ·¨¸´Ô­£¬È·ÈÏÂð£¿(Y/N) [N] ", buf);
    if (buf[0] != 'Y' && buf[0] != 'y') {     /* if not yes quit */
        return FULLUPDATE;
    }
...
static int prompt_newkey(struct _select_def* conf,struct fileheader *fileinfo,void* extraarg) {
    ...
    clear();
    move(3,0);
    prints("\t\t\033[1;4;31m%s¿ì½Ý¼ü»ØÊÕµÚÒ»ºÅÁî\033[0m\n\n", BBS_FULL_NAME);
    prints("\t\tÍ¬Ö¾ÃÇ£¬ÅóÓÑÃÇ£¡¿ì½Ý¼ü»ØÊÕÀ²£¡\n\n\t\tÄú¸Õ²Å°´µÄ¼üÒÑ¾­±»¼¼ÊõÕ¾ÎñË½ÍÌ¡£\n\n");
...
{   
    ...
    strcpy(ans, buf);
    move(t_lines - 1, 0);    
    getdata(t_lines - 1, 0, "ÐÞ¸Ä±¾°æÖÐÎÄÃû£º", ans, STRLEN - 1, DOECHO, NULL, false);
    ...
    if (!(currboard->flag & BOARD_SUPER_CLUB) && !((currboard->title[0]=='P')&&(currboard->title[1]=='_'))) {
        sprintf(pmt, "ÐÞ¸Ä %s °æÖÐÎÄÃû", currboard->filename); 
...
static struct key_command read_comms[] = { /*ÔÄ¶Á×´Ì¬£¬¼ü¶¨Òå */
...
#ifdef FILTER
    {'@', (READ_KEY_FUNC)set_article_flag,(void*)FILE_CENSOR_FLAG},         /* czz: 2002.9.29 ÉóºË±»¹ýÂËÎÄÕÂ */
#endif
    ...
    {'H', (READ_KEY_FUNC)read_callfunc0, (void *)read_hot_info},   /* flyriver: 2002.12.21 Ôö¼ÓÈÈÃÅÐÅÏ¢ÏÔÊ¾ */
...
static void read_top_title(struct _select_def *conf){
    ...
    chkmailflag=chkmail();
    if(chkmailflag==2)
        sprintf(title,"%s","[ÄúµÄÐÅÏä³¬¹ýÈÝÁ¿,²»ÄÜÔÙÊÕÐÅ!]");
    else if(chkmailflag)
        sprintf(title,"%s","[ÄúÓÐÐÅ¼þ]");
    else
    ...
    sprintf(genbuf,"%s\033[m",DEFINE(getCurrentUser(),DEF_HIGHCOLOR)?(
        ...
        "Ñ¡Ôñ[\033[1;32m¡ü\033[1;37m,\033[1;32m¡ý\033[1;37m] ÔÄ¶Á[\033[1;32m¡ú\033[1;37m,\033[1;32mr\033[1;37m] "
        ...
        "Ñ¡Ôñ[\033[32m¡ü\033[37m,\033[32m¡ý\033[37m] ÔÄ¶Á[\033[32m¡ú\033[37m,\033[32mr\033[37m] "
    ...
    switch(arg->readmode){
        case READ_THREAD:
            sprintf(genbuf,"%s","\033[36;44m[Ê®´óÄ£Ê½] \033[31;44m[Í¬Ö÷ÌâÔÄ¶Á]\033[33;44m "
                "½áÊø Q,¡û | ÉÏÒ»Æª ¡ü | ÏÂÒ»Æª <Space>,¡ý ");
            break;
        case READ_AUTHOR:
        ...
        default:
            sprintf(genbuf,"%s","\033[36;44m[Ê®´óÄ£Ê½] \033[31;44m[ÔÄ¶ÁÎÄÕÂ]\033[33;44m "
                "½áÊø Q,¡û | ÉÏÒ»Æª ¡ü | ÏÂÒ»Æª <Space>,¡ý | Í¬Ö÷Ìâ ^X,p ");
            break;
    }
...
#define ST_UPDATE_TOPINFO()                                                             \
    do{                                                                                 \
        ...
        if(!total||(stat("etc/posts/day",&st)==-1||!S_ISREG(st.st_mode))){              \
            move(t_lines-1,0);                                                          \
            clrtoeol();                                                                 \
            prints("\033[1;31m%s\033[0;33m<Enter>\033[m","Ä¿Ç°Ê®´óÈÈÃÅ»°Ìâ²»´æÔÚ!");    \
            WAIT_RETURN;                                                                \
            return -1;                                                                  \
        }                                                                               \
    }while(0)
    ...
    do{
        if(update){
            ...
            move(t_lines-2,8);
            prints("%s","\033[1;33m¶¨Î»\033[1;37m[\033[1;32mUP\033[1;37m,\033[1;32mDOWN\033[1;37m,"
                ...
                switch(toupper(key=igetkey())){
                    ...
                    case ' ':
                        if(read_top(index,0)<0){
                            move(t_lines-1,4);
                            clrtoeol();
                            prints("\033[1;31m%s\033[0;33m<Enter>\033[m","»ñÈ¡Ê®´óÐÅÏ¢Ê±·¢Éú´íÎó!");
                            WAIT_RETURN;
                        }
                        update=1;
                        break;
