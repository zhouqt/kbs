...
static int set_smsg_select(struct _select_def *conf)
{
	clear();
	move(0,0);
	prints("%s–≈»À:%s    ±º‰:%s", s_m[conf->pos-1].type?"∑¢":" ’", s_m[conf->pos-1].dest, s_m[conf->pos-1].time);
...
static int set_smsg_show(struct _select_def *conf, int i)
{
	...
	prints(" %s%-3d %-13s %-14s %s %-40s%s",s_m[i-1].readed?"":"[1m",sm_start + i, s_m[i-1].dest, s_m[i-1].time, s_m[i-1].type?"[1;32m∑¢[m":"[1;33m ’[m", title, s_m[i-1].readed?"":"[m");
