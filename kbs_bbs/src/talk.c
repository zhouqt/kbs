...
int count_useshell(struct user_info *uentp, int *count, int pos)
{
    if (!uentp->active || !uentp->pid)
        return 0;
    if (uentp->pid == 1 || uentp->mode == CSIE_TIN || uentp->mode == CSIE_GOPHER || uentp->mode == EXCE_CHESS || uentp->mode == EXCE_BIG2 || uentp->mode == EXCE_MJ || uentp->mode == IRCCHAT)
        (*count)++;
    return 1;
}
...
char *cmdfile;
{
    ...
    ******/
    sethomepath(userhome, getCurrentUser()->userid);
    sprintf(buf, "/bin/sh %s %s %s %s", cmdfile, userhome, getCurrentUser()->userid, getCurrentUser()->username);
    RUNSH = true;
    do_exec(buf, NULL);
    RUNSH = false;
