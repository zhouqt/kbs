...
int super_filter(int ent, struct fileheader *fileinfo, char *direct)
{
    ...
    prints("                  ³¬Ç¿ÎÄÕÂÑ¡Ôñ\n\n");
    move(5,0);
    prints("±äÁ¿: no(ÎÄÕÂºÅ) m(mÎÄÕÂ) g(gÎÄÕÂ) b(m&&g) noreply(²»¿É»Ø¸´) sign(±ê¼Ç)\n"
           "      del(É¾³ý±ê¼Ç) attach(´ø¸½¼þ) unread(Î´¶Á)\n"
           "      title(±êÌâ) author(×÷Õß)\n"
           "º¯Êý: sub(s1,s2)µÚÒ»¸ö×Ö·û´®ÔÚµÚ¶þ¸öÖÐµÄÎ»ÖÃ,Èç¹û²»´æÔÚ·µ»Ø0\n"
           "      len(s)×Ö·û´®³¤¶È\n"
           "¾ÙÀý: ÎÒÒª²éÑ¯ËùÓÐbadÐ´µÄ±ê¼ÇÊÇbµÄÎÄÕÂ:\n"
           "              author=='bad'&&b\n"
           "      ÎÒÒª²éÑ¯ËùÓÐ²»¿É»Ø¸´²¢ÇÒÎ´¶ÁµÄÎÄÕÂ:\n"
           "              noreply&&unread\n"
           "      ÎÒÒª²éÑ¯ËùÓÐ1000-2000·¶Î§ÄÚ´ø¸½¼þµÄÎÄÕÂ:\n"
           "              (no>=1000)&&(no<=2000)&&attach\n"
           "      ÎÒÒª²éÑ¯±êÌâ³¤¶ÈÔÚ5-10Ö®¼äµÄÎÄÕÂ:\n"
           "              len(title)>=5&&len(title)<=10\n"
           "      ÎÒÒª²éÑ¯±êÌâÀïº¬ÓÐfaintµÄÎÄÕÂ:\n"
           "              sub('faint',title)\n"
           "      ÎÒÒª²éÑ¯±êÌâÀï°üº¬hehe²¢ÇÒÎ»ÖÃÔÚ×îºóµÄÎÄÕÂ:\n"
           "              sub('hehe',title)==len(title)-3\n"
           "      ÎÒÒª²éÑ¯......×Ô¼º¶¯ÊÖ²é°É,hehe"
);
