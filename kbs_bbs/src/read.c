...
char *direct ;
{
    ...
    clrtoeol();
    /* Leeward 98.04.16 */
    sprintf(buf,256,"ÊÇ·ñ´Ó´ËÖ÷ÌâµÚÒ»Æª¿ªÊ¼%s (Y)µÚÒ»Æª (N)Ä¿Ç°ÕâÆª (C)È¡Ïû (Y/N/C)? [Y]: ",SR_BMitems[BMch-1]);
    getdata(t_lines-3, 0,buf,ch,3,DOECHO,NULL,YEA);
    switch (ch[0])
    {
    default:
...
char            *powner;
{
    ...
    sprintf( pmt,STRLEN, "%sµÄÎÄÕÂËÑÑ°×÷Õß [%s]: ", offset > 0 ?  "ÍùááÀ´" : "ÍùÏÈÇ°", currauth );
    move(t_lines-1,0);
    clrtoeol();
