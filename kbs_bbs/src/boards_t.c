...
static int grl_read_boards_getdata(struct _select_def* conf, int pos, int len)
{
    ...
    int sort;
    struct newpostdata *load_to_nbrd;
    const char **load_to_namelist;
    int load_pos, load_len;

    arg = (struct favboard_proc_arg *)(conf->arg);
    sort = (getCurrentUser()->flags & BRDSORT_FLAG) ? ((getCurrentUser()->flags & BRDSORT1_FLAG) + 1) : 0;

    if (pos == -1) {
        load_to_nbrd = NULL;
        load_to_namelist = arg->namelist;
        load_pos = 1;
        load_len = conf->item_count;
    } else {
        load_to_nbrd = arg->nbrd;
        load_to_namelist = NULL;
        load_pos = pos;
        load_len = len;
    }
    ...
    if (GRL_GS_CURR.type == GS_ALL) {
        ...
        conf->item_count = load_boards(load_to_nbrd, prefix, 0, load_pos, load_len, sort, 0, load_to_namelist, getSession());
    }
    // 读取新分类讨论区或个人定制区
    else if ((GRL_GS_CURR.type == GS_NEW) || (GRL_GS_CURR.type == GS_FAV)) {
        conf->item_count = fav_loaddata(load_to_nbrd, GRL_GS_CURR.favid, load_pos, load_len, sort, load_to_namelist, getSession());
    }
    // 读取目录版面
    else if (GRL_GS_CURR.type == GS_GROUP) {
        conf->item_count = load_boards(load_to_nbrd, NULL, GRL_GS_CURR.bid, load_pos, load_len, sort, 0, load_to_namelist, getSession());
    }
    return SHOW_CONTINUE;
}
...
int grl_read_boards()
{
    ...
    bzero((char *)&arg, sizeof(struct favboard_proc_arg));
    arg.nbrd = (struct newpostdata *)malloc(BBS_PAGESIZE * sizeof(struct newpostdata));
