...
static int fav_show(struct _select_def *conf, int pos)
{
    ...
    if ((ptr->dir >= 1)&&arg->favmode)          /* added by bad 2002.8.3*/
        prints("%-50s", buf);
          ...
          prints("%c%-16s %s%s%-36s %-12s", ((ptr->zap && !(ptr->flag & BOARD_NOZAPFLAG)) ? '*' : ' '), ptr->name, (ptr->flag & BOARD_VOTEFLAG) ? "[31;1mV[m" : " ", flag, buf, ptr->BM[0] <= ' ' ? "³ÏÕ÷°æÖ÷ÖĞ" : strtok(tmpBM, " ")); /*µÚÒ»¸ö°æÖ÷ */
#ifdef NINE_BUILD
        if(scr_cols>=80+5) {
            int x,y;
            getyx(&y, &x);
...
static void fav_refresh(struct _select_def *conf)
{
    ...
    setbcolor(BLUE);
    clrtoeol();
    prints("  %s ÌÖÂÛÇøÃû³Æ        V Àà±ğ ×ªĞÅ  %-24s °æ  Ö÷     ", arg->newflag ? "È«²¿ Î´¶Á" : "±àºÅ Î´¶Á", "ÖĞ  ÎÄ  Ğğ  Êö");
    if(scr_cols>=80+5) {
        move(2, 81);
        prints("ÔÚÏß");
    }
