...
int query_bm()
{
    ...
    move(2, 0);
    clrtobot();
    prints("<ÊäÈëÊ¹ÓÃÕß´úºÅ, °´¿Õ°×¼ü¿ÉÁĞ³ö·ûºÏ×Ö´®>\n");
    move(1, 0);
    clrtoeol();
    prints("²éÑ¯Ë­: ");
...
struct favboard_proc_arg {
    ...
    int father; /*±£´æ¸¸½áµã£¬Èç¹ûÊÇÊÕ²Ø¼Ğ£¬ÊÇfav_father,
...
static int fav_show(struct _select_def *conf, int pos)
{
    ...
    if ((ptr->dir >= 1)&&arg->favmode)          /* added by bad 2002.8.3*/
        prints("%-50s\n", buf);
          ...
          prints("%c%-16s %s%s%-36s %-12s\n", ((ptr->zap && !(ptr->flag & BOARD_NOZAPFLAG)) ? '*' : ' '), ptr->name, (ptr->flag & BOARD_VOTEFLAG) ? "[31;1mV[m" : " ", flag, buf, ptr->BM[0] <= ' ' ? "³ÏÕ÷°æÖ÷ÖĞ" : strtok(tmpBM, " ")); /*µÚÒ»¸ö°æÖ÷ */
    }
...
static int fav_prekey(struct _select_def *conf, int *command)
{
    ...
    if (arg->loop_mode) {
        int tmp, num;
/* search_boardÓĞÎÊÌâ£¬Ä¿Ç°ÎÒÃÇÖ»ÓĞÒ»Ò³µÄÊı¾İ£¬Ö»ÄÜsearch
Ò»Ò³*/
    ...
    case 'a':
        {
            ...
            if (favbrd_list_t >= FAVBOARDNUM) {
                move(2, 0);
                clrtoeol();
                prints("¸öÈËÈÈÃÅ°æÊıÒÑ¾­´ïÉÏÏŞ(%d)£¡", FAVBOARDNUM);
                pressreturn();
                return SHOW_REFRESH;
            }
            if (BOARD_FAV == arg->yank_flag) {
                move(0, 0);
                clrtoeol();
                prints("ÊäÈëÌÖÂÛÇøÓ¢ÎÄÃû (´óĞ¡Ğ´½Ô¿É£¬°´¿Õ°×¼ü×Ô¶¯ËÑÑ°): ");
                clrtoeol();
...
static void fav_refresh(struct _select_def *conf)
{
    ...
    if (DEFINE(currentuser, DEF_HIGHCOLOR)) {
        ...
        move(2, 0);
        prints("[1;44m[37m  %s ÌÖÂÛÇøÃû³Æ       V  Àà±ğ ×ªĞÅ  %-24s °æ  Ö÷     ", arg->newflag ? "È«²¿ Î´¶Á" : "±àºÅ Î´¶Á", "ÖĞ  ÎÄ  Ğğ  Êö");
        clrtoeol();
    } else {
        if (arg->yank_flag == BOARD_FAV)
        ...
        move(2, 0);
        prints("[44m[37m  %s ÌÖÂÛÇøÃû³Æ        V Àà±ğ ×ªĞÅ  %-24s °æ  Ö÷     ", arg->newflag ? "È«²¿ Î´¶Á" : "±àºÅ Î´¶Á", "ÖĞ  ÎÄ  Ğğ  Êö");
        clrtoeol();
    }
    if (!arg->loop_mode)
        update_endline();
...
/* Ñ¡Ôñ °æ£¬ readnew»òreadboard */
    ...
    int y;
    struct newpostdata *nbrd;
    int favlevel = 0;           /*µ±Ç°²ãÊı */
    int favlist[FAVBOARDNUM];   /* ±£´æµ±Ç°µÄgroup ĞÅÏ¢ºÍÊÕ²Ø¼ĞĞÅÏ¢*/
    int sellist[FAVBOARDNUM];   /*±£´æÄ¿Â¼µİ¹éĞÅÏ¢ */
    int oldmode;
    int changelevel=-1; /*±£´æÔÚÄÇÒ»¼¶µÄÄ¿Â¼×ª»»ÁËmode,¾ÍÊÇ´ÓÊÕ²Ø¼Ğ½øÈëÁË°æÃæÄ¿Â¼*/
    ...
    while (1) {
        ...
        if (favboard_conf.item_count==0)
            if (arg.yank_flag == BOARD_FAV || arg.yank_flag == BOARD_BOARDALL)
                ...
                char ans[3];
                getdata(t_lines - 1, 0, "¸ÃÌÖÂÛÇø×éµÄ°æÃæÒÑ¾­±»ÄãÈ«²¿È¡ÏûÁË£¬ÊÇ·ñ²é¿´ËùÓĞÌÖÂÛÇø£¿(Y/N)[N]", ans, 2, DOECHO, NULL, true);
                if (toupper(ans[0]) != 'Y')
                    break;
        ...
        if (list_select_loop(&favboard_conf) == SHOW_QUIT) {
            ...
            if (favlevel == -1)
                break;
            if (favlevel==changelevel) //´Ó°æÃæÄ¿Â¼·µ»ØÊÕ²Ø¼Ğ
                favmode=1;
        } else {
            /*Ñ¡ÔñÁËÒ»¸öÄ¿Â¼,SHOW_SELECT£¬×¢ÒâÓĞ¸ö¼ÙÉè£¬Ä¿Â¼µÄÉî¶È
            ²»»á´óÓÚFAVBOARDNUM£¬·ñÔòselist»áÒç³ö
            ×¢Òâ£¬arg->tmpnum±»ÓÃÀ´±íÊ¾ÊÇselectÁË°æÃæ»¹ÊÇÓÃsÌø×ªµÄ¡£
            Èç¹ûÊÇsÌø×ª£¬tmpnum=-1,·ñÔòtmpnum=0;
            TODO: ¶¯Ì¬Ôö³¤sellist
            */
