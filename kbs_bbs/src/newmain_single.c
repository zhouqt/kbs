...
/* Add by KCN for let sysop can use extra 10 UTMP */
    ...
    if ((fn = fopen(fname, "r")) != NULL) {
        mail_file(currentuser->userid, fname, currentuser->userid, "²»Õý³£¶ÏÏßËù±£ÁôµÄ²¿·Ý...", BBSPOST_MOVE);
        fclose(fn);
    }
...
int tBBSlog_recover()
{
    ...
    if (genbuf[0] == 'M' || genbuf[0] == 'm')
        mail_file(currentuser->userid, buf, currentuser->userid, "ÁÄÌì¼ÇÂ¼", BBSPOST_MOVE);
    else
        unlink(buf);
    return;
