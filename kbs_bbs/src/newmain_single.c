...
// ws code to reset max_user. do not enable unless you know what you are doing.
    ...
    attempts = 0;
#ifdef LOGINASNEW
    prints("\n\033[1m\033[37m试用请输入 `\033[36mguest\033[37m', 注册请输入`\033[36mnew\033[37m', add `\033[36m.\033[37m' after your ID for BIG5\033[m");
#else
    prints("\n\033[1m\033[37m试用请输入 `\033[36mguest\033[37m', 暂停注册新帐号, add `\033[36m.\033[37m' after your ID for BIG5\033[m");
#endif //LOGINASNEW
    while (1) {
        if (attempts++ >= 3) {
...
//    read_userdata(getCurrentUser()->userid, &curruserdata);
    ...
    if (!HAS_PERM(getCurrentUser(), PERM_SYSOP) && getCurrentUser()->score_user < pls && check_proxy_IP(getSession()->fromhost, NULL)) {
        prints("您的用户积分不足 %d，不能穿梭登录本站，请尝试直连 ...", pls);
        oflush();
        Net_Sleep(3);
        abort_bbs(0);
    }
