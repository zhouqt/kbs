...
/*    if(dashf(fname,"r")) This block is disabled for long, not for SHARE MEM
    ...
    prints( "\033[1mª∂”≠π‚¡Ÿ [31m%s[37m °Ù±æ’æ π”√ Ôπ‚π´Àæ Ôπ‚ÃÏ—›∑˛ŒÒ∆˜°Ù [36mƒø«∞…œœﬂ»À ˝ \033[1m%d[m", BoardName, curr_login_num);
/*{
char ii[16];
...
/*Haohmaru.98.11.3*/
        ...
        getdata( 0, 0, "\n\033[1m[37m«Î ‰»Î¥˙∫≈( ‘”√«Î ‰»Î `\033[36mguest\033[37m'; “ÚœµÕ≥π ’œ£¨‘›Õ£◊¢≤·–¬’ ∫≈π¶ƒ‹“ª∂Œ ±º‰£¨º˚¡¬): [m",*/
                uid, STRLEN-1, DOECHO, NULL ,YEA);
        if( strcmp( uid, "new" ) == 0 ) {
#ifdef LOGINASNEW
            /*prints( "\033[32m¥ÌŒÛµƒ π”√’ﬂ¥˙∫≈...\033[m\n" );*/
...
defnotepad()
{
  ...
  if(currentuser->notemode==-1)
  {
     fclose(def2);
     while(1){
        getdata( 0, 0, "«Î ‰»Îƒ„“™µƒ¡Ù—‘∞Âƒ£ Ω (1) »´ø¥ (2) ÷ªø¥√ªø¥π˝µƒ (3) ∂º≤ªø¥: ", spbuf, 2, DOECHO, NULL ,YEA);
