    ...
    else
    {
        ...
        if (!strcmp(currboard,"Announce"))
            ...
            fprintf(fp,"·¢ÐÅÈË: %s (%s), ÐÅÇø: %s       \n",(noname&&Anony)?"Anonymous":uid,
                    (noname&&Anony)?"ÎÒÊÇÄäÃûÌìÊ¹":uname,currboard) ;
    } /***** ¸ÃÓï¾äÔö¼ÓÔÄ¶Á´ÎÊý¼ÆÊýÏî Add by Luzi 1998/1/8 ****/
    ...
    if(uinfo.mode!=CCUGOPHER)
    {
        ...
        */
        if(uinfo.mode == POSTING)
        {
            ...
            /*strcpy(p_buf,"(S)¼Ä³ö, (A)È¡Ïû, or (E)ÔÙ±à¼­? [S]: " );*/
            sprintf(p_buf,"¸ø %s µÄÐÅ£º(S)¼Ä³ö, (F)×Ô¶¯»»ÐÐ¼Ä³ö, (A)È¡Ïû, or (E)ÔÙ±à¼­? [S]: ", lookupuser.userid ); /* Leeward 98.01.17 Prompt whom you are writing to */
        else
