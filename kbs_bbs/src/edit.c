...
void msgline()
{
    ...
    sprintf(buf2, " ◊¥Ã¨ [[32m%s[33m][[32m%d[33m,[32m%d[33m][[32m%c[33m][[32m%c[33m]      ±º‰", insert_character ? "≤Â»Î" : "ÃÊªª", currln + 1, currpnt + 1,
        show_eof?'~':' ', auto_newline?' ':'X');
    strcat(buf, buf2);
    sprintf(buf2, "[33m[44m°æ[32m%.16s[33m°ø", ctime(&now));
    strcat(buf, buf2);
    move(t_lines - 1, 0);
    prints("%s", buf);
    clrtoeol();
