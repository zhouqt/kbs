...
void go()
{
    ...
    getdata(23, 0, "ÇëÎÊÒªÌøµ½µÚ¼¸ÐÐ: ", tmp, 7, DOECHO, NULL, 0);
    domsg();
    if (tmp[0] == '\0')
        return;
    line = atoi(tmp);
    ...
    int saveheader;
{
    ...
    if (uinfo.mode != CCUGOPHER) {
        ...
        if (uinfo.mode == POSTING) {
            if (local_article == 1)
                strcpy(p_buf, "(L)Õ¾ÄÚ, (S)×ªÐÅ, (F)×Ô¶¯»»ÐÐ·¢±í, (A)È¡Ïû, (T)¸ü¸Ä±êÌâ or (E)ÔÙ±à¼­? [L]: ");
            else
                strcpy(p_buf, "(S)×ªÐÅ, (L)Õ¾ÄÚ, (F)×Ô¶¯»»ÐÐ·¢±í, (A)È¡Ïû, (T)¸ü¸Ä±êÌâ or (E)ÔÙ±à¼­? [S]: ");
