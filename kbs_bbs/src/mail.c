...
void mailtitle(struct _select_def* conf)
{
    ...
    if (0 != get_mailnum(arg->direct) && 0 == UsedSpace)
        ...
        UsedSpace = 0;
    prints("[44m±àºÅ    %-12s %6s  %-13sÄúµÄĞÅÏäÉÏÏŞÈİÁ¿%4dK£¬µ±Ç°ÒÑÓÃ%4dK ", (strstr(arg->direct, ".SENT")) ? "ÊÕĞÅÕß" : "·¢ĞÅÕß", "ÈÕ  ÆÚ", "±ê  Ìâ", MailSpace, UsedSpace);    /* modified by dong , 1998.9.19 */
    clrtoeol();
    prints("\n");
    resetcolor();
}
