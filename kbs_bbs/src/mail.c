...
char *maildoent(char *buf, int num, struct fileheader *ent,struct fileheader* readfh,struct _select_def* conf)
{
  ...
  if (!gShowSize) {
    ...
    if (!strncmp("Re:", ent->title, 3)) {
        ...
        sprintf(buf, " %s%3d\033[m %c%c %-12.12s %6.6s  ★ %s%.47s\033[m", same ? c2 : "", num, reply_status, status, b2, date, same ? c2 : "", ent->title);
    }                           /* modified by dong, 1998.9.19 */
#ifdef ZIXIA
  } else {
    int size = 0;
    ...
    if (!strncmp("Re:", ent->title, 3)) {
        ...
        sprintf(buf, " %s%3d\033[m %c%c %-12.12s %6.6s  ★ %s%-42.42s%4dK\033[m", same ? c2 : "", num, reply_status, status, b2, date, same ? c2 : "", ent->title, size);
    }                           /* modified by dong, 1998.9.19 */
  }
#endif
    return buf;
