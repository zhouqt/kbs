...
//            mail_file(currentuser->userid, filepath, currentuser->userid, save_title, 0);
		...
		getdata(2, 0, "±£´æĞÅ¼şµ½·¢¼şÏä? [N]: ", buf2, 2, DOECHO, 0, 0);
        ...
        if (stat(filepath, &st) != -1) {
            ...
            /*currentuser->usedspace += st.st_size;*/ /* ÕâÀï¶à¼ÆËãÁËÒ»´Î */
        }
    ...
    if (mail_list_t) {
   	...
   	for (i=0;i<mail_list_t;i++) {
	    ...
	    num = get_num_records(buf, sizeof(struct fileheader));
    	    if (num) {
    			char prompt[80];
    			sprintf(prompt,"Çå³ı×Ô¶¨ÒåÓÊÏä %s Ã´?",mail_list[i]);
    			if (askyn(prompt,0))
    				delete_range(buf, 1, num, 2);
    	    }
    	}
...
const static struct command_def mail_cmds[] = {
    ...
    {"S) ¼ÄĞÅ", PERM_LOGINOK, m_send, NULL},
