...
chkmail()
{
    ...
    if ( !HAS_PERM(PERM_SYSOP) ) {/*Haohmaru.99.4.4.¶ÔÊÕÐÅÒ²¼ÓÏÞÖÆ,¸Ä¶¯ÏÂÃæµÄÊý×ÖÊ±ÇëÍ¬Ê±¸Ä¶¯chkreceiverº¯Êý*/
...
char *userid, *title ;
{
    ...
    if ( !HAS_PERM(PERM_SYSOP) ) {
        ...
        else if (HAS_PERM(PERM_MANAGER))  /* alexÓÚ1996.10.20Ìí¼Ó£¬revised by stephen on 2001.11.1, mailboxÈÝÁ¿ÏÞÖÆ */
        {
            sumlimit = 300;
            numlimit = 300;
        }
...
int num ;
{
    ...
    if ( !HAS_PERM(PERM_SYSOP) ) {
        ...
        else if (HAS_PERM(PERM_MANAGER))  /* Leeward ÓÚ1997.12.13Ìí¼Ó£¬revised by stephen on 2001.11.1 , */
/* mailbox ÈÝÁ¿ÏÞÖÆ */
        {
            sumlimit = 300;
            numlimit = 300;
...
int isuu;
{
    ...
    if ( !HAS_PERM(PERM_SYSOP) ) {
        ...
        else if (HAS_PERM(PERM_MANAGER))  /* Leeward ÓÚ1997.12.13Ìí¼Ó£¬revised by stephen on 2001.11.1, mailbox ÈÝÁ¿ÏÞÖÆ */
        {
            sumlimit = 300;
            numlimit = 300;
        }
    ...
    if(askyn("ÊÇ·ñÐÞ¸ÄÎÄÕÂÄÚÈÝ",0)==1)
    {
        vedit(fname,NA);
        y = 2;
        sprintf(tmp_buf,"ÐÞ¸Ä±»×ªÌùµÄÎÄÕÂ»òÐÅ¼þ: %s",title);/*Haohmaru.00.05.01*/
        report(tmp_buf);
        /* clear(); */
    }
