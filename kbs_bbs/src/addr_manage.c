...
int add_addresslist( struct addresslist * oldal ){
	...
	if(oldal)
		...
		ans[0]=0;
	getdata(4, 0, "«Î ‰»Î“∆∂ØµÁª∞:", ans, 15, DOECHO, NULL, false);
	...
	if(oldal)
		...
		ans[0]=0;
	getdata(5, 0, "«Î ‰»Î∑÷◊È√˚≥∆:", ans, 10, DOECHO, NULL, false);
	...
	if(oldal)
		...
		ans[0]=0;
	getdata(6, 0, "«Î ‰»Î—ß–£:", ans, 100, DOECHO, NULL, false);
	...
	if(oldal)
		...
		ans[0]=0;
	getdata(7, 0, "«Î ‰»Î” ’˛±‡¬Î:", ans, 7, DOECHO, NULL, false);
	...
	if(oldal)
		...
		ans[0]=0;
	getdata(8, 0, "«Î ‰»Îº“Õ•µÿ÷∑:", ans, 100, DOECHO, NULL, false);
	...
	if(oldal)
		...
		ans[0]=0;
	getdata(9, 0, "«Î ‰»Îπ§◊˜µÿ÷∑:", ans, 100, DOECHO, NULL, false);
	...
	if(oldal)
		...
		ans[0]=0;
	getdata(10, 0, "«Î ‰»Îº“Õ•µÁª∞:", ans, 20, DOECHO, NULL, false);
	...
	if(oldal)
		...
		ans[0]=0;
	getdata(11, 0, "«Î ‰»Îπ§◊˜µÁª∞:", ans, 20, DOECHO, NULL, false);
	...
	if(oldal)
		...
		ans[0]=0;
	getdata(12, 0, "«Î ‰»ÎµÁ◊”” œ‰:", ans, 30, DOECHO, NULL, false);
	...
	if(oldal)
		...
		ans[0]=0;
	getdata(13, 0, "«Î ‰»Îqq:", ans, 10, DOECHO, NULL, false);
...
{
	clear();
	docmdtitle("[Õ®—∂¬ºπ‹¿Ì]","ÕÀ≥ˆ[[1;32mq[m] ‘ˆº”[[1;32ma[m] –ﬁ∏ƒ[[1;32me[m] …æ≥˝[[1;32md[m] —°‘Ò[[1;32ms[m] ≈≈–Ú[[1;32mTAB[m] µº»Î[[1;32mi[m] ∑¢ÀÕ∂Ã–≈[[1;32mS[m]");
	move(2,0);
	prints("[0;1;44m  %-4s %s%-8s %s%-10s%s %-10s%s %-12s %-28s              [m","–Ú∫≈",al_order==AL_ORDER_NAME?"[1;32m":"",al_order==AL_ORDER_NAME?"{√˚◊÷}":"√˚◊÷",al_order==AL_ORDER_NAME?"[0;1;44m":(al_order==AL_ORDER_BBSID?"[1;32m":""),al_order==AL_ORDER_BBSID?"{id}":"id",al_order==AL_ORDER_BBSID?"[0;1;44m":(al_order==AL_ORDER_GROUPNAME?"[1;32m":""),al_order==AL_ORDER_GROUPNAME?"{∑÷◊È}":"∑÷◊È",al_order==AL_ORDER_GROUPNAME?"[0;1;44m":"", " ÷ª˙","±∏◊¢");
	update_endline();
	return SHOW_CONTINUE;
}
