...
static int set_al_refresh(struct _select_def *conf)
{
	...
	move(2,0);
	prints("[0;1;44m  %-4s %s%-8s %s%-10s%s %-10s %-40s              [m","–Ú∫≈",al_order==AL_ORDER_NAME?"[1;32m":"",al_order==AL_ORDER_NAME?"{√˚◊÷}":"√˚◊÷",al_order==AL_ORDER_NAME?"[0;1;44m":(al_order==AL_ORDER_BBSID?"[1;32m":""),al_order==AL_ORDER_BBSID?"{id}":"id",al_order==AL_ORDER_BBSID?"[0;1;44m":"", "∑÷◊È", "±∏◊¢");
	update_endline();
	return SHOW_CONTINUE;
}
