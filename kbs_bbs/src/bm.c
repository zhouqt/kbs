...
int
deny_user()  /* ½ûÖ¹POSTÓÃ»§Ãûµ¥ Î¬»¤Ö÷º¯Êý*/
{
    char uident[STRLEN];
    int page=0;
...
Here:
        ...
        if (count>0 && count<20)/*Haohmaru.12.18,¿´ÏÂÒ»ÆÁ*/
            getdata(1,0,"(A)Ôö¼Ó (D)É¾³ý or (E)Àë¿ª [E]: ",ans,7,DOECHO,NULL,YEA);
        else if(count>20)
            getdata(1,0,"(A)Ôö¼Ó (D)É¾³ý (N)ºóÃæµÚNÆÁ or (E)Àë¿ª [E]: ", ans, 7, DOECHO, NULL,YEA);
        else
        ...
        if (*ans == 'A' || *ans == 'a') {
            ...
            {
                move(3,0) ;
                prints("¸ÃID²»ÔÚ·â½ûÃûµ¥ÄÚ(Çë×¢Òâ£º´óÐ¡Ð´Ò»¶¨ÒªºÍ·â½ûÃûµ¥ÀïµÄÒ»ÖÂ!)") ;
                clrtoeol() ;
                pressreturn() ;
                goto Here;
            }
...
BoardFilter()
{
    ...
    ch=atoi(ans);
    /*   if (ch==2)added by Haohmaru,98.9.6,Çå³ýÎÞ·¨Çø¶ÎÉ¾³ýµÄ´íÎó
          { FILE* tmp;
           sprintf(buf,"~bbsroot/boards/%s/.tmpfile",currboard);
           if((tmp=fopen(buf,"r"))==NULL)
           {
            getdata(t_lines-1,0,"                              Çë°´ ¡ôEnter¡ô ¼ÌÐø",buf,2,NOECHO,NULL,YEA);
           }
           else
           { fclose(tmp);
