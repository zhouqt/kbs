...
int addtodeny(char* uident) /* Ìí¼Ó ½ûÖ¹POSTÓÃ»§ */
{
    ...
    *denymsg = 0;
    if ((reasonfile=open("etc/deny_reason",O_RDONLY))!=-1) {
		...
		prints("%s","0.ÊÖ¶¯ÊäÈë·â½ûÀíÓÉ");
		    while (1) {
		    	getdata(2,0,"Çë´ÓÁÐ±íÑ¡Ôñ·â½ûÀíÓÉ(0ÎªÊÖ¹¤ÊäÈë,*ÍË³ö):",denymsg,2,DOECHO,NULL,YEA);
			if (denymsg[0]=='*') {
		    	free(file_buf);
    			close(reasonfile);
				return 0;
    ...
    if(denyday && autofree) {
        ...
        sprintf( strtosave, "%-12.12s %-30.30s%-12.12s %2dÔÂ%2dÈÕ½â\x1b[%lum",
        ...
        time_t undenytime = now+denyday*24*60*60;
        tmtime=gmtime(&undenytime);
        sprintf( strtosave, "%-12.12s %-30.30s%-12.12s %2dÔÂ%2dÈÕºó\x1b[%lum",
                uident, denymsg, currentuser->userid,
