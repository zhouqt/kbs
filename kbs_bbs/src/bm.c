...
{                               /* Ìí¼Ó ½ûÖ¹POSTÓÃ»§ */
    ...
    *denymsg = 0;
    ...
    sprintf(filebuf, "ÊäÈëÌìÊý(×î³¤%dÌì)(°´*È¡Ïû·â½û)", maxdeny);
    denyday = 0;
    while (!denyday) {
    ...
    if (addtofile(genbuf, strtosave) == 1) {
        ...
        /*
         * Haohmaru.4.1.×Ô¶¯·¢ÐÅÍ¨Öª²¢·¢ÎÄÕÂÓÚ°æÉÏ 
         */
        ...
        if ((HAS_PERM(currentuser, PERM_SYSOP) || HAS_PERM(currentuser, PERM_OBOARDS)) && !chk_BM_instr(currBM, currentuser->userid)) {
            ...
            fprintf(fn, "À´  Ô´: %s\n", NAME_BBS_ENGLISH);
            fprintf(fn, "\n");
            fprintf(fn, "ÓÉÓÚÄúÔÚ \x1b[4m%s\x1b[m °æ \x1b[4m%s\x1b[m£¬ÎÒºÜÒÅº¶µØÍ¨ÖªÄú£¬ \n", currboard, denymsg);
            if (denyday)
                fprintf(fn, "Äú±»ÔÝÊ±È¡ÏûÔÚ¸Ã°æµÄ·¢ÎÄÈ¨Á¦ \x1b[4m%d\x1b[m Ìì", denyday);
            else
                fprintf(fn, "Äú±»ÔÝÊ±È¡ÏûÔÚ¸Ã°æµÄ·¢ÎÄÈ¨Á¦");
            if (!autofree)
                fprintf(fn, "£¬µ½ÆÚºóÇë»Ø¸´\n´ËÐÅÉêÇë»Ö¸´È¨ÏÞ¡£\n");
            fprintf(fn, "\n");
            fprintf(fn, "                            " NAME_BBS_CHINESE NAME_SYSOP_GROUP "Öµ°àÕ¾Îñ£º\x1b[4m%s\x1b[m\n", currentuser->userid);
            fprintf(fn, "                              %s\n", ctime(&now));
        ...
        fn = fopen(filename, "w+");
        fprintf(fn, "ÓÉÓÚ \x1b[4m%s\x1b[m ÔÚ \x1b[4m%s\x1b[m °æµÄ \x1b[4m%s\x1b[m ÐÐÎª£¬\n", uident, currboard, denymsg);
        if (denyday)
            fprintf(fn, "±»ÔÝÊ±È¡ÏûÔÚ±¾°æµÄ·¢ÎÄÈ¨Á¦ \x1b[4m%d\x1b[m Ìì¡£\n", denyday);
        else
        ...
        *ans = (char) toupper((int) *ans);
        ...
        if (*ans == 'A' || *ans == 'O') {
            ...
            if (ans[1] == 0)
                ...
                page = atoi(ans);
            if (page < 0)
                break;          /*²»»á·âÈË³¬¹ý10ÆÁ°É?ÄÇ¿ÉÊÇ200ÈË°¡!  »áµÄ£¡ */
            listdeny(page);
            pressanykey();
        } else
            break;
...
int addclubmember(char *uident, int readperm)
{
	...
	getdata(4, 0, "ÕæµÄÒªÌí¼ÓÃ´?[Y/N]: ", ans, 7, 1, NULL, true);
	if ((*ans != 'Y') && (*ans != 'y'))
		return -1;
...
int clubmember(int ent, struct fileheader *fh, char *direct)
{
    ...
    if ((bh.flag & BOARD_CLUB_READ) && (bh.flag & BOARD_CLUB_WRITE)) {
        ...
        struct _select_item menu_conf[] = {
            {left, top + 2, 'R', SIT_SELECT, (void *) "[R] Éè¶¨¿É¶ÁÈ¡ÓÃ»§Ãûµ¥"},
            {left, top + 3, 'P', SIT_SELECT, (void *) "[P] Éè¶¨¿É·¢±íÓÃ»§Ãûµ¥"},
            {-1, -1, -1, 0, NULL}
        };
        ...
        if (*ans == 'A' || *ans == 'a') {
            ...
            if (*uident != '\0') {
                if (addclubmember(uident, readperm) == 1) {
                    sprintf(genbuf, "%sÓÉ%sÊÚÓè%s¾ãÀÖ²¿È¨Á¦", uident, currentuser->userid, currboard);
            ...
            move(1, 0);
            namecomplete("É¾³ý¾ãÀÖ²¿Ê¹ÓÃÕß: ", uident);
            move(1, 0);
            clrtoeol();
            if (uident[0] != '\0') {
                sprintf(genbuf, "ÕæµÄÒªÈ¡Ïû%sµÄ¾ãÀÖ²¿È¨Á¦Ã´£¿", uident);
                if (askyn(genbuf, true))
                    if (delclubmember(uident, readperm)) {
                        sprintf(genbuf, " %s ±»%s È¡Ïû %s ¾ãÀÖ²¿ È¨Á¦", uident, currentuser->userid, currboard);
                        securityreport(genbuf, NULL, NULL);
                        mail_buf(currentuser, genbuf, uident, genbuf);
                        deliverreport(genbuf, genbuf);
                    }
