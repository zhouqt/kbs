...
static void printchatline(chatcontext * pthis, const char *str) /*ÏÔÊ¾Ò»ÐÐ£¬²¢ÏÂÒÆÖ¸Ê¾·û*/
{
    char tmpstr[256];
    const char *p;
    int i;
...
static void query_user(chatcontext *pthis,const char* userid)
{
    ...
    if (apply_utmpuid((APPLY_UTMP_FUNC) chat_status,tuid,(char*)pthis)) {
        ...
        sprintf(buf, "Ä¿Ç°ÕýÔÚÏßÉÏ: À´×Ô %s ÉÏÏßÊ±¼ä %s"/*\n"*/,
                (lookupuser->lasthost[0] == '\0'/* || DEFINE(currentuser,DEF_HIDEIP)*/ ? "(²»Ïê)" : lookupuser->lasthost), inbuf);/*Haohmaru.99.12.18*/
        printchatline(pthis,buf);
