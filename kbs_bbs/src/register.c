...
int ProtectID()
{
	...
	if(!HAS_PERM(currentuser,PERM_LOGINOK)) {
        	move(11, 28);
		prints("\033[1;33mÄãÉĞÎ´Í¨¹ıÉí·İÈÏÖ¤,²»ÄÜÉè¶¨ÃÜÂë±£»¤!\033[m");
        	pressanykey();
		return -1;
        }
	...
	if(dashf(buf)) {
        	move(11, 28);
		prints("\033[1;33mÄãÒÑ¾­Éè¶¨ÃÜÂë±£»¤¹¦ÄÜ,²»ÄÜÔÙ¸ü¸ÄÉè¶¨!\033[m");
        	pressanykey();
		return -1;
	}
	    
	move(1, 0);
    	prints("Ñ¡ÔñÃÜÂë±£»¤¹¦ÄÜºó,¿ÉÒÔÔÚÒÅÍüÃÜÂë»òÕßÃÜÂë±»µÁÓÃµÄÇé¿öÏÂ,¸ù¾İÊÂÏÈÉè¶¨µÄĞÅÏ¢");
	move(2, 0);
	prints("ÖØĞÂÕÒ»Ø×Ô¼ºµÄÃÜÂë.");
    	move(4, 0);
    	prints("ÕâĞ©Éè¶¨µÄĞÅÏ¢°üÀ¨:\033[1;31m ĞÕÃû/ÉúÈÕ/Email/ÃÜÂëÌáÊ¾ÎÊÌâ/ÎÊÌâ´ğ°¸\033[m");
    	move(6,0);
	//prints("ÉÏÃæµÄĞÅÏ¢ÉèÖÃÍê³É, ÃÜÂë±£»¤¹¦ÄÜÒ»µ©³É¹¦´ò¿ªºó, ½«²»ÄÜÔÙ¸ü¸ÄÕâĞ©ĞÅÏ¢,ÇĞ¼Ç,ÇĞ¼Ç!");
	//move(7,0);
	prints("ÕÒ»ØÃÜÂëÊ±,ĞèÒª ĞÕÃû/ÉúÈÕ/ÎÊÌâ´ğ°¸ ºÍÉè¶¨ÍêÈ«Ò»ÖÂ,²Å»á½«ĞÂÃÜÂë·¢Íù Email .");

	move(8,0);
    	if (askyn("ÄãÈ·¶¨Òª´ò¿ªÃÜÂë±£»¤¹¦ÄÜÂğ£¿", 0) == 0) {
		return -1;
	}
	...
	sprintf(print_buf,"ÇëÊäÈëÄúµÄÕæÊµĞÕÃû: [%s]",currentmemo->ud.realname);
	...
	sprintf(print_buf,"ËÄÎ»Êı¹«ÔªÄê: [%d]",currentmemo->ud.birthyear);
	...
	sprintf(print_buf,"³öÉúÔÂ: [%d]",currentmemo->ud.birthmonth);
	...
	sprintf(print_buf,"³öÉúÈÕ: [%d]",currentmemo->ud.birthday);
