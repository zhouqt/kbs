...
#ifdef NINE_BUILD
    sprintf(title, "ĞÂÊÖÉÏÂ· : %s", newuser.userid);
    fname = "tmp/newuser";
    fn = fopen(fname, "w");
    fprintf(fn, "´ó¼ÒºÃ,\n\n");
    fprintf(fn, "ÎÒÊÇ %s (%s), À´×Ô %s\n");
    fprintf(fn, "½ñÌìĞ¡µÜ³õÀ´´ËÕ¾±¨µ½, Çë´ó¼Ò¶à¶àÖ¸½Ì¡£\n\n\n");
    fprintf(fn, "×ÔÎÒ½éÉÜ:\n\n");
		...
		if (currentuser->numlogins == 1)
		{
			...
			if ((fout = fopen(buf, "w")) != NULL)
			{
				...
				fprintf(fout, "½ñÌì%s³õÀ´´ËÕ¾±¨µ½, Çë´ó¼Ò¶à¶àÖ¸½Ì¡£\n",
						(currentuser->gender == 'M') ? "Ğ¡µÜ" : "Ğ¡Å®×Ó");
				move(9, 0);
    ...
    if (!(currentuser->userlevel & PERM_LOGINOK)) {
        ...
        if (!invalid_realmail(currentuser->userid, ud.realemail, STRLEN - 16)) {
            ...
            */
            ...
            prints("ÇëÔÚ[31m¸öÈË¹¤¾ßÏä[mÄÚ[31mÏêÏ¸×¢²áÉí·İ[m\n");
