...
x_level()
{
    ...
    if (newlevel == lookupuser->userlevel)
        ...
        if ((lookupuser->userlevel & PERM_BOARDS ) && flag==0 )
            mail_file("etc/forbm",lookupuser->userid,"ÐÂÈÎ"NAME_BM"±Ø¶Á");
        /* Bigman 2000.1.5 ÐÞ¸ÄÈ¨ÏÞ×Ô¶¯·¢ÐÅ */
        if ((lookupuser->userlevel & PERM_CLOAK ) && flag1==0 )
            mail_file("etc/forcloak",lookupuser->userid,NAME_SYSOP_GROUP"ÊÚÓèÄúÒþÉíÈ¨ÏÞ");
        if ((lookupuser->userlevel & PERM_XEMPT ) && flag2==0 )
            mail_file("etc/forlongid",lookupuser->userid,NAME_SYSOP_GROUP"ÊÚÓèÄú³¤ÆÚÕÊºÅÈ¨ÏÞ");
    }
    pressreturn() ;
    clear() ;
    return 0 ;
...
a_edits()
{
    ...
    char *e_file[]={"../Welcome","../vote/notes","issue","movie","logout","menu.ini", "proxyIP", "mailcheck","s_fill","f_fill.realname","f_fill.unit","f_fill.address","f_fill.telephone","f_fill.real","f_fill.chinese","f_fill.toomany","f_fill.proxy","smail","f_fill","../.badname", "../.badIP", "../.badword", "sysconf.ini", "www_menu.js", "../0Announce/hotinfo", "forbm", "forcloak", "forlongid", NULL};
    /* "/usr/share/apache/htdocs/script/menucontext.js", NULL};    */
#ifndef LEEWARD_X_FILTER
    har *explain_file[]={"Welcome","¹«ÓÃ±¸ÍüÂ¼","½øÕ¾»¶Ó­µµ","»î¶¯¿´°æ","ÀëÕ¾»­Ãæ", "menu.ini","´©ËóIP", "Éí·ÝÈ·ÈÏµµ","×¢²áµ¥Íê³Éµµ","×¢²áµ¥Ê§°Üµµ(ÕæÊµÐÕÃû)","×¢²áµ¥Ê§°Üµµ(·þÎñµ¥Î»)","×¢²áµ¥Ê§°Üµµ(¾Ó×¡µØÖ·)","×¢²áµ¥Ê§°Üµµ(ÁªÂçµç»°)","×¢²áµ¥Ê§°Üµµ(ÕæÊµ×ÊÁÏ)","×¢²áµ¥Ê§°Üµµ(ÖÐÎÄÌîÐ´)","×¢²áµ¥Ê§°Üµµ(¹ý¶àµÄID)","×¢²áµ¥Ê§°Üµµ(²»ÄÜ´©Ëó×¢²á" ,"Éí·ÝÈ·ÈÏÍê³Éµµ        ","Éí·ÝÈ·ÈÏÊ§°Üµµ","²»¿É×¢²áµÄ ID         ", "²»¿ÉµÇÂ¼µÄ IP", "ÏµÍ³×Ô¶¯¹ýÂËµÄ´ÊÓï", "sysconf.ini", "WWWÖ÷²Ëµ¥",  "½üÆÚÈÈµã", "¸øÐÂÈÎ°æÖ÷µÄÐÅ", "¸øÒþÉíÓÃ»§µÄÐÅ", "¸ø³¤ÆÚÓÃ»§µÄÐÅ", NULL};
#else
char *explain_file[]={"Welcome","¹«ÓÃ±¸ÍüÂ¼","½øÕ¾»¶Ó­µµ","»î¶¯¿´°æ","ÀëÕ¾»­Ãæ", "menu.ini","´©ËóIP","Éí·ÝÈ·ÈÏµµ","×¢²áµ¥Íê³Éµµ","×¢²áµ¥Ê§°Üµµ(ÕæÊµÐÕÃû)","×¢²áµ¥Ê§°Üµµ(·þÎñµ¥Î»)","×¢²áµ¥Ê§°Üµµ(¾Ó×¡µØÖ·)","×¢²áµ¥Ê§°Üµµ(ÁªÂçµç»°)","×¢²áµ¥Ê§°Üµµ(ÕæÊµ×ÊÁÏ)","×¢²áµ¥Ê§°Üµµ(ÖÐÎÄÌîÐ´)","×¢²áµ¥Ê§°Üµµ(¹ý¶àµÄID)","×¢²áµ¥Ê§°Üµµ(²»ÄÜ´©Ëó×¢²á)" ,"Éí·ÝÈ·ÈÏÍê³Éµµ        ","Éí·ÝÈ·ÈÏÊ§°Üµµ","²»¿É×¢²áµÄ ID         ", "²»¿ÉµÇÂ¼µÄ IP", "ÏµÍ³×Ô¶¯¹ýÂËµÄ´ÊÓï    ", "sysconf.ini", "WWWÖ÷²Ëµ¥             ", "½üÆÚÈÈµã","¸øÐÂÈÎ°æÖ÷µÄÐÅ        ", "¸øÒþÉíÓÃ»§µÄÐÅ","¸ø³¤ÆÚÓÃ»§µÄÐÅ        ",NULL};
#endif
