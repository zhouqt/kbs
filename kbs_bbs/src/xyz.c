...
unsigned int setperms(unsigned int pbits, unsigned int basic, char *prompt, int numbers, int (*show) (struct _select_def *, int), int (*select) (struct _select_def *))
{
    ...
    move(4, 0);
    prints("Çë°´ÏÂÄãÒªµÄ´úÂëÀ´Éè¶¨%s, Ctrl+QÍË³ö£¬Ctrl+A·ÅÆúÐÞ¸ÄÍË³ö.\n", prompt);
    move(6, 0);
    clrtobot();
...
int confirm_delete_id()
{
    ...
    prints("\n");
    clrtoeol();
    getdata(9,0,"ÇëÊäÈëÒªÈ·ÈÏÇå³ýµÄÓÃ»§ID: ", genbuf, IDLEN+1,1,NULL,true);
    if (genbuf[0] == '\0') {
        clear();
        return 1;
    }
...
int x_level()
{
    ...
    clear();
    move(0, 0);
    prints("¸ü¸Ä" NAME_USER_SHORT "µÄÈ¨ÏÞ(\x1b[m×¢Òâ£ºÈç¹ûÊÇ·â½û½â·â£¬ÇëÊ¹ÓÃ·â½ûÑ¡µ¥£¡\x1b[m\n");
    clrtoeol();
    move(1, 0);
...
/*Bad 2002.7.6 ÊÜÏÞÓë½äÍøÎÊÌâ*/
    ...
    newlevel = setperms(lookupuser->userlevel, basicperm, "È¨ÏÞ", NUMPERMS, setperm_show, NULL);
    move(2, 0);
    if (newlevel == lookupuser->userlevel)
        ...
        prints(NAME_USER_SHORT " '%s' µÄÈ¨ÏÞÒÑ¸ü¸Ä,×¢Òâ£ºÈç¹ûÊÇ·â½û½â·â£¬\n", lookupuser->userid);
        ...
        if ((lookupuser->userlevel & PERM_BOARDS) && flag == 0)
            mail_file(currentuser->userid, "etc/forbm", lookupuser->userid, "ÐÂÈÎ" NAME_BM "±Ø¶Á", BBSPOST_LINK);
        /*
         * Bigman 2000.1.5 ÐÞ¸ÄÈ¨ÏÞ×Ô¶¯·¢ÐÅ 
         */
        if ((lookupuser->userlevel & PERM_CLOAK) && flag1 == 0)
            mail_file(currentuser->userid, "etc/forcloak", lookupuser->userid, NAME_SYSOP_GROUP "ÊÚÓèÄúÒþÉíÈ¨ÏÞ", BBSPOST_LINK);
        if ((lookupuser->userlevel & PERM_XEMPT) && flag2 == 0)
            mail_file(currentuser->userid, "etc/forlongid", lookupuser->userid, NAME_SYSOP_GROUP "ÊÚÓèÄú³¤ÆÚÕÊºÅÈ¨ÏÞ", BBSPOST_LINK);
...
int x_userdefine()
{
    ...
    clrtobot();
    move(2, 0);
    newlevel = setperms(lookupuser->userdefine, 0, "²ÎÊý", NUMDEFINES, showuserdefine, NULL);
    move(2, 0);
...
int x_cloak()
{
    ...
    if (!uinfo.in_chat) {
        move(1, 0);
        clrtoeol();
        prints("ÒþÉíÊõ (cloak) ÒÑ¾­%sÁË!", (uinfo.invisible) ? "Æô¶¯" : "Í£Ö¹");
        pressreturn();
    }
    return 0;
}
...
void a_edits()
{
    ...
    static const char *explain_file[] =
        ...
        "Éí·ÝÈ·ÈÏÍê³Éµµ        ",
        "Éí·ÝÈ·ÈÏÊ§°Üµµ",
        "²»¿É×¢²áµÄ ID         ", "²»¿ÉµÇÂ¼µÄ IP", "ÏµÍ³×Ô¶¯¹ýÂËµÄ´ÊÓï    ", "sysconf.ini", "WWWÖ÷²Ëµ¥             ", "½üÆÚÈÈµã", "¸øÐÂÈÎ°æÖ÷µÄÐÅ        ", "¸øÒþÉíÓÃ»§µÄÐÅ",
        "¸ø³¤ÆÚÓÃ»§µÄÐÅ        ",
        "×ªÐÅ°æºÍÐÂÎÅ×é¶ÔÓ¦",
        "·â½ûÀíÓÉÁÐ±í", NULL
    };
...
int flag;
char result[];
/* ²éÕÒIPµÄÖ÷³ÌÐò£¬flag£º1	ÆÕÍ¨µ÷ÓÃ£¬´òÓ¡ËùÓÐÐÅÏ¢ */
