...
/*Bad 2002.7.6 ÊÜÏÞÓë½äÍøÎÊÌâ*/
    ...
    newlevel = setperms(lookupuser->userlevel,basicperm,"È¨ÏÞ",NUMPERMS,setperm_show);
    move(2,0);
...
XCheckLevel() /* Leeward 98.06.05 */
{
    ...
    newlevel = setperms(scanuser.userlevel, 0, "È¨ÏÞ",NUMPERMS,showperminfo);
    move(2,0);
...
x_userdefine()
{
    ...
    clrtobot();
    move(2,0);
    newlevel = setperms(lookupuser->userdefine,0 , "²ÎÊý",NUMDEFINES,showuserdefine);
    move(2,0);
