...
/*Bad 2002.7.6 ÊÜÏÞÓë½äÍøÎÊÌâ*/
    ...
    if (newlevel == lookupuser->userlevel)
        ...
        if ((lookupuser->userlevel & PERM_CLOAK) && flag1 == 0)
            mail_file(currentuser->userid, "etc/forcloak", lookupuser->userid, NAME_SYSOP_GROUP "ÊÚÓèÄúÒþÉíÈ¨ÏÞ", BBSPOST_LINK, NULL);
        if ((lookupuser->userlevel & PERM_XEMPT) && flag2 == 0)
            mail_file(currentuser->userid, "etc/forlongid", lookupuser->userid, NAME_SYSOP_GROUP "ÊÚÓèÄú³¤ÆÚÕÊºÅÈ¨ÏÞ", BBSPOST_LINK, NULL);
...
void a_edits()
{
    ...
    static const char *explain_file[] =
        ...
        "Éí·ÝÈ·ÈÏÍê³Éµµ        ",
        "Éí·ÝÈ·ÈÏÊ§°Üµµ",
        "²»¿É×¢²áµÄ ID         ", "²»¿ÉµÇÂ¼µÄ IP", "ÏµÍ³×Ô¶¯¹ýÂËµÄ´ÊÓï    ",
        "sysconf.ini", "WWWÖ÷²Ëµ¥             ", "½üÆÚÈÈµã", "ÏµÍ³ÈÈµã",
        "¸øÐÂÈÎ°æÖ÷µÄÐÅ        ", "¸øÒþÉíÓÃ»§µÄÐÅ",
        "¸ø³¤ÆÚÓÃ»§µÄÐÅ        ",
        "×ªÐÅ°æºÍÐÂÎÅ×é¶ÔÓ¦",
        "·â½ûÀíÓÉÁÐ±í", "ÐÂÓÃ»§¸öÈË¶¨ÖÆÇø","¸øÐÂ×¢²áÓÃ»§µÄÐÅ", NULL
    };
