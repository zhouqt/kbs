...
int pc_add_user()
{
	...
	if(getuser(ans, &lookupuser) != 0){
		if( !HAS_PERM(lookupuser, PERM_BASIC) ){
			move(7,0);
			prints("´ËÓÃ»§ÉĞÎŞ»ù±¾È¨ÏŞ£¬ÎŞ·¨¿ªÆô¸öÈËÎÄ¼¯");
			pressanykey();
			return 0;
		}
	}else{
	...
	if( get_pc_users(&pu, lookupuser->userid) ){
		...
		if(ans[0]=='2'){
			...
			if( del_pc_users( &pu ) ){
				...
				prints("É¾³ı³É¹¦");
				pressanykey();
				sprintf(buf1, "%s È¡Ïû %s µÄ¸öÈËÎÄ¼¯", currentuser->userid, lookupuser->userid);
		...
		move(4,0);
		prints("¸ÃÓÃ»§ÉĞÃ»ÓĞ¸öÈËÎÄ¼¯,Ìí¼Ó½øĞĞÖĞ,²»ÊäÈë¸öÈËÎÄ¼¯Ãû³Æ×Ô¶¯È¡Ïû");
	}
	...
	sprintf(sql, "¸öÈËÎÄ¼¯ÔÊĞí×î¶àÎÄÕÂÊı [%s]:",ans);
	...
	sprintf(sql, "¸öÈËÎÄ¼¯ÔÊĞí×î¶àÄ¿Â¼Êı [%s]:",ans);
	...
	if(pu.uid)
		getdata(15,0,"È·¶¨ĞŞ¸Ä? (Y/N) [N]:",ans,3, DOECHO,NULL,true);
	else
	...
	if(add_pc_users(&pu) ){
		...
		if(pu.uid)
			...
			prints("Ìí¼Ó³É¹¦,ÓÃ»§%sÒÑ¾­ÓµÓĞ¸öÈËÎÄ¼¯",lookupuser->userid);
		pressanykey();
		sprintf(buf1, "%s Ôö¼Ó %s µÄ¸öÈËÎÄ¼¯", currentuser->userid, lookupuser->userid);
        securityreport(buf1, lookupuser, NULL);
		return 1;
	}else{
		move(18,0);
...
static int pc_change_friend()
{
    ...
    while (1) {
        ...
        if (count)
            getdata(1, 0, "(A)Ôö¼Ó (D)É¾³ıor (E)Àë¿ª[E]",
        ...
        if (*ans == 'A' || *ans == 'a') {
            ...
            move(1, 0);
            namecomplete("É¾³ı¸öÈËÎÄ¼¯ºÃÓÑ³ÉÔ±: ", uident);
            if (uident[0] != '\0') {
...
int pc_selusr( char prefix)
{
	...
	if(ret <= 0){
		move(3,0);
		prints("Êı¾İ´íÎó»òÕßÉĞÎ´ÓĞ¸Ã×ÖÄ¸¿ªÍ·µÄÓÃ»§¸öÈËÎÄ¼¯");
		pressanykey();
		return -1;
	}
...
int pc_conv_node_to_file( unsigned long nid ,char *fname)
{
	...
	snprintf(buf, 255, "±ê  Ìâ: %s\n",pn.subject);
	write(fd, buf, strlen(buf));
	snprintf(buf, 255, "·¢ĞÅÕ¾: %s (%24.24s), ÎÄ¼¯\n\n",BBS_FULL_NAME, ctime(&pn.created));
	write(fd, buf, strlen(buf));
...
static int pc_dir_key(struct _select_def *conf, int key)
{
	...
	case 'd':
		...
		if( pc_dirmode == 5 ){
			...
			if(get_pc_nodes(&pn, pc_u->uid,pc_n[conf->pos-conf->page_pos].nid,-1, 3, 0, 1, 0)>0){
				move(t_lines -1, 0);
				clrtoeol();
				prints("²»ÄÜÉ¾³ı·Ç¿ÕÄ¿Â¼,°´ÈÎÒâ¼ü¼ÌĞø");
				igetkey();
				update_endline();
				return SHOW_CONTINUE;
			}
	...
	case 'D':
	{
		...
		if( pc_dirmode != 5 )
			return SHOW_CONTINUE;
		clear();
		getdata(1,0,"È·ÊµÒªÇå¿ÕÀ¬»øÏäÂğ? (Y/N) [N]:",ans,3,DOECHO,NULL,true);
		if( ans[0] != 'y' && ans[0] != 'Y' )
			return SHOW_REFRESH;
	...
	case 'c':
		...
		move(t_lines -1, 0);
		clrtoeol();
		prints("ÒÑ¾­¸´ÖÆ¸ÃÌõÄ¿µ½¼ôÌù°åÖĞ,°´ÈÎÒâ¼ü¼ÌĞø");
		igetkey();
		update_endline();
		return SHOW_CONTINUE;
		break;
	case 'p':
		...
		if( pc_pasteboard <= 0 ){
			move(t_lines -1, 0);
			clrtoeol();
			prints("¼ôÌù°åÃ»ÓĞÄÚÈİ,°´ÈÎÒâ¼ü¼ÌĞø");
			igetkey();
			update_endline();
			return SHOW_DIRCHANGE;
		}
...
//	pc_dir_start = 0;
	...
	if( i == 0 ){
		...
		clear();
		move(7,0);
		prints("±¾ÇøÔİÊ±Ã»ÓĞÎÄÕÂ,Ôö¼ÓĞÂÎÄÕÂ");
		if( ! pc_add_a_node(0) ){
			free(pts);
			if(first)
				free(pc_n);
...
int pc_read_comment()
{
	...
	if( i == 0 ){
		...
		clear();
		getdata(7,0, "±¾ÎÄÔİÊ±Ã»ÓĞÆÀÂÛ,Ôö¼ÓĞÂÆÀÂÛÂğ? (Y/N) [N]:",ans,3, DOECHO,NULL,true);
...
int import_to_pc(int ent, struct fileheader *fileinfo, char *direct)
{
...

	{
		...
		clrtoeol();
		if( ret )
   		 	getdata(t_lines-1, 0, "ÊÕÂ¼µ½¸öÈËÎÄ¼¯³É¹¦£¬°´»Ø³µ¼ÌĞø<<", buf, 3, NOECHO, NULL, true);
		else
   		 	getdata(t_lines-1, 0, "ÊÕÂ¼µ½¸öÈËÎÄ¼¯Ê§°Ü£¬°´»Ø³µ¼ÌĞø<<", buf, 3, NOECHO, NULL, true);
	}
