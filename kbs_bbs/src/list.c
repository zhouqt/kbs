...
int print_title()
{
    ...
    docmdtitle((friendmode) ? "[ºÃÅóÓÑÁÐ±í]" : "[Ê¹ÓÃÕßÁÐ±í]", " ÁÄÌì[\x1b[1;32mt\x1b[m] ¼ÄÐÅ[\x1b[1;32mm\x1b[m] ËÍÑ¶Ï¢[\x1b[1;32ms\x1b[m] ¼Ó,¼õÅóÓÑ[\x1b[1;32mo\x1b[m,\x1b[1;32md\x1b[m] ¿´ËµÃ÷µµ[\x1b[1;32m¡ú\x1b[m,\x1b[1;32mr\x1b[m] ÇÐ»»Ä£Ê½ [\x1b[1;32mf\x1b[m] Çó¾È[\x1b[1;32mh\x1b[m]");
#else
    docmdtitle((friendmode) ? "[ºÃÅóÓÑÁÐ±í]" : "[Ê¹ÓÃÕßÁÐ±í]", " ÁÄÌì[t] ¼ÄÐÅ[m] ËÍÑ¶Ï¢[s] ¼Ó,¼õÅóÓÑ[o,d] ¿´ËµÃ÷µµ[¡ú,r] ÇÐ»»Ä£Ê½ [f] Çó¾È[h]");
#endif
    update_endline();
    return 0;
}
...
{
#ifdef  SITE_HIGHCOLOR
    docmdtitle((friendmode) ? "[ºÃÅóÓÑÁÐ±í]" : "[Ê¹ÓÃÕßÁÐ±í]", "          ¼ÄÐÅ[\x1b[1;32mm\x1b[m] ¼Ó,¼õÅóÓÑ[\x1b[1;32mo\x1b[m,\x1b[1;32md\x1b[m] ¿´ËµÃ÷µµ[\x1b[1;32m¡ú\x1b[m,\x1b[1;32mr\x1b[m] Ñ¡Ôñ[\x1b[1;32m¡ü\x1b[m,\x1b[1;32m¡ý\x1b[m] Çó¾È[\x1b[1;32mh\x1b[m]");
#else
    docmdtitle((friendmode) ? "[ºÃÅóÓÑÁÐ±í]" : "[Ê¹ÓÃÕßÁÐ±í]", "          ¼ÄÐÅ[m] ¼Ó,¼õÅóÓÑ[o,d] ¿´ËµÃ÷µµ[¡ú,r] Ñ¡Ôñ[¡ü,¡ý] Çó¾È[h]");
#endif
    update_endline();
    return 0;
}
...
int do_userlist()
{
    ...
    for (i = 0; i < BBS_PAGESIZE && i + page < range; i++) {
        ...
        if (user_record[i + page] == NULL) {
            clear();
            prints("[1;31mÏµÍ³³öÏÖBug,Çëµ½Sysop°æ±¨¸æ£¬Ð»Ð»![m");
            oflush();
            sleep(10);
            exit(0);
        }
...

{
    ...
    case 'K':
        ...
        sprintf(buf, "ÄãÒª°Ñ %s Ìß³öÕ¾ÍâÂð (Yes/No) [N]: ", user_record[allnum]->userid);
        move(BBS_PAGESIZE + 3, 0);
        clrtoeol();
    ...
    case 'O':
        ...
        if (addtooverride(user_record[allnum]->userid) == -1) {
            sprintf(buf, "%s ÒÑÔÚÅóÓÑÃûµ¥", user_record[allnum]->userid);
        }

        else {
            sprintf(buf, "%s ÁÐÈëÅóÓÑÃûµ¥", user_record[allnum]->userid);
        }
        msgflag = true;
        break;
    case 'd':
    case 'D':
         ...
         */
        sprintf(buf, "ÄãÒª°Ñ %s ´ÓÅóÓÑÃûµ¥ÒÆ³ýÂð (Y/N) [N]: ", user_record[allnum]->userid);
        move(BBS_PAGESIZE + 3, 0);
        clrtoeol();
        getdata(BBS_PAGESIZE + 3, 0, buf, genbuf, 4, DOECHO, NULL, true);
        move(BBS_PAGESIZE + 3, 0);
...

{
    ...
    case 'O':
        ...
        if (addtooverride(user_data[allnum - pagenum].userid) == -1) {
            ...
            sprintf(buf, "%s ÁÐÈëÅóÓÑÃûµ¥", user_data[allnum - pagenum].userid);
            show_message(buf);
        }
        msgflag = true;
        if (!friendmode)
    ...
    case 'D':
         ...
         * Leeward: 97.12.19: confirm removing operation 
         */
        sprintf(buf, "ÄãÒª°Ñ %s ´ÓÅóÓÑÃûµ¥ÒÆ³ýÂð (Y/N) [N]: ", user_data[allnum - pagenum].userid);
        move(BBS_PAGESIZE + 3, 0);
        clrtoeol();
        getdata(BBS_PAGESIZE + 3, 0, buf, genbuf, 4, DOECHO, NULL, true);
        move(BBS_PAGESIZE + 3, 0);
        ...
        if (deleteoverride(user_data[allnum - pagenum].userid) == -1) {
            sprintf(buf, "%s ±¾À´¾Í²»ÔÚÅóÓÑÃûµ¥ÖÐ", user_data[allnum - pagenum].userid);
            show_message(buf);
        }

        else {
            sprintf(buf, "%s ÒÑ´ÓÅóÓÑÃûµ¥ÒÆ³ý", user_data[allnum - pagenum].userid);
            show_message(buf);
        }
        msgflag = true;
        if (!friendmode)
...
#ifdef  SITE_HIGHCOLOR
        prints
            ("[44mÁÄÌì[\x1b[1;32mt\x1b[0m] ¼ÄÐÅ[\x1b[1;32mm\x1b[0m] ËÍÑ¶Ï¢[\x1b[1;32ms\x1b[0m] ¼Ó,¼õÅóÓÑ[\x1b[1;32mo\x1b[0m,\x1b[1;32md\x1b[0m] Ñ¡ÔñÊ¹ÓÃÕß[\x1b[1;32m¡ü\x1b[0m,\x1b[1;32m¡ý\x1b[0m] ÇÐ»»Ä£Ê½ [\x1b[1;32mf\x1b[0m] Çó¾È[\x1b[1;32mh\x1b[0m][m");
#else
        prints("\x1b[44mÁÄÌì[t] ¼ÄÐÅ[m] ËÍÑ¶Ï¢[s] ¼Ó,¼õÅóÓÑ[o,d] Ñ¡ÔñÊ¹ÓÃÕß[¡ü,¡ý] ÇÐ»»Ä£Ê½ [f] Çó¾È[h]\x1b[m");
#endif
    }
    return 0;
}
