...
void chat_show_allmsgs()
{
    ...
    {
        printchatline("Ã»ÓÐÈÎºÎµÄÑ¶Ï¢´æÔÚ£¡£¡");
    }
}
...
int mode;
{
    ...
    *msgbak = 0;	/* period 2000-11-20 may be used without init */
    ...
    {/*
            move(2,0) ;
            prints("¶Ô·½ÕýÔÚ·¢ÎÄÕÂ»òÐ´ÐÅ£¬ÇëÉÔºòÔÙ·¢»ò¸øËû(Ëý)Ð´ÐÅ...\n");
            pressreturn() ;
            move(2,0) ;
            clrtoeol() ;
            return -1 ;*/
...
struct user_info *uin;
{
    ...
    if (strcmp(uin->userid, "guest")) /* Leeward 98.06.19 */
    {
        ...
        do_sendmsg(uin,buf2,0); /* ¹ã²¥Ê±±ÜÃâ±»¹ý¶àµÄ guest ´ò¶Ï */
    }
}
...
r_msg()
{
    ...
    if((uinfo.mode == POSTING || uinfo.mode == SMAIL) && !DEFINE(DEF_LOGININFORM))/*Haohmaru.99.12.16.·¢ÎÄÕÂÊ±²»»Ømsg*/
    {
