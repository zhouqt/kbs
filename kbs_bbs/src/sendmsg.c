...
int get_msg(char * uid, char * msg, int line, int sms)
{
    ...
    while (1) {
		...
		if(sms){
			char tmp[100];
			sprintf(tmp,"È·¶¨ÒªËÍ³öÂğ(¹²%d×Ö½Ú,²»°üÀ¨Ç°ºó×º)(Y)ÊÇµÄ (N)²»Òª (E)ÔÙ±à¼­? [Y]: ", strlen(msg));
    ...
    if (uentp == NULL) {
        ...
        if( namecomplete(NULL, uident) == '#' ){
    		...
    		if(!currentmemo->ud.mobileregistered)
				return 0;
            getdata(1, 0, "ËÍ¶ÌĞÅ¸ø:", uident, MOBILE_NUMBER_LEN+1, true, NULL, true);
			if( uident[0] == 0)
				return 0;
			if( !isdigit( uident[0] ) ){
				char mobile[STRLEN];
...
//    if(!curruserdata.mobileregistered) {
    ...
    if (dest == NULL) {
        move(1, 0);
        clrtobot();
        getdata(1, 0, "ËÍ¶ÌĞÅ¸ø: ", uident, 15, 1, 0, 1);
        if (uident[0] == '\0') {
            clear();
