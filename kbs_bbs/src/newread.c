...
/* »ñµÃÉÏÒ»´ÎÔÄ¶ÁµÄÎ»ÖÃ
  * @param mode ÔÄ¶ÁÄ£Ê½
  * @param direct dirµÄÎÄ¼þÃû
  * @return ÉÏ´ÎÎ»ÖÃ£¬Îª0ÔòÊ§°Ü
*/
...
/* Ñ°ÕÒÏÂÒ»¸öÎ´¶ÁÎÄÕÂ£¬ÕÒµ½·µ»ØÎ»ÖÃ£¬·ñÔò·µ»Ø0*/
...
/*ÆäÊµÕâÀïÓÐ¸öÎÊÌâ£¬ÔÚsavePosµÄÊ±ºò£¬arg->directÆäÊµÒÑ¾­±»¸Ä¶¯³ÉÐÂµÄdirectÁË
  ÕýÈ·µÄ×ö·¨ÊÇÀàËÆnewmodeµÄÐ´·¨£¬ÔÚÕâÀï±£´æÔ­À´µÄ¡£
  µ«ÊÇ£¬ÒòÎª·µ»ØNEWDIRECTµÄÖ»ÓÐÔÄ¶ÁµÄÊ±ºò£¬¶øÔÄ¶ÁµÄÊ±ºòÖ»ÓÃµ±Ç°°æÃæ×÷Îªkey,
  ËùÒÔ£¬ÆäÊµarg->directÊÇÎÞÓÃµÄ¡£
  ÁíÒ»¸öÎÊÌâ£¬ÔÚÕâÀï£¬conf->posÃ»·¨Éè¶¨³Éfilecount,ÒòÎª´ËÊ±»¹Ã»ÓÐgetdata
*/
        ...
        case GOTO_NEXT:
            ...
            } else  if (arg->readmode==READ_THREAD) { /* ´¦ÀíÍ¬Ö÷ÌâÔÄ¶Á*/
		...
		if (arg->oldpos!=0) {
                /*»Ö¸´µ½Ô­À´µÄÎ»ÖÃ*/
                list_select_add_key(conf,KEY_REFRESH); //ÏÈË¢ÐÂÒ»ÏÂ
    ...
    if ((ret!=SHOW_SELCHANGE)&&!list_select_has_key(conf)) /*·µ»Ø·ÇË³ÐòÔÄ¶ÁÄ£Ê½*/
        arg->readmode=READ_NORMAL;
    return ret;
}
...
static int read_prekey(struct _select_def *conf, int *command)
{
    ...
    switch (*command) {
        ...
        case 'P': //·­Ò³¿ØÖÆÊ¹ÓÃarg->filecount£¬²»µ½´ïÖÃ¶¥
        case KEY_PGUP:
        case Ctrl('B'):
...
int new_i_read(enum BBS_DIR_MODE cmdmode, char *direct, void (*dotitle) (struct _select_def*), READ_ENT_FUNC doentry, struct key_command *rcmdlist, int ssize)
{
    ...
    if ((arg.mode==DIR_MODE_NORMAL)||
        ...
        //ÉèÖÃÖÃ¶¥µÄ.DIR direct TODO:ÓÃtmpfs
...
int post_search(struct _select_def* conf, struct fileheader* fh, void* extraarg)
{
    ...
    snprintf(pmt, STRLEN, "ËÑÑ°%sµÄÎÄÕÂ [%s]: ", up ? "ÍùÏÈÇ°":"ÍùºóÀ´", ans);
    move(t_lines - 1, 0);
    clrtoeol();
...
bool isThreadTitle(char* a,char* b)
{
    ...
    if (!strncmp(a, "©À ", 3)) a+=3;
    if (!strncmp(b, "©À ", 3)) b+=3;
    if (!strncmp(a, "©¸ ", 3)) a+=3;
    if (!strncmp(b, "©¸ ", 3)) b+=3;
