...
void mem_printline(struct MemMoreLines *l, char *fn,char* begin)
{
	...
	return;
        ...
        if (current_attach_link)
            ...
            strcpy(link,"(ÓÃwww·½Ê½ÔÄ¶Á±¾ÎÄ¿ÉÒÔÏÂÔØ´Ë¸½¼þ)");
...
/*	prints
    ...
    if (currentuser != NULL && DEFINE(currentuser, DEF_HIGHCOLOR))
        prints("[1;44m[32mÏÂÃæ»¹ÓÐà¸ (%d%%)  µÚ(%d-%d)ÐÐ  ¹²%dÐÐ[33m | gÌø×ª | jkÉÏÏÂÆª | /?ËÑË÷ |", total ? (100 * l2 / total) : (100 * read / size), l1, l2, total);
    else
        prints("[44m[32mÏÂÃæ»¹ÓÐà¸ (%d%%)  µÚ(%d-%d)ÐÐ  ¹²%dÐÐ[33m | gÌø×ª | jkÉÏÏÂÆª | /?ËÑË÷ |", total ? (100 * l2 / total) : (100 * read / size), l1, l2, total);
    clrtoeol();
    resetcolor();
}
...
/*    move(t_lines/2+1, 0);*/
    ...
    for (i = 0, j = 0;;) {
        ...
        if (!header || (!((i == 0) && ((!strncmp(l.curr, "·¢ÐÅÈË: ", 8) || (!strncmp(l.curr, "¼ÄÐÅÈË: ", 8))))) &&
                        !((i == 1) && !strncmp(l.curr, "±ê  Ìâ: ", 8)) && !((i == 2) && !strncmp(l.curr, "·¢ÐÅÕ¾: ", 8)) && !((i == 3) && (l.currlen == 0)))) {
