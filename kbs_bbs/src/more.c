...
int NNread_init()
{
    ...
    /*---	Ô­ÓĞ³ÌĞòË³ĞòÓĞÎó, !DEFINE --> returnºóÃ»close	---*/
    if ((fffd = fopen("etc/movie", "r")) == NULL)
	return 0;
    /*---	---*/
...
void setcalltime()
{
    ...
    move(1, 0);
    clrtoeol();
    getdata(1, 0, "¼¸·ÖÖÓááÒªÏµÍ³ÌáĞÑÄã: ", ans, 3, DOECHO, NULL, true);
    if (!isdigit(ans[0]))
	return;
    ttt = atoi(ans);
    if (ttt <= 0)
...
int morekey()
{
	...
	case KEY_UP:
	    return KEY_UP;
	    /*************** ĞÂÔö¼ÓÔÄ¶ÁÊ±µÄÈÈ¼ü Luzi 1997.11.1 ****************/
	case 'h':
	case '?':
	    return 'H';
	case 'o':
...
static int rawmore(char *filename, int promptend, int row, int numlines)
{
	...
	if (linesread <= numlines || numlines == 0) {
		...
		move(t_lines - 1, 0);
		clrtoeol();
		prints("[44m[32mÏÂÃæ»¹ÓĞà¸ (%d%%)[33m   ©¦ ½áÊø ¡û <q> ©¦ ¡ü/¡ı/PgUp/PgDn ÒÆ¶¯ ©¦ ? ¸¨ÖúËµÃ÷ ©¦     [m", (viewed * 100) / tsize);
		ch = morekey();
		move(t_lines - 1, 0);
		clrtoeol();
		refresh();
		...
		/* Luzi  ĞÂÔöÔÄ¶ÁÈÈ¼ü 1997.11.1 */
			...
			for (count = 0; count < t_lines; count++)
			    if (pID = strstr(big_picture[count].data, "ÓÃ»§´úºÅ(êÇ³Æ) : "))
				break;
			pID = pID ? pID + 17 : err;
			...
			strcat(buffer, " ·¢ĞÅ£¬ÒªÇóÆä²¹Æë¸öÈË×¢²á×ÊÁÏ");
			if (askyn(buffer, 0)) {
			    ...
			    fp = fopen(filepath, "w");
			    if (fp) {
				fprintf(fp, "¼ÄĞÅÈË: BBS " NAME_BBS_CHINESE "Õ¾" NAME_MATTER "¹ÜÀíÏµÍ³\n");
				fprintf(fp, "±ê  Ìâ: ÄúµÄ¸öÈË×ÊÁÏ²»¹»ÏêÊµ£¬Çëµ½¸öÈË¹¤¾ßÏäÄÚ²¹³äÉè¶¨\n");
				fprintf(fp, "·¢ĞÅÕ¾: BBS " NAME_BBS_CHINESE "Õ¾ (%24.24s)\n", ctime(&now));
				fprintf(fp, "À´  Ô´: %s \n\n", currentuser->lasthost);
				fprintf(fp, "[1m[33mÄúµÄ¸öÈË×ÊÁÏ²»¹»ÏêÊµ£¬ÇëÂíÉÏµ½¸öÈË¹¤¾ßÏäÄÚ²¹³äÉè¶¨¡£[0m[0m\n\n");
				fprintf(fp, "Éè¶¨Ê±Çë×¢Òâ£º\n\n"
					"        1) ÇëÌîĞ´ÕæÊµĞÕÃû.           (¿ÉÓÃÆ´Òô)\n"
					"        2) ÇëÏêÌîÑ§Ğ£¿ÆÏµÓëÄê¼¶.     (×¢Ã÷Äê¼¶»ò¹¤×÷µ¥Î»£¬Ã»ÓĞµ¥Î»µÄĞ´ÎŞÒµ)\n"
					"        3) ÇëÌîĞ´ÍêÕûµÄ×¡Ö·×ÊÁÏ.     (ÍêÕûÏêÏ¸£¬ÄÜ¹»Í¨ĞÅ)\n"
					"        4) ÇëÏêÌîÁ¬Âçµç»°.           (Ã»ÓĞµç»°Çë×¢Ã÷¡°ÎŞ¡±)\n"
					"        5) ÇëÌîĞ´ÓĞĞ§µÄµç×ÓÓÊ¼şĞÅÏä. (ÄÜ¹»Í¨ĞÅ£¬Ã»ÓĞÇë×¢Ã÷¡°ÎŞ¡±)\n" "        6) ²»ÔÊĞíÊ¹ÓÃ²»ÑÅ»òÕßÊÇ²»Ç¡µ±µÄêÇ³Æ.\n" "        7) Çë¾¡Á¿Ê¹ÓÃÖĞÎÄÌîĞ´.\n\n");
				fprintf(fp, "[1m[31mÄ¿Ç°£¬Õ¾Îñ¹ÜÀíÏµÍ³ÒÑÔİÊ±È¡ÏûÄúÔÚ±¾Õ¾µÄÕ¾Îñ¹ÜÀíÈ¨ÏŞ£¬\nµÈÄú½«¸öÈË×ÊÁÏ²¹³äÉè¶¨ºÏ¸ñºó£¬ÇëĞ´ĞÅ¸øÖ÷¹ÜÄúÖ°ÎñµÄÕ¾³¤ÉêÇë»Ö¸´È¨ÏŞ¡£[0m[0m\n");

				fclose(fp);
			    } else
				a_prompt(-1, "¾¯¸æ£ºÎŞ·¨Éú³ÉĞÅ¼şÕıÎÄ£¬Çë°´ÈÎºÎ¼üÒÔ¼ÌĞø <<", buffer);
			    setmailfile(buffer, userid, DOT_DIR);
			    if (append_record(buffer, &newmessage, sizeof(newmessage)) == -1)
				a_prompt(-1, "´íÎó£ºÎŞ·¨Í¶µİĞÅ¼ş£¬Çë°´ÈÎºÎ¼üÒÔ¼ÌĞø <<", buffer);
			    else {
				...
				if (newlevel != lookupuser->userlevel) {
				    sprintf(buffer, "ĞŞ¸Ä %s µÄÈ¨ÏŞXPERM%d %d", lookupuser->userid, lookupuser->userlevel, newlevel);
				    ...
				    a_prompt(-1, "Ïà¹ØÈ¨ÏŞÔçÒÑÈ¡Ïû£¬½ö·¢ËÍĞÅ¼şËµÃ÷ÒªÇó£¬Çë°´ÈÎºÎ¼üÒÔ¼ÌĞø <<", buffer);
			    }
			}
		    }
...
struct MemMoreLines {
    ...
    int size;
    char *line[100];
    char ty[100];		/* 0: ÆÕÍ¨, ÓĞ»Ø³µ; 1: ÆÕÍ¨, ÎŞ»Ø³µ; 2: ÒıÎÄ, ÓĞ»Ø³µ; 3: ÒıÎÄ, ÎŞ»Ø³µ */
    int len[100];
    int s[100];
    int start;			/* this->line[start%100]ÊÇ¼ìË÷µÄĞĞºÅ×îĞ¡µÄĞĞ£¬ĞĞºÅÎª start */
    int num;			/* ¹²¼ìË÷ÁËrowµ½row+num-1ÕâÃ´¶àĞĞ */
    int curr_line;		/* µ±Ç°ÓÎ±êÎ»ÖÃ */
    char *curr;			/* µ±Ç°ÓÎ±êµÄĞĞ */
    char currty;
    int currlen;
    int total;
};
...
int effectiveline;		//ÓĞĞ§ĞĞÊı, Ö»¼ÆËãÇ°ÃæµÄ²¿·Ö, Í·²¿²»º¬, ¿ÕĞĞ²»º¬, Ç©Ãûµµ²»º¬, ÒıÑÔ²»º¬ 
...
void mem_printline(char *ptr, int len, char *fn, char ty)
{
    ...
    if (!strncmp(ptr, "¡õ ÒıÓÃ", 7) || !strncmp(ptr, "==>", 3)
	|| !strncmp(ptr, "¡¾ ÔÚ", 5) || !strncmp(ptr, "¡ù ÒıÊö", 7)) {
	outns("\033[1;33m", 7);
	outns(ptr, len);
	outns("\033[m\n", 4);
	return;
...
/*	static int n = 0;
	char *(s[4]) = {
		"½áÊø ¡û q | ¡ü ¡ı PgUp PgDn ÒÆ¶¯",
		"s ¿ªÍ· | e Ä©Î² | b f ÏòÇ°ºó·­Ò³",
		"g Ìøµ½Ö¸¶¨ĞĞ | ? / ÏòÉÏÏÂËÑË÷×Ö·û´®",
