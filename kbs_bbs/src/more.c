...
enum LINE_CODE {
    ...
    LINE_NORMAL_NOCF,        //Ã»ÓÐ »Ø³µµÄÐÐ
    LINE_QUOTA,                    //ÒýÎÄ
    LINE_QUOTA_NOCF,         //Ã»ÓÐ»Ø³µµÄÒýÎÄ
    LINE_ATTACHMENT,         //¸½¼þ
    LINE_ATTACHLINK,           //¸½¼þÁ´½Ó
    LINE_ATTACHALLLINK           //±¾ÎÄÁ´½Ó
};
...
int measure_line(char *p0, int size, int *l, int *s, char oldty, char *ty)
{
    ...
    if (oldty!=LINE_ATTACHMENT) { //ÉÏÒ»ÐÐ²»ÊÇ¸½¼þ
    ...
    if ( oldty==LINE_ATTACHMENT|| (size > ATTACHMENT_SIZE
        ...
        *ty = LINE_ATTACHMENT;
        p = p0;
        /* ÉÏÒ»ÐÐÎª¸½¼þµÄÐÐµÄsize=1,Òò´Ë-1»ñµÃÕæÕýµÄattach¿ªÊ¼*/
	 if (oldty==LINE_ATTACHMENT) p--;
	 /*  Ìø¹ýattachmentÇ°ÃæµÄPAD */
        p += ATTACHMENT_SIZE;
	...
	if (oldty==LINE_ATTACHMENT) { /*ÉÏ´ÎÊÇ¸½¼þ£¬ÏÂÒ»ÐÐÊÇ¸½¼þÁ¬½ÓÐÐ*/
          *ty=LINE_ATTACHLINK;
          *s--;
	}
	else {
...
/*	prints
    ...
    if (currentuser != NULL && DEFINE(currentuser, DEF_HIGHCOLOR))
        prints("[1;44m[32mÏÂÃæ»¹ÓÐà¸ (%d%%) µÚ(%d-%d)ÐÐ[33m | g Ìø×ª |%s / ? ËÑË÷ | s e ¿ªÍ·Ä©Î²|", total ? (100 * l2 / total) : (100 * read / size), l1, l2, uinfo.mode==READING?" l n ÉÏÏÂÆª |":"");
    else
        prints("[44m[32mÏÂÃæ»¹ÓÐà¸ (%d%%) µÚ(%d-%d)ÐÐ[33m | g Ìø×ª |%s / ? ËÑË÷ | s e ¿ªÍ·Ä©Î²|", total ? (100 * l2 / total) : (100 * read / size), l1, l2 ,uinfo.mode==READING?" l n ÉÏÏÂÆª |":"");
    clrtoeol();
    resetcolor();
}
