...
static int rawmore(char *filename, int promptend, int row, int numlines)
{
    ...
    if (fstat(fd, &st)) {
        close(fd);
                    /*---   period 2000-10-20 file should be closed here   ---*/
        return -1;
    }
                    ...
                    if (HAS_PERM(currentuser, PERM_ADMIN) && HAS_PERM(currentuser, PERM_SYSOP)
                        ...
                        if (askyn(buffer, 0)) {
                            ...
                            if (append_record(buffer, &newmessage, sizeof(newmessage)) == -1)
                                a_prompt(-1, "´íÎó£ºÎÞ·¨Í¶µÝÐÅ¼þ£¬Çë°´ÈÎºÎ¼üÒÔ¼ÌÐø <<", buffer);
                            else {
                                ...
                                if (newlevel != lookupuser->userlevel) {
                                    sprintf(buffer, "ÐÞ¸Ä %s µÄÈ¨ÏÞXPERM%d %d", lookupuser->userid, lookupuser->userlevel, newlevel);
                                    ...
                                    a_prompt(-1, "Ïà¹ØÈ¨ÏÞÔçÒÑÈ¡Ïû£¬½ö·¢ËÍÐÅ¼þËµÃ÷ÒªÇó£¬Çë°´ÈÎºÎ¼üÒÔ¼ÌÐø <<", buffer);
                            }
                        }
                    }
