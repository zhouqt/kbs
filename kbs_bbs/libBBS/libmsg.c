...
//            space=33;
            break;
        case 5:
            sprintf(dest, "[45%sm\x1b[36m%-14.14s\x1b[33m(\x1b[36m%-16.16s\x1b[33m)\x1b[37m                                                [m\n", DEFINE(currentuser, DEF_HIGHCOLOR)?";1":"", head->id, time);
...
//            space=29;
    ...
    for(i=0;i<strlen(src);i++){
        ...
        if(j==0&&pos>=80||j==1&&pos>=79||src[i]=='n'||src[i]=='r') {
            for(;pos<80;pos++)
                dest[len++]=' ';
            dest[len++]='';
            dest[len++]='[';
