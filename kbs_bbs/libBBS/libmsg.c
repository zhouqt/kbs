...
int sendmsgfunc(struct user_info *uentp, const char *msgstr, int mode)
{
    ...
    *(timestr + 8) = '\0';
    ...
    if (mode == 0 || mode == 2 || mode == 4) {
        ...
        if (mode == 3) {
            sprintf(msgbuf, "[1;44;33m’æ≥§Ï∂ %8.8s  ±π„≤•£∫" "[37m%-55.55s[m\033[%dm\n",
                    /*
                     * "[37m%-59.59s[m\033[%dm\n", 
                     */
                    timestr, msgstr, uin->pid + 100);
        } else if (mode == 1) {
            sprintf(msgbuf, "[1;44;36m%-12.12s(%-5.5s) —˚«Îƒ„[37m%-43.43s(%s)[m[%dm\033[%dm\n", currentuser->userid, timestr, msgstr, ret_str, getuinfopid() + 100, uin->pid + 100);
            sprintf(msgbak, "[1;44;37mƒ„(%-5.5s) —˚«Î%-12.12s[36m%-43.43s(%s)[m[%dm\033[%dm\n", timestr, uident, msgstr, ret_str, getuinfopid() + 100, uin->pid + 100);
        } else if (mode == 3) {
            sprintf(msgbuf, "[1;44;32mBBS œµÕ≥Õ®∏Ê(%-5.5s):[37m%-59.59s[31m(%s)[m\033[%dm\n", timestr, msgstr, ret_str, uin->pid + 100);
        } else if (mode == 5) {
	    sprintf(msgbuf,
    ...
    if (Gmode == 2)
        sprintf(msgbuf, "[1;44;33m’æ≥§Ï∂ %8.8s  ±π„≤•£∫[37m%-59.59s[m\033[%dm\n", timestr, buf, uin->pid + 100);
#ifdef BBSMAIN
