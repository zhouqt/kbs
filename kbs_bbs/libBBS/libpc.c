...
int add_pc_users(struct pc_users *pn)
{
	...
	if( pn->uid <= 0 )
		sprintf(sql,"INSERT INTO users VALUES (NULL, '%s', '%s', '%s', '%s', %d, %d, '%s', 0 ,NULL,0,0,NULL,'%s' , '' , 0 , 5 , 600);",pn->username, newcorp, newdesc, newtheme, pn->nodelimit, pn->dirlimit, tt2timestamp(pn->createtime,newts), tt2timestamp(pn->createtime,newts) );
	else
