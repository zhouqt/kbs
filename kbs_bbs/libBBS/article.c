...
void getcross(char* filepath,char* quote_file,struct userec* user,int in_mail,char* board,char* title,int Anony,int mode,char* sourceboard)      /* °Ñquote_file¸´ÖÆµ½filepath (×ªÌù»ò×Ô¶¯·¢ÐÅ)*/
{
    FILE        *inf, *of;
    char        buf[256];
    char        owner[256];
...
/* Add by SmallPig */
int post_cross(struct userec* user,char* toboard,char* fromboard,char* title,char* filename,int Anony,int in_mail,char islocal,int mode)    /* (×Ô¶¯Éú³ÉÎÄ¼þÃû) ×ªÌù»ò×Ô¶¯·¢ÐÅ */
{
    struct fileheader postfile ;
    ...
    */
    ...
    if(!mode)       /* ÓÃ»§post»¹ÊÇ×Ô¶¯·¢ÐÅ*/
        ...
        sprintf(buf,"×Ô¶¯·¢±íÏµÍ³ POST '%s' on '%s'", postfile.title, toboard) ;
    bbslog("1user",buf) ;
    return 1;
}
...
/* ½«Ä³ÎÄ¼þ POST ÔÚÄ³°æ */
{
    ...
    post_cross(user,nboard,fromboard,posttitle,filename,Anony,NA,'l',mode);  /* post ÎÄ¼þ */
    return 0;
}
