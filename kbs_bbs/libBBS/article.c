...
int cmpname(fhdr,name)  /* Haohmaru.99.3.30.±È½Ï Ä³ÎÄ¼şÃûÊÇ·ñºÍ µ±Ç°ÎÄ¼ş ÏàÍ¬ */
...
*/
        ...
        { /* Leeward 98.06.17 ÔÚÎÄÕªÇøÉ¾ÎÄ²»¼õÎÄÕÂÊıÄ¿ */
            if (owned)
            {
                ...
                {
                    user->numposts--;/*×Ô¼ºÉ¾³ıµÄÎÄÕÂ£¬¼õÉÙpostÊı*/
                }
            } else if ( !strstr(usrid,".")&&BMDEL_DECREASE&&decpost/*°æÖ÷É¾³ı,¼õÉÙPOSTÊı*/){
...
void add_loginfo(char* filepath,struct userec* user,char* currboard,int Anony)    /* POST ×îºóÒ»ĞĞ Ìí¼Ó */
{
    FILE *fp;
    int color,noidboard;
    char fname[STRLEN];
    ...
    fp=fopen(filepath,"a");
    if (!dashf(fname)||          /* ÅĞ¶ÏÊÇ·ñÒÑ¾­ ´æÔÚ Ç©Ãûµµ */
...
void getcross(char* filepath,char* quote_file,struct userec* user,int in_mail,char* board,char* title,int Anony,int mode,char* sourceboard)      /* °Ñquote_file¸´ÖÆµ½filepath (×ªÌù»ò×Ô¶¯·¢ĞÅ)*/
{
    FILE        *inf, *of;
    char        buf[256];
    char        owner[256];
    ...
    if(mode==0/*×ªÌù*/)
    {
        ...
        write_header(of,user,in_mail,sourceboard,title,Anony,1/*²»Ğ´Èë .posts*/);
