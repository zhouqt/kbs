...
/*
  * ÅÐ¶Ïµ±Ç°Ä£Ê½ÊÇ·ñ¿ÉÒÔÊ¹ÓÃid¶þ·Ö
  */
...
/*
 * Ð´dirÎÄ¼þÖ®Ç°°ÑÎÄ¼þ¶¨Î»µ½ÕýÈ·µÄÎ»ÖÃ£¬lock×¡£¬²¢·µ»ØÏàÓ¦µÄÊý¾Ý
 * ÕâÀï±ß²¢²»µ÷ÓÃfree_write_dir_argÀ´ÊÍ·Å×ÊÔ´¡£ÐèÒªÉÏ²ã×Ô¼ºÀ´
 * µ÷ÓÃ´Ëº¯ÊýÈç¹û³É¹¦£¬¼ÇµÃ¼°Ê±flock(filearg->fd,LOCK_UN)
 * @param filearg ´«ÈëµÄ½á¹¹¡£Èç¹ûfilearg->fd²»Îª-1£¬ËµÃ÷ÐèÒª´ò¿ªdirarg->directÎÄ¼þ
 *                        ·ñÔòÊ¹ÓÃfilearg->fd×÷ÎªÎÄ¼þ¾ä±ú¡£
 *                        filearg->entÎªÔ¤¼ÆµÄÎ»ÖÃ£¬ent>=1
 *                        filearg->fileptrÈç¹û²»µÈÓÚMAP_FAILED,ÄÇÃ´Õâ¸öº¯Êý»ámmapÎÄ¼þ²¢
 *                        ÌîÐ´filearg->fileptrºÍfilearg->sizeÓÃÓÚ·µ»ØÊý¾Ý
 * @param fileinfo ÓÃÓÚ¶¨Î»µÄ¶«Î÷£¬²»Îª¿ÕµÄÊ±ºòÐèÒª¶¨Î»
 * @param mode µ±Ç°Ä£Ê½£¬Ö»ÓÐsortedµÄ¿ÉÒÔÊ¹ÓÃ¶þ·Ö
 * @return 0³É¹¦,´ËÊ±mmapÍê±Ï¡£
 */
int prepare_write_dir(struct write_dir_arg * filearg,struct fileheader* fileinfo,int mode)
{
    ...
    BBS_TRY {
        ...
        if (needrelocation) { //ÖØ¶¨Î»Õâ¸öÎ»ÖÃ
            int i;
            if (is_sorted_mode(mode)) {
                ...
                filearg->ent=-1;
                /* ÏÈ´Óµ±Ç°Î»ÖÃÍùÇ°ÕÒ£¬ÒòÎªÒ»°ã¶¼ÊÇ±»É¾³ýµ¼ÖÂÏòÇ°ÁË*/
...
int do_del_post(struct userec *user, struct write_dir_arg*dirarg,struct fileheader *fileinfo, char *board, int currmode, int decpost)
{
    ...
    BBS_END;
	if (dirarg->needlock)
        flock(dirarg->fd,LOCK_UN); /*Õâ¸öÊÇÐèÒª¸Ï½ô×öµÄ*/
    if (fh.id == fh.groupid)
    ...
    owned = isowner(user, &fh);
        ...
        if ((DIR_MODE_NORMAL == currmode)        /* ²»¿ÉÒÔÓÃ ¡°NA ==¡± ÅÐ¶Ï£ºdigestmode ÈýÖµ */
            &&!((fh.accessed[0] & FILE_MARKED)
                && (fh.accessed[1] & FILE_READ)
                && (fh.accessed[0] & FILE_FORWARDED))) { /* Leeward 98.06.17 ÔÚÎÄÕªÇøÉ¾ÎÄ²»¼õÎÄÕÂÊýÄ¿ */
            if (owned) {
                if ((int) user->numposts > 0 && !junkboard(board)) {
                    user->numposts--;   /*×Ô¼ºÉ¾³ýµÄÎÄÕÂ£¬¼õÉÙpostÊý */
                }
            } else if (!strstr(fh.owner, ".") && BMDEL_DECREASE && decpost /*°æÖ÷É¾³ý,¼õÉÙPOSTÊý */ ) {
...
/*
  dirarg£¬Òª²Ù×÷µÄdir½á¹¹
  id1,id2, ÆðÊ¼±àºÅ
  É¾³ýÄ£Ê½ [del_mode = 0]±ê¼ÇÉ¾³ý [1]ÆÕÍ¨É¾³ý [2]Ç¿ÖÆÉ¾³ý
  TODO: use mmap
*/
...
#define DEL_RANGE_BUF 2048
    ...
    /*
     * curmode=4, 5µÄÇéÐÎ»òÕßÔÊÐíÇø¶ÎÉ¾³ý,»òÕß²»ÔÊÐí,Õâ¿ÉÒÔÔÚ
     * µ÷ÓÃº¯ÊýÖÐ»òÕßÈÎºÎµØ·½¸ø¶¨, ÕâÀïµÄ´úÂëÊÇ°´ÕÕ²»ÔÊÐíÉ¾³ýÐ´µÄ,
     * µ«ÊÇÎªÁËÐÞÀíÈÎºÎÔµ¹ÊÔì³ÉµÄÁÙÊ±ÎÄ¼þ¹ÊÕÏ(±ÈÈç×Ô¶¯É¾³ý»ú), »¹ÊÇ
     * ³¢ÊÔÁËÒ»ÏÂ´ò¿ª²Ù×÷; tmpfileÊÇ·ñ¶ÔÃ¿ÖÖÄ£Ê½¶ÀÁ¢, Õâ¸ö»¹ÊÇÖµµÃ
     * ÉÌÈ¶µÄ.  -- ylsdd 
     */
    if ((curmode != DIR_MODE_NORMAL)&& (curmode != DIR_MODE_MAIL)) {   /* KCN:ÔÝ²»ÔÊÐí */
        return 0;
    }
#endif                          /* 
                                 */
    ...
    if (id2 > totalcount) {
        ...
        if (dirarg->needlock)
            flock(dirarg->fd,LOCK_UN);
        getdata(6, 0, "ÎÄÕÂ±àºÅ´óÓÚÎÄÕÂ×ÜÊý£¬È·ÈÏÉ¾³ý (Y/N)? [N]: ", buf, 2, DOECHO, NULL, true);
        if (dirarg->needlock)
...
int add_digest(struct fileheader* fileinfo,char* boardname)
{
    ...
    link(newpath, path);
    append_record(dirpath, &digest, sizeof(digest));    /* ÎÄÕªÄ¿Â¼ÏÂÌí¼Ó .DIR */
...
{                               /* É¾³ýÎÄÕªÄÚÒ»ÆªPOST, dname=postÎÄ¼þÃû,boardname°æÃæÃû×Ö*/
...
/**
  ÉèÖÃfileheaderµÄÊôÐÔ
  @param dirarg ÐèÒª²Ù×÷µÄ.DIR
  @param currmode µ±Ç°dirµÄÄ£Ê½
  @param board µ±Ç°°æÃæ
  @param fileinfo ÎÄÕÂ½á¹¹
  @param flag Òª²Ù×÷µÄ±êÖ¾
  @param data ½øÐÐµÄ²Ù×÷Êý¾Ý¡£
  @param bmlog ÊÇ·ñ×ö°æÖ÷²Ù×÷¼ÇÂ¼
  TODO: ¼ì²éµ÷ÓÃÕâ¸öº¯ÊýµÄµØ·½±ØÐë¶¼¼ì²é°æÖ÷È¨ÏÞ
  @return 0 ³É¹¦
              1 ²»ÄÜ×ö²Ù×÷
              2 ÕÒ²»µ½Ô­ÎÄ
              3 ²Ù×÷ÒÑÍê³É
        ...
        struct fileheader *fileinfo, int flag,struct fileheader * data,bool dobmlog)
{
    ...
    if (flag&FILE_DIGEST_FLAG) {
        if (data->accessed[0] & FILE_DIGEST)  {     /*ÉèÖÃDIGEST*/ 
            if (dobmlog)
                bmlog(currentuser->userid, board->filename, 3, 1);
            ret=add_digest(originFh,board->filename);
        } else {/* Èç¹ûÒÑ¾­ÊÇÎÄÕªµÄ»°£¬Ôò´ÓÎÄÕªÖÐÉ¾³ý¸Ãpost */
