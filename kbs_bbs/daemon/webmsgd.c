	...
	*ptr2++ = '\0';
	...
	if (alloc_msglist_ent(destutmp, destid) < 0)
	{
		...
		snprintf(msgbuf->rawdata, sizeof(msgbuf->rawdata), 
			"ERR Allocating user %s failed\n", destid);
		write_response(msgbuf);
		return -1;
	}
	msgbuf->type = MSGD_HLO;
	snprintf(msgbuf->rawdata, sizeof(msgbuf->rawdata), 
		"HLO Allocating user %s succeeded\n", destid);
	write_response(msgbuf);
...
int delete_user(msg_t *msgbuf)
{
	...
	*ptr2++ = '\0';
	...
	if (free_msglist_ent(destutmp, destid) < 0)
	{
		msgbuf->type = MSGD_ERR;
		snprintf(msgbuf->rawdata, sizeof(msgbuf->rawdata), 
			"ERR User %s(%d) not found\n", destid, destutmp);
		write_response(msgbuf);
		return -1;
	}
	msgbuf->type = MSGD_BYE;
	snprintf(msgbuf->rawdata, sizeof(msgbuf->rawdata), 
		"BYE Freeing user %s(%d) succeeded\n", 
		destid, destutmp);
	write_response(msgbuf);
...
int read_msg(msg_t *msgbuf)
{
	...
	*/
	msgbuf->type = MSGD_OK;
	snprintf(msgbuf->rawdata, sizeof(msgbuf->rawdata), 
		"OK Ready to receive %s(%d)'s message\n",
		destid, destutmp);
	write_response(msgbuf);
	...
	*ptr++ = '\0';
	...
	if (save_msg(destutmp, destid, srcutmp, srcid, ptr) < 0)
	{
		msgbuf->type = MSGD_ERR;
		snprintf(msgbuf->rawdata, sizeof(msgbuf->rawdata), 
			"ERR Saving %s(%d)'s message failed\n",
			destid, destutmp);
		write_response(msgbuf);
		return -1;
	}
	msgbuf->type = MSGD_OK;
	snprintf(msgbuf->rawdata, sizeof(msgbuf->rawdata), 
		"OK Saving %s(%d)'s message succeeded\n",
		destid, destutmp);
	write_response(msgbuf);
